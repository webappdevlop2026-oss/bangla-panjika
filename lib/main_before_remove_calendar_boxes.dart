...
class _CalendarDayCellState extends State<_CalendarDayCell> {
  ...
  Widget build(BuildContext context) {
    ...
    final currentEvent = events.isNotEmpty
        ? events[_infoIndex % events.length]
        : null;
    ...
    final isPurnima =
        currentEvent?.category == 'purnima' || tithi.name.contains('পূর্ণিমা');

    final isAmabasya =
        currentEvent?.category == 'amabasya' || tithi.name.contains('অমাবস্যা');

    final isEkadashi =
        currentEvent?.category == 'ekadashi' || tithi.name.contains('একাদশী');
    ...
    final movingInfo = <String>[
      '$nakshatra • $rashi',
      if (currentEvent != null) '${currentEvent.icon} ${currentEvent.label}',
    ];

    final movingText = movingInfo[_infoIndex % movingInfo.length];
        ...
        child: AnimatedContainer(
          ...
          decoration: BoxDecoration(
            ...
            border: Border.all(
              ...
              width: widget.isSelected || widget.isToday ? 1.7 : 0.8,
            ),
            boxShadow: widget.isToday
                ? [
                    BoxShadow(
                      color: const Color(0xFFFFC342).withValues(alpha: 0.25),
                      blurRadius: 6,
                      spreadRadius: 1,
                    ),
                  ]
