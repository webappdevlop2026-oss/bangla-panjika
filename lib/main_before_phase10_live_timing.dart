...
class _TodaySmartLaunchCard extends StatelessWidget {
  const _TodaySmartLaunchCard();
  ...
  Widget build(BuildContext context) {
    ...
    return InkWell(
      ...
      child: Container(
        ...
        padding: const EdgeInsets.all(14),
        decoration: BoxDecoration(
          color: const Color(0xFF07172F).withValues(alpha: 0.62),
          borderRadius: BorderRadius.circular(18),
          border: Border.all(
            color: const Color(0xFFFFD36E).withValues(alpha: 0.28),
          ),
        ),
        child: Row(
          children: [
            Container(
              ...
              alignment: Alignment.center,
              decoration: BoxDecoration(
                color: const Color(0xFFFFD36E).withValues(alpha: 0.12),
                borderRadius: BorderRadius.circular(14),
              ),
            ...
            Expanded(
              child: Column(
                ...
                children: [
                  ...
                  const SizedBox(height: 3),
                  Text(
                    '${tithi.paksha} পক্ষ • ${tithi.name} • রাহুকাল ${bnTime12(rahu['start']!)}–${bnTime12(rahu['end']!)}',
                    maxLines: 2,
                    overflow: TextOverflow.ellipsis,
                    style: const TextStyle(
                      color: Colors.white70,
...
class _PanchangTimelineItem {
  final DateTime when;
  final String icon;
  final String title;
  final bool caution;

  const _PanchangTimelineItem(
    this.when,
    this.icon,
    this.title, {
    this.caution = false,
...
class TodaySmartScreen extends StatelessWidget {
  const TodaySmartScreen({super.key});
  ...
  Widget _fact(String icon, String label, String value) {
    return Container(
  ...
  Widget build(BuildContext context) {
    final now = DateTime.now();
    final info = BengaliDateUtil.monthInfoFor(now);
    final bDay = now.difference(info.start).inDays + 1;
    final sun = PanchangCalculator.sunTimes(
      now,
      lat: AppLocation.lat,
      lon: AppLocation.lon,
    ...
    final tithi = PanchangCalculator.tithiFor(sun.sunrise);
    final nakIdx = PanchangCalculator.nakshatraIndexFor(sun.sunrise);
    final rashiIdx = PanchangCalculator.rashiIndexFor(sun.sunrise);
    final yogaIdx = PanchangCalculator.yogaIndexFor(sun.sunrise);
    final karana = PanchangCalculator.karanaFor(sun.sunrise);
    final rahu = PanchangCalculator.rahuKalam(
      now,
      lat: AppLocation.lat,
      lon: AppLocation.lon,
    ...
    final timeline = <_PanchangTimelineItem>[
      ...
      _PanchangTimelineItem(
        rahu['start']!,
        '⚠️',
        'রাহুকাল শুরু',
        caution: true,
      ),
      _PanchangTimelineItem(rahu['end']!, '✅', 'রাহুকাল শেষ'),
      if (abhijit['applicable'] == true)
        _PanchangTimelineItem(
          abhijit['start'] as DateTime,
          '✨',
          'অভিজিৎ মুহূর্ত শুরু',
        ),
      if (abhijit['applicable'] == true)
        _PanchangTimelineItem(
          abhijit['end'] as DateTime,
          '✨',
          'অভিজিৎ মুহূর্ত শেষ',
        ),
      _PanchangTimelineItem(sun.sunset, '🌇', 'সূর্যাস্ত'),
    ]..sort((a, b) => a.when.compareTo(b.when));

    return CosmicBackground(
      child: Column(
        children: [
          const _ScreenHeader(title: '🧭 আজকের স্মার্ট পঞ্জিকা'),
          Expanded(
            child: ListView(
                ...
                _glass(
                  child: Column(
                    ...
                    children: [
                      ...
                      const SizedBox(height: 3),
                      Text(
                        '${PanchangCalculator.weekdayName(now)} • ${bnNum(now.day)} ${gregMonthBn(now.month)} ${bnNum(now.year)}',
                        style: const TextStyle(
                          color: Colors.white70,
                          fontSize: 12.5,
                        ),
                      ...
                      Wrap(
                        ...
                        runSpacing: 8,
                        children: [
                          _fact('🌙', 'তিথি', '${tithi.paksha} ${tithi.name}'),
                          _fact(
                            '⭐',
                            'নক্ষত্র',
                            PanchangCalculator.nakshatraNames[nakIdx],
                ...
                _glass(
                  child: Column(
                    ...
                    children: [
                      ...
                      if (abhijit['applicable'] == true)
                        Text(
                          '✅ অভিজিৎ মুহূর্ত: ${bnTime12(abhijit['start'] as DateTime)} – ${bnTime12(abhijit['end'] as DateTime)}',
                          style: const TextStyle(
                            color: Colors.white,
                            fontSize: 12.5,
                            fontWeight: FontWeight.w600,
                        ...
                        const Text(
                          'ℹ️ বুধবার হওয়ায় ঐতিহ্যগতভাবে অভিজিৎ মুহূর্ত প্রযোজ্য ধরা হয়নি।',
                          style: TextStyle(
                            color: Colors.white70,
                            fontSize: 12.5,
                          ),
                      ...
                      Text(
                        '⚠️ রাহুকাল: ${bnTime12(rahu['start']!)} – ${bnTime12(rahu['end']!)}',
                        style: const TextStyle(
                          color: Color(0xFFFFB4A7),
                          fontSize: 12.5,
                          fontWeight: FontWeight.w600,
                        ),
                      ),
                      const SizedBox(height: 7),
                      Text(
                        '🌙 তিথি: ${bnTime12(tithiStart)} থেকে ${bnTime12(tithiEnd)}',
                        style: const TextStyle(
                          color: Colors.white70,
                          fontSize: 12,
                        ),
                      ...
                      const Text(
                        'এগুলো পঞ্জিকা-ভিত্তিক ঐতিহ্যগত সময় নির্দেশিকা; বড় আর্থিক/বিবাহসংক্রান্ত সিদ্ধান্তের একমাত্র ভিত্তি হিসেবে ব্যবহার করবেন না।',
                        style: TextStyle(
                ...
                _glass(
                  child: Column(
                    ...
                    children: [
                      ...
                      if (events.isEmpty)
                        const Text(
                          'আজ ক্যালেন্ডারে আলাদা কোনো বিশেষ উৎসব/শুভদিন চিহ্নিত নেই।',
                    ...
                    Expanded(
                      child: OutlinedButton.icon(
                        onPressed: () => Navigator.push(
                          context,
                          MaterialPageRoute(
                            builder: (_) => const ShubhoMuhurtaScreen(),
                          ),
                        ),
                        icon: const Icon(Icons.auto_awesome, size: 17),
                        label: const Text('শুভ মুহূর্ত'),
                        style: OutlinedButton.styleFrom(
                          foregroundColor: const Color(0xFFFFD36E),
                          side: BorderSide(
                            color: const Color(
                              0xFFFFD36E,
                    ...
                    Expanded(
                      child: OutlinedButton.icon(
                        onPressed: () => Navigator.push(
                          context,
                          MaterialPageRoute(
                            builder: (_) => const PanchangShareCardScreen(),
                          ),
                        ),
                        icon: const Icon(Icons.share_rounded, size: 17),
                        label: const Text('শেয়ার কার্ড'),
                        style: OutlinedButton.styleFrom(
...
class _DateConverterScreenState extends State<DateConverterScreen> {
  ...
  DateTime _englishDate = DateTime.now();
  late int _bYear;
  late String _bMonth;
  late int _bDay;
  ...
  void initState() {
    super.initState();
    final info = BengaliDateUtil.monthInfoFor(_englishDate);
    _bYear = info.year;
    _bMonth = info.name;
    _bDay = _englishDate.difference(info.start).inDays + 1;
  }

  BengaliMonthInfo? _findBengaliMonth(int year, String month) {
    final start = DateTime(year + 593, 1, 1);
    final end = DateTime(year + 594, 12, 31);
    var d = start;
    while (!d.isAfter(end)) {
      final info = BengaliDateUtil.monthInfoFor(d);
      if (info.year == year && info.name == month) return info;
      d = info.end.add(const Duration(days: 1));
    }
    return null;
  }

  DateTime? get _convertedGregorian {
    final info = _findBengaliMonth(_bYear, _bMonth);
    if (info == null) return null;
    final total = info.end.difference(info.start).inDays + 1;
    final day = _bDay.clamp(1, total).toInt();
    return info.start.add(Duration(days: day - 1));
  }

  Future<void> _pickEnglish() async {
    final d = await showDatePicker(
      context: context,
      initialDate: _englishDate,
      firstDate: DateTime(1950),
      lastDate: DateTime(2100),
    );
    if (d == null) return;
    setState(() => _englishDate = d);
  }
  ...
  @override
  Widget build(BuildContext context) {
    final eInfo = BengaliDateUtil.monthInfoFor(_englishDate);
    final eBDay = _englishDate.difference(eInfo.start).inDays + 1;
    final greg = _convertedGregorian;
    final bInfo = _findBengaliMonth(_bYear, _bMonth);
    final bMax = bInfo == null
        ? 30
        : bInfo.end.difference(bInfo.start).inDays + 1;
    final safeBDay = _bDay.clamp(1, bMax).toInt();

    return CosmicBackground(
      child: Column(
        children: [
          const _ScreenHeader(title: '🔄 বাংলা ↔ ইংরেজি তারিখ'),
          Expanded(
            child: ListView(
              padding: const EdgeInsets.fromLTRB(16, 12, 16, 30),
              children: [
                  ...
                  Column(
                    ...
                    children: [
                      ...
                      SizedBox(
                        width: double.infinity,
                        child: OutlinedButton.icon(
                          onPressed: _pickEnglish,
                          icon: const Icon(Icons.calendar_month),
                          label: Text(
                            '${_englishDate.day}/${_englishDate.month}/${_englishDate.year}',
                          ),
                          style: OutlinedButton.styleFrom(
                      ...
                      const SizedBox(height: 12),
                      Text(
                        '${bnNum(eBDay)} ${eInfo.name} ${bnNum(eInfo.year)}',
                        style: const TextStyle(
                          color: Colors.white,
                          fontSize: 20,
                          fontWeight: FontWeight.w900,
                      ...
                      const SizedBox(height: 4),
                      Text(
                        '${PanchangCalculator.weekdayName(_englishDate)} • ${PanchangCalculator.tithiFor(_englishDate).paksha} পক্ষ • ${PanchangCalculator.tithiFor(_englishDate).name}',
                        style: const TextStyle(
                          color: Colors.white70,
                          fontSize: 11.5,
                        ),
                  ...
                  Column(
                    ...
                    children: [
                        ...
                        children: [
                          Expanded(
                            child: DropdownButtonFormField<int>(
                              ...
                              items: List.generate(9, (i) => _bYear - 4 + i)
                                  .map(
                                    (y) => DropdownMenuItem(
                                      value: y,
                                      child: Text(bnNum(y)),
                      ...
                      DropdownButtonFormField<int>(
                        initialValue: safeBDay,
                        dropdownColor: const Color(0xFF0B1B35),
                        style: const TextStyle(color: Colors.white),
                        decoration: const InputDecoration(
                          labelText: 'বাংলা তারিখ',
                          labelStyle: TextStyle(color: Colors.white70),
                          enabledBorder: OutlineInputBorder(
                            borderSide: BorderSide(color: Colors.white24),
                          ),
                        ...
                        items: List.generate(bMax, (i) => i + 1)
                            .map(
                              (d) => DropdownMenuItem(
                                value: d,
                                child: Text(bnNum(d)),
                            ...
                            .toList(),
                        onChanged: (v) {
                          if (v != null) setState(() => _bDay = v);
                        },
                      ),
                      const SizedBox(height: 12),
                      if (greg == null)
                        const Text(
                          'এই বাংলা তারিখের ইংরেজি তারিখ পাওয়া যায়নি।',
                          style: TextStyle(color: Colors.white70),
                        )
                      else ...[
                        Text(
                          '${greg.day} ${gregMonthBn(greg.month)} ${greg.year}',
                          style: const TextStyle(
                            color: Colors.white,
                            fontSize: 20,
                            fontWeight: FontWeight.w900,
                        ...
                        const SizedBox(height: 4),
                        Text(
                          '${PanchangCalculator.weekdayName(greg)} • ${PanchangCalculator.tithiFor(greg).paksha} পক্ষ • ${PanchangCalculator.tithiFor(greg).name}',
                          style: const TextStyle(
                            color: Colors.white70,
                            fontSize: 11.5,
                          ),
                ...
                const SizedBox(height: 10),
                const Text(
                  'ℹ️ রূপান্তরটি এই অ্যাপের একই সূর্য-সংক্রান্তি ভিত্তিক বাংলা ক্যালেন্ডার হিসাব ব্যবহার করে।',
...
class _MonthSummaryScreenState extends State<MonthSummaryScreen> {
  ...
  Widget _stat(String icon, String label, int count) {
    return Container(
  ...
  Widget build(BuildContext context) {
    ...
    for (var i = 0; i < total; i++) {
      final d = info.start.add(Duration(days: i));
      final events = BengaliCalendarData.eventsFor(d);
      if (events.isEmpty) continue;
      rows.add({'date': d, 'day': i + 1, 'events': events});
    ...
    final goodCount = counts.entries
        .where((e) => goodKeys.contains(e.key))
        .fold<int>(0, (a, b) => a + b.value);

    return CosmicBackground(
      child: Column(
        children: [
          const _ScreenHeader(title: '📊 মাসের Summary'),
          Expanded(
            child: ListView(
              padding: const EdgeInsets.fromLTRB(16, 12, 16, 30),
              children: [
                ...
                Container(
                  ...
                  decoration: BoxDecoration(
                    ...
                    borderRadius: BorderRadius.circular(18),
                    border: Border.all(
                      color: const Color(0xFFFFD36E).withValues(alpha: 0.24),
                    ),
                  ),
                  child: Column(
                    children: [
                      ...
                      GridView.count(
                        ...
                        children: [
                          _stat('🎉', 'উৎসব', counts['general'] ?? 0),
                          _stat('🌿', 'একাদশী', counts['ekadashi'] ?? 0),
                          _stat('🌕', 'পূর্ণিমা', counts['purnima'] ?? 0),
                          _stat('🌑', 'অমাবস্যা', counts['amabasya'] ?? 0),
                          _stat('⭐', 'শুভদিন', goodCount),
                          _stat('📌', 'মোট বিশেষ', rows.length),
                        ],
                      ),
                    ],
                  ),
                ...
                if (rows.isEmpty)
                  const Text(
                    'এই মাসে আলাদা কোনো event পাওয়া যায়নি।',
                    style: TextStyle(color: Colors.white70),
                  )
                else
                  ...rows.map((row) {
                    ...
                    final events = row['events'] as List<CalendarEvent>;
                    return Container(
                      margin: const EdgeInsets.only(bottom: 8),
                      padding: const EdgeInsets.all(12),
                      decoration: BoxDecoration(
                        color: const Color(0xFF07172F).withValues(alpha: 0.58),
                          ...
                          Expanded(
                            child: Column(
                              ...
                              children: [
                                ...
                                const SizedBox(height: 3),
                                Text(
                                  '${d.day}/${d.month}/${d.year} • ${PanchangCalculator.weekdayName(d)}',
                                  style: const TextStyle(
                                    color: Colors.white60,
                                    fontSize: 9.5,
                                  ),
...
class _JapaCounterScreenState extends State<JapaCounterScreen> {
  static const _countKey = 'japa_counter_count';
  static const _roundKey = 'japa_counter_rounds';
  ...
  Future<void> _load() async {
    ...
    if (!mounted) return;
    setState(() {
      _count = p.getInt(_countKey) ?? 0;
      _rounds = p.getInt(_roundKey) ?? 0;
    });
  }

  Future<void> _save() async {
    final p = await SharedPreferences.getInstance();
    await p.setInt(_countKey, _count);
    await p.setInt(_roundKey, _rounds);
  }
  ...
  Widget build(BuildContext context) {
    return CosmicBackground(
      child: Column(
        children: [
          const _ScreenHeader(title: '📿 জপ কাউন্টার'),
          Expanded(
            child: ListView(
              padding: const EdgeInsets.fromLTRB(16, 12, 16, 30),
              children: [
                ...
                Container(
                  padding: const EdgeInsets.all(22),
                  decoration: BoxDecoration(
                    color: const Color(0xFF07172F).withValues(alpha: 0.66),
                    borderRadius: BorderRadius.circular(26),
                    border: Border.all(
                      color: const Color(0xFFFFD36E).withValues(alpha: 0.30),
                    ),
                  ),
                  child: Column(
                    children: [
                        ...
                        child: Container(
                          ...
                          decoration: BoxDecoration(
                            ...
                            gradient: const RadialGradient(
                              colors: [Color(0xFFFFD36E), Color(0xFFB36A14)],
                            ),
                            boxShadow: [
                              BoxShadow(
                                color: const Color(
                      ...
                      Row(
                        ...
                        children: [
                          ...
                          TextButton(
                            ...
                            child: const Text(
                              'Reset',
                              style: TextStyle(color: Color(0xFFFFB4A7)),
                            ),
                          ),
                        ],
                      ),
...
class PujaChecklistScreen extends StatefulWidget {
  const PujaChecklistScreen({super.key});

  @override
  State<PujaChecklistScreen> createState() => _PujaChecklistScreenState();
}

class _PujaChecklistScreenState extends State<PujaChecklistScreen> {
  static const _itemsKey = 'puja_checklist_items_v1';
  static const _doneKey = 'puja_checklist_done_v1';
  ...
  List<String> _items = [..._defaultItems];
  Set<String> _done = {};
  ...
  Future<void> _load() async {
    ...
    if (!mounted) return;
    setState(() {
      _items = p.getStringList(_itemsKey) ?? [..._defaultItems];
      _done = (p.getStringList(_doneKey) ?? const <String>[]).toSet();
    });
  }

  Future<void> _save() async {
    final p = await SharedPreferences.getInstance();
    await p.setStringList(_itemsKey, _items);
    await p.setStringList(_doneKey, _done.toList());
  }

  void _add() {
    final v = _controller.text.trim();
    if (v.isEmpty || _items.contains(v)) return;
    setState(() {
      _items.add(v);
      _controller.clear();
    });
  ...
  @override
  Widget build(BuildContext context) {
    final completed = _items.where(_done.contains).length;
    return CosmicBackground(
      child: Column(
        children: [
          const _ScreenHeader(title: '✅ পূজা Checklist'),
          Expanded(
            child: ListView(
              padding: const EdgeInsets.fromLTRB(16, 12, 16, 30),
              children: [
                ...
                Container(
                  ...
                  child: Column(
                    ...
                    children: [
                      Text(
                        'প্রস্তুতি ${bnNum(completed)}/${bnNum(_items.length)}',
                        style: const TextStyle(
                          color: Color(0xFFFFD36E),
                          fontWeight: FontWeight.w800,
                        ),
                      ...
                      LinearProgressIndicator(
                        value: _items.isEmpty ? 0 : completed / _items.length,
                        ...
                        children: [
                          Expanded(
                            child: TextField(
                              ...
                              style: const TextStyle(color: Colors.white),
                              decoration: InputDecoration(
                                hintText: 'নিজের সামগ্রী যোগ করুন',
                ...
                ..._items.map(
                  (item) => Container(
                    ...
                    decoration: BoxDecoration(
                      color: const Color(0xFF07172F).withValues(alpha: 0.56),
                      borderRadius: BorderRadius.circular(14),
                      border: Border.all(
                        color: Colors.white.withValues(alpha: 0.10),
                      ),
                    ...
                    child: CheckboxListTile(
                      ...
                      activeColor: const Color(0xFFFFD36E),
                      checkColor: const Color(0xFF071428),
                      title: Text(
                        item,
                        style: TextStyle(
                          color: _done.contains(item)
                              ? Colors.white54
                              : Colors.white,
                          decoration: _done.contains(item)
                ...
                TextButton.icon(
                  onPressed: _resetDone,
                  icon: const Icon(Icons.refresh),
                  label: const Text('সব tick reset করুন'),
...
class FavouriteDayScreen extends StatefulWidget {
  const FavouriteDayScreen({super.key});

  @override
  State<FavouriteDayScreen> createState() => _FavouriteDayScreenState();
}

class _FavouriteDayScreenState extends State<FavouriteDayScreen> {
  static const _key = 'favourite_days_v1';
  ...
  Future<void> _save() async {
    final p = await SharedPreferences.getInstance();
    await p.setStringList(_key, _items.map((e) => jsonEncode(e)).toList());
  }
  ...
  Future<void> _add() async {
    ...
    final label = await showDialog<String>(
      ...
      builder: (context) => AlertDialog(
        title: const Text('দিনটির নাম দিন'),
        content: TextField(
          controller: c,
          autofocus: true,
          decoration: const InputDecoration(
            hintText: 'যেমন: আমাদের বিবাহবার্ষিকী',
          ),
        ),
        actions: [
          TextButton(
          ...
          FilledButton(
            onPressed: () => Navigator.pop(
              context,
              c.text.trim().isEmpty ? 'প্রিয় দিন' : c.text.trim(),
            ),
    ...
    setState(() {
      _items.add({'label': label, 'date': d.millisecondsSinceEpoch});
      _items.sort((a, b) => (a['date'] as int).compareTo(b['date'] as int));
    });
    _save();
  }
  ...
  Widget build(BuildContext context) {
    return CosmicBackground(
      child: Column(
        children: [
          const _ScreenHeader(title: '❤️ Favourite Day'),
          Expanded(
            child: ListView(
              padding: const EdgeInsets.fromLTRB(16, 12, 16, 30),
              children: [
                ...
                SizedBox(
                  ...
                  child: FilledButton.icon(
                    onPressed: _add,
                    icon: const Icon(Icons.add),
                    label: const Text('প্রিয় দিন যোগ করুন'),
                  ),
                ),
                const SizedBox(height: 12),
                if (_items.isEmpty)
                  const Text(
                    'এখনো কোনো প্রিয় দিন save করা হয়নি।',
                    style: TextStyle(color: Colors.white70),
                  )
                else
                  ...List.generate(_items.length, (i) {
                    ...
                    return Container(
                      ...
                      padding: const EdgeInsets.all(12),
                      decoration: BoxDecoration(
                        color: const Color(0xFF07172F).withValues(alpha: 0.60),
                        borderRadius: BorderRadius.circular(15),
                        border: Border.all(
                          color: Colors.white.withValues(alpha: 0.10),
                        ),
                          ...
                          Expanded(
                            child: Column(
                              ...
                              children: [
                                ...
                                const SizedBox(height: 3),
                                Text(
                                  '${d.day}/${d.month}/${d.year} • ${bnNum(bDay)} ${info.name} ${bnNum(info.year)}',
                                  style: const TextStyle(
                                    color: Colors.white60,
                                    fontSize: 10.5,
                                  ),
...
class FestivalCountdownScreen extends StatelessWidget {
  const FestivalCountdownScreen({super.key});

  @override
  Widget build(BuildContext context) {
    final festivals = BengaliCalendarData.upcomingFestivals(
      maxDays: 365,
      count: 24,
    );
    ...
    return CosmicBackground(
      child: Column(
        children: [
          const _ScreenHeader(title: '🏆 Festival Countdown'),
          Expanded(
            child: ListView(
              padding: const EdgeInsets.fromLTRB(16, 12, 16, 30),
              children: [
                const SizedBox(height: 155),
                ...festivals.map((f) {
                  final left = int.tryParse(f['daysLeft'] ?? '') ?? 0;
                  final label = left == 0
                      ? 'আজ'
                      : left == 1
                      ? 'কাল'
...
class TithiGuideScreen extends StatelessWidget {
  const TithiGuideScreen({super.key});

  static const _items = [
    ['প্রতিপদা', 'পক্ষের প্রথম তিথি'],
    ['দ্বিতীয়া', 'পক্ষের দ্বিতীয় তিথি'],
    ['তৃতীয়া', 'পক্ষের তৃতীয় তিথি'],
    ['চতুর্থী', 'পক্ষের চতুর্থ তিথি'],
    ['পঞ্চমী', 'পক্ষের পঞ্চম তিথি'],
    ['ষষ্ঠী', 'পক্ষের ষষ্ঠ তিথি'],
    ['সপ্তমী', 'পক্ষের সপ্তম তিথি'],
    ['অষ্টমী', 'পক্ষের অষ্টম তিথি'],
    ['নবমী', 'পক্ষের নবম তিথি'],
    ['দশমী', 'পক্ষের দশম তিথি'],
    ['একাদশী', 'পক্ষের একাদশ তিথি; বহু বৈষ্ণব ব্রতে গুরুত্বপূর্ণ'],
    ['দ্বাদশী', 'পক্ষের দ্বাদশ তিথি'],
    ['ত্রয়োদশী', 'পক্ষের ত্রয়োদশ তিথি'],
    ['চতুর্দশী', 'পক্ষের চতুর্দশ তিথি'],
    [
      'পূর্ণিমা / অমাবস্যা',
      'শুক্লপক্ষের শেষ পূর্ণিমা, কৃষ্ণপক্ষের শেষ অমাবস্যা',
    ],
  ];
  ...
  Widget build(BuildContext context) {
    return CosmicBackground(
      child: Column(
        children: [
          const _ScreenHeader(title: '📖 তিথির অর্থ'),
          Expanded(
            child: ListView(
              padding: const EdgeInsets.fromLTRB(16, 170, 16, 30),
              children: [
                const Text(
                  'তিথি সূর্য ও চন্দ্রের কৌণিক দূরত্বের ভিত্তিতে নির্ধারিত চন্দ্রদিন। একই ইংরেজি দিনে তিথি বদলাতে পারে।',
                  style: TextStyle(
                    color: Colors.white70,
                    height: 1.4,
                    fontSize: 12,
...
class UpobasInfoScreen extends StatelessWidget {
  const UpobasInfoScreen({super.key});

  @override
  Widget build(BuildContext context) {
    const items = [
      ['🌿 একাদশী', 'উপবাসের রীতি পরিবার/সম্প্রদায়ভেদে আলাদা হতে পারে।'],
      [
        '🌕 পূর্ণিমা',
        'অনেকে পূজা, দান বা সংযম পালন করেন; নিয়ম অঞ্চলভেদে বদলায়।',
      ],
      [
        '🌑 অমাবস্যা',
        'তর্পণ/পূজার প্রথা পরিবার ও অঞ্চলের রীতির ওপর নির্ভর করে।',
      ],
      [
        '🕉️ শিবরাত্রি',
        'উপবাস ও রাত্রিজাগরণের প্রচলন আছে; ব্যক্তিগত সামর্থ্য অনুযায়ী পালন করা উচিত।',
      ],
    ];
    ...
    return CosmicBackground(
      child: Column(
        children: [
          const _ScreenHeader(title: '🥗 উপবাস তথ্য'),
          Expanded(
            child: ListView(
              padding: const EdgeInsets.fromLTRB(16, 170, 16, 30),
              children: [
                ...
                const Text(
                  'স্বাস্থ্যগত সমস্যা, গর্ভাবস্থা, ওষুধ বা বিশেষ খাদ্যনিয়ম থাকলে উপবাসের আগে চিকিৎসকের পরামর্শ নিন।',
                  style: TextStyle(
                    color: Colors.white54,
                    fontSize: 10.5,
                    height: 1.4,
...
class TodayHistoryScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    for (var offset = 0; offset < 31; offset++) {
      ...
      for (final f in _historicalFigures) {
        if (f.month == d.month && f.day == d.day) {
          rows.add({'date': d, 'figure': f, 'offset': offset});
        }
      }
    }
    ...
    return CosmicBackground(
      child: Column(
        children: [
          const _ScreenHeader(title: '📚 আজকের ইতিহাস'),
          Expanded(
            child: ListView(
              padding: const EdgeInsets.fromLTRB(16, 170, 16, 30),
              children: [
                if (rows.isEmpty)
                  const Text(
                    'আগামী ৩০ দিনের তালিকায় কোনো সংরক্ষিত ঐতিহাসিক দিন নেই।',
                    style: TextStyle(color: Colors.white70),
                  )
                else
                  ...rows.map((row) {
                    final d = row['date'] as DateTime;
                    final f = row['figure'] as _HistoricalFigure;
                            ...
                            child: Column(
                              ...
                              children: [
                                ...
                                Text(
                                  offset == 0
                                      ? 'আজ'
                                      : '${d.day} ${gregMonthBn(d.month)} • ${bnNum(offset)} দিন পরে',
                                  style: const TextStyle(
                                    color: Color(0xFFFFD36E),
                                    fontSize: 10.5,
                                  ),
...
class HomeWidgetInfoScreen extends StatelessWidget {
  const HomeWidgetInfoScreen({super.key});
  ...
  Widget build(BuildContext context) {
    return CosmicBackground(
      child: Column(
        children: [
          const _ScreenHeader(title: '📱 Home Widget'),
          Expanded(
            child: ListView(
              padding: const EdgeInsets.fromLTRB(16, 170, 16, 30),
              children: [
                Container(
                  ...
                  child: const Column(
                    ...
                    children: [
                      Text(
                        'ফোনের Home Screen-এ বাংলা তারিখ ও তিথি',
                        style: TextStyle(
                          color: Color(0xFFFFD36E),
                          fontWeight: FontWeight.w800,
                          fontSize: 15,
                      ...
                      SizedBox(height: 8),
                      Text(
                        'ফোনের Home Screen-এ long-press করুন → Widgets → বাংলা পঞ্জিকা widget নির্বাচন করুন।',
                        style: TextStyle(
                          color: Colors.white70,
                          fontSize: 12,
                          height: 1.5,
                ...
                FilledButton.icon(
                  onPressed: () async {
                    ...
                    ScaffoldMessenger.of(context).showSnackBar(
                      const SnackBar(
                        content: Text('Widget data refresh করা হয়েছে'),
                      ),
                    );
                  },
                  icon: const Icon(Icons.refresh),
                  label: const Text('Widget data refresh করুন'),
                ),
              ],
            ),
          ),
...
class _SearchScreenState extends State<SearchScreen> {
  ...
  Future<void> _search() async {
    ...
    // আজ থেকে প্রায় ২ বছর পর্যন্ত খোঁজা হবে।
    for (int i = 0; i <= 760; i++) {
      ...
      for (final event in events) {
        final categoryText = _categoryBn(event.category);

        final searchable = '${event.label} $categoryText ${event.category}'
            .toLowerCase();
          ...
          child: Column(
            children: [
              Padding(
                ...
                child: Container(
                  decoration: BoxDecoration(
                    color: const Color(0xFF07172F).withValues(alpha: 0.72),
                    borderRadius: BorderRadius.circular(16),
                    border: Border.all(
                      color: Colors.white.withValues(alpha: 0.16),
                    ),
                  ...
                  child: TextField(
                    ...
                    decoration: InputDecoration(
                      hintText: 'যেমন: দুর্গাপূজা, একাদশী, পূর্ণিমা, বিবাহ...',
                      hintStyle: const TextStyle(
                        fontSize: 12,
                        color: Colors.white54,
                      ),
              ...
              if (!_busy)
                Expanded(
                  child: !_searched
                      ? const Center(
                          child: Text(
                            'পূজা, উৎসব বা শুভদিন লিখে খুঁজুন',
                            style: TextStyle(
                              color: Colors.white70,
                              fontSize: 13,
                            ),
                                  ...
                                  Expanded(
                                    child: Column(
                                      ...
                                      children: [
                                        ...
                                        const SizedBox(height: 4),
                                        Text(
                                          '${_bn(hit.bengaliDay)} ${hit.bengaliMonth} • ${hit.category}',
                                          style: const TextStyle(
                                            color: Colors.white70,
                                            fontSize: 11,
                                            fontWeight: FontWeight.w600,
                                        ...
                                        const SizedBox(height: 2),
                                        Text(
                                          '${hit.date.day}/${hit.date.month}/${hit.date.year}',
                                          style: const TextStyle(
                                            color: Colors.white60,
                                            fontSize: 9.5,
                                          ),
...
// =====================================================================
// SUPER APP PHASE-2 — Signature smart features
// Live Muhurta Radar • Best Day Finder • Panchang Compare • Festival Hub
// =====================================================================

class _TraditionalDayInsight {
  final DateTime date;
  final int score;
  final String label;
  ...
  final Map<String, dynamic> abhijit;
  final Map<String, DateTime> rahu;

  const _TraditionalDayInsight({
    required this.date,
    required this.score,
    required this.label,
    required this.tithi,
  ...
  static _TraditionalDayInsight forDate(DateTime date) {
    final sun = PanchangCalculator.sunTimes(
      date,
      lat: AppLocation.lat,
      lon: AppLocation.lon,
    ...
    final rashi = PanchangCalculator
        .rashiNames[PanchangCalculator.rashiIndexFor(sun.sunrise)];
    final events = BengaliCalendarData.eventsFor(date);
    final abhijit = PanchangCalculator.abhijitMuhurta(
      date,
      lat: AppLocation.lat,
    ...
    var score = 52;
    if (tithi.paksha == 'শুক্ল') score += 10;
    if (abhijit['applicable'] == true) score += 6;
    if (date.weekday == DateTime.tuesday) score -= 8;
    if (tithi.name == 'অমাবস্যা') score -= 6;
    if (tithi.name == 'পূর্ণিমা') score += 4;
    ...
    if (events.any((e) => auspicious.contains(e.category))) score += 18;
    if (events.any((e) => e.category == 'general')) score += 3;
    score = score.clamp(25, 95).toInt();
...
class _SignatureFeaturesStrip extends StatelessWidget {
  const _SignatureFeaturesStrip();

  Widget _item(BuildContext context, String icon, String title, Widget page) {
    return Expanded(
      child: InkWell(
        ...
        child: Container(
          ...
          decoration: BoxDecoration(
            color: const Color(0xFF07172F).withValues(alpha: 0.64),
            borderRadius: BorderRadius.circular(17),
            border: Border.all(
              color: const Color(0xFFFFD36E).withValues(alpha: 0.20),
            ),
          ),
          child: Column(
            mainAxisAlignment: MainAxisAlignment.center,
        ...
        Row(
          children: [
            ...
            _item(
              ...
              '🎯',
              'Best Day Finder',
              const BestDayFinderScreen(),
            ),
            const SizedBox(width: 9),
            _item(context, '⚖️', 'Day Compare', const PanchangCompareScreen()),
          ],
        ),
      ],
    );
...
class LiveMuhurtaRadarScreen extends StatefulWidget {
  const LiveMuhurtaRadarScreen({super.key});

  @override
  State<LiveMuhurtaRadarScreen> createState() => _LiveMuhurtaRadarScreenState();
}

class _LiveMuhurtaRadarScreenState extends State<LiveMuhurtaRadarScreen> {
  Timer? _timer;
  ...
  int _minute(DateTime d) => d.hour * 60 + d.minute;

  bool _inside(int now, DateTime start, DateTime end) {
    final a = _minute(start);
    final b = _minute(end);
    return now >= a && now < b;
  }

  String _durationText(int minutes) {
    if (minutes <= 0) return 'এখন';
    final h = minutes ~/ 60;
    final m = minutes % 60;
    if (h == 0) return '${bnNum(m)} মিনিট';
    if (m == 0) return '${bnNum(h)} ঘণ্টা';
    return '${bnNum(h)} ঘণ্টা ${bnNum(m)} মিনিট';
  }
  ...
  Widget build(BuildContext context) {
    ...
    final nowIst = nowReal.toUtc().add(const Duration(hours: 5, minutes: 30));
    final currentMinute = _minute(nowIst);
    final rahu = PanchangCalculator.rahuKalam(
      nowReal,
      lat: AppLocation.lat,
      lon: AppLocation.lon,
    ...
    final inRahu = _inside(currentMinute, rahu['start']!, rahu['end']!);
    final inYama = _inside(currentMinute, yama['start']!, yama['end']!);
    final inGulika = _inside(currentMinute, gulika['start']!, gulika['end']!);
    final abhijitOk = abhijit['applicable'] == true;
    final inAbhijit =
        abhijitOk &&
        _inside(
          currentMinute,
    ...
    if (inRahu) {
      status = 'রাহুকাল চলছে';
      statusIcon = '⚠️';
      statusColor = const Color(0xFFFF9F93);
      activeEnd = rahu['end'];
    } else if (inYama) {
      status = 'যমগণ্ড চলছে';
      statusIcon = '⚠️';
      ...
      statusIcon = '⚠️';
      statusColor = const Color(0xFFFF9F93);
      activeEnd = gulika['end'];
    } else if (inAbhijit) {
      status = 'অভিজিৎ মুহূর্ত চলছে';
      statusIcon = '✨';
      statusColor = const Color(0xFFFFD36E);
      activeEnd = abhijit['end'] as DateTime;
    } else {
      status = 'সাধারণ পঞ্জিকা সময়';
      statusIcon = '🕰️';
      statusColor = const Color(0xFF9DD6FF);
    }

    int remaining = 0;
    if (activeEnd != null) {
      remaining = (_minute(activeEnd) - currentMinute).clamp(0, 1440).toInt();
    }

    final tithi = PanchangCalculator.tithiFor(nowReal);
    final nak = PanchangCalculator
        .nakshatraNames[PanchangCalculator.nakshatraIndexFor(nowReal)];

    return CosmicBackground(
      child: Column(
        children: [
          const _ScreenHeader(title: '📡 Live Muhurta Radar'),
          Expanded(
            child: ListView(
              padding: const EdgeInsets.fromLTRB(16, 12, 16, 30),
              children: [
                          ...
                          Expanded(
                            child: Column(
                              ...
                              children: [
                                ...
                                Text(
                                  activeEnd == null
                                      ? 'IST ${bnNum(nowIst.hour)}:${bnNum(nowIst.minute).padLeft(2, '০')}'
                                      : '${_durationText(remaining)} পরে পরিবর্তন',
                                  style: const TextStyle(
                                    color: Colors.white70,
                                    fontSize: 11.5,
                                  ),
                ...
                const SizedBox(height: 10),
                const Text(
                  'ℹ️ এটি প্রচলিত পঞ্জিকা-ভিত্তিক সময় নির্দেশিকা। স্বাস্থ্য, আইন, অর্থ বা জীবনের বড় সিদ্ধান্তের নিশ্চয়তা/পূর্বাভাস নয়।',
                  style: TextStyle(
                    color: Colors.white54,
                    fontSize: 10.5,
                    height: 1.4,
...
class BestDayFinderScreen extends StatefulWidget {
  const BestDayFinderScreen({super.key});

  @override
  State<BestDayFinderScreen> createState() => _BestDayFinderScreenState();
}

class _BestDayFinderScreenState extends State<BestDayFinderScreen> {
  String _category = 'marriage';

  static const Map<String, (String, String)> _categories = {
    'marriage': ('💍', 'বিবাহ'),
    'griha': ('🏠', 'গৃহপ্রবেশ'),
  ...
  Widget build(BuildContext context) {
    ...
    return CosmicBackground(
      child: Column(
        children: [
          const _ScreenHeader(title: '🎯 Best Day Finder'),
          Expanded(
            child: ListView(
              padding: const EdgeInsets.fromLTRB(16, 12, 16, 30),
              children: [
                ...
                const SizedBox(height: 12),
                Text(
                  '${selected.$1} ${selected.$2} — পরবর্তী ${bnNum(results.length)}টি পঞ্জিকা-ভিত্তিক দিন',
                  style: const TextStyle(
                    color: Colors.white,
                    fontSize: 14,
                    fontWeight: FontWeight.w800,
                ...
                if (results.isEmpty)
                  _glass(
                    const Text(
                      'আগামী এক বছরে matching দিন পাওয়া যায়নি।',
                      style: TextStyle(color: Colors.white70),
                    ),
                  )
                else
                  ...results.map((row) {
                    ...
                    final info = BengaliDateUtil.monthInfoFor(d);
                    final bDay = d.difference(info.start).inDays + 1;
                    final tithi = row['tithi'] as TithiInfo;
                    final abhijitOk = row['abhijitApplicable'] == true;
                    return Container(
                      margin: const EdgeInsets.only(bottom: 9),
                      padding: const EdgeInsets.all(13),
                      decoration: BoxDecoration(
                      ...
                      child: Column(
                        ...
                        children: [
                              ...
                              Expanded(
                                child: Column(
                                  ...
                                  children: [
                                    ...
                                    Text(
                                      '${PanchangCalculator.weekdayName(d)} • ${bnNum(d.day)} ${gregMonthBn(d.month)}',
                                      style: const TextStyle(
                                        color: Colors.white60,
                                        fontSize: 10.5,
                                      ),
                          ...
                          const SizedBox(height: 9),
                          Text(
                            '🌙 ${tithi.paksha} ${tithi.name} • ⭐ ${row['nakshatra']}',
                            style: const TextStyle(
                              color: Colors.white70,
                              fontSize: 11.5,
                            ),
                          ...
                          Text(
                            abhijitOk
                                ? '✨ অভিজিৎ: ${bnTime12(row['abhijitStart'] as DateTime)} – ${bnTime12(row['abhijitEnd'] as DateTime)}'
                                : 'ℹ️ এই দিনে অভিজিৎ মুহূর্ত প্রযোজ্য ধরা হয়নি',
                            style: const TextStyle(
                              color: Color(0xFFFFD36E),
                              fontSize: 10.8,
                            ),
                          ...
                          const SizedBox(height: 4),
                          Text(
                            '⚠️ রাহুকাল: ${bnTime12(row['rahuStart'] as DateTime)} – ${bnTime12(row['rahuEnd'] as DateTime)}',
                            style: const TextStyle(
                              color: Color(0xFFFFB4A7),
                              fontSize: 10.8,
                            ),
                ...
                const SizedBox(height: 4),
                const Text(
                  'ℹ️ এই তালিকা app-এর existing তিথি/নক্ষত্র-ভিত্তিক সরলীকৃত ঐতিহ্যগত নিয়ম থেকে তৈরি; ব্যক্তিগত লগ্ন বা পূর্ণ জ্যোতিষ বিচার নয়।',
                  style: TextStyle(
                    color: Colors.white54,
                    fontSize: 10.3,
                    height: 1.4,
...
class PanchangCompareScreen extends StatefulWidget {
  const PanchangCompareScreen({super.key});

  @override
  State<PanchangCompareScreen> createState() => _PanchangCompareScreenState();
}

class _PanchangCompareScreenState extends State<PanchangCompareScreen> {
  DateTime _a = DateTime.now();
  DateTime _b = DateTime.now().add(const Duration(days: 7));

  Future<void> _pick(bool first) async {
    final initial = first ? _a : _b;
    final picked = await showDatePicker(
      context: context,
      initialDate: initial,
      ...
      child: Column(
        ...
        children: [
          ...
          if (data.events.isNotEmpty) ...[
            const SizedBox(height: 7),
            Text(
              '${data.events.first.icon} ${data.events.first.label}',
              maxLines: 2,
              overflow: TextOverflow.ellipsis,
              style: const TextStyle(
  ...
  @override
  Widget build(BuildContext context) {
    final a = _TraditionalDayInsight.forDate(_a);
    final b = _TraditionalDayInsight.forDate(_b);
    final diff = (a.score - b.score).abs();
    final winner = a.score == b.score
        ? 'দুই দিনের সূচক সমান'
        : a.score > b.score
        ? 'A দিনের সূচক ${bnNum(diff)} পয়েন্ট বেশি'
        : 'B দিনের সূচক ${bnNum(diff)} পয়েন্ট বেশি';

    return CosmicBackground(
      child: Column(
        children: [
          const _ScreenHeader(title: '⚖️ Panchang Day Compare'),
          Expanded(
            child: ListView(
              padding: const EdgeInsets.fromLTRB(16, 12, 16, 30),
              children: [
                ...
                Row(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    Expanded(child: _dateCard('DATE A', a, () => _pick(true))),
                    const SizedBox(width: 9),
                    Expanded(child: _dateCard('DATE B', b, () => _pick(false))),
                  ],
                ),
                const SizedBox(height: 12),
                const Text(
                  'ℹ️ স্কোরটি app-এর একটি সহজ “ঐতিহ্যভিত্তিক পঞ্জিকা সূচক” — শুক্ল/কৃষ্ণপক্ষ, বিদ্যমান শুভদিন event, অভিজিৎ প্রযোজ্যতা ও কিছু সাধারণ নিয়ম মিলিয়ে তুলনা করে। এটি জ্যোতিষীয় নিশ্চয়তা নয়।',
                  style: TextStyle(
                    color: Colors.white54,
                    fontSize: 10.4,
                    height: 1.4,
...
class FestivalHubScreen extends StatelessWidget {
  const FestivalHubScreen({super.key});

  @override
  Widget build(BuildContext context) {
    final festivals = BengaliCalendarData.upcomingFestivals(count: 8);
    final first = festivals.isEmpty ? null : festivals.first;

    Widget action(String icon, String label, Widget page) {
      return Expanded(
        child: InkWell(
          onTap: () =>
              Navigator.push(context, MaterialPageRoute(builder: (_) => page)),
    ...
    return CosmicBackground(
      child: Column(
        children: [
          const _ScreenHeader(title: '🪔 Festival Super Hub'),
          Expanded(
            child: ListView(
              padding: const EdgeInsets.fromLTRB(16, 12, 16, 30),
              children: [
                ...
                if (first != null)
                  Container(
                    ...
                    decoration: BoxDecoration(
                      gradient: const LinearGradient(
                        colors: [Color(0xFF4B173F), Color(0xFF742A31)],
                      ),
                        ...
                        Expanded(
                          child: Column(
                            ...
                            children: [
                              ...
                              const SizedBox(height: 4),
                              Text(
                                '${first['date']} • ${first['daysLeft'] == '0' ? 'আজ' : '${bnNum(int.tryParse(first['daysLeft'] ?? '') ?? 0)} দিন বাকি'}',
                                style: const TextStyle(
                                  color: Color(0xFFFFD36E),
                                  fontSize: 11.5,
                                  fontWeight: FontWeight.w700,
                ...
                Row(
                  children: [
                    action('✅', 'Puja Checklist', const PujaChecklistScreen()),
                    const SizedBox(width: 8),
                    action('🎨', 'Poster', const FestivalPosterScreen()),
                    const SizedBox(width: 8),
                    action('🔔', 'Reminder', const ReminderScreen()),
                  ],
                ),
                const SizedBox(height: 14),
                const Text(
                ...
                const SizedBox(height: 9),
                ...festivals.map((f) {
                  final days = int.tryParse(f['daysLeft'] ?? '') ?? 0;
                  return Container(
                    margin: const EdgeInsets.only(bottom: 8),
                    padding: const EdgeInsets.all(12),
                    decoration: BoxDecoration(
...
// =====================================================================
// SUPER APP PHASE-3 — Personal intelligence layer
// Bengali Voice Assistant • Morning Brief • Smart Agenda • Family Board
// • Bengali-date Daily Diary
// =====================================================================

class _Phase3SmartStrip extends StatelessWidget {
  const _Phase3SmartStrip();
      ...
      child: InkWell(
        ...
        child: Container(
          ...
          decoration: BoxDecoration(
            ...
            borderRadius: BorderRadius.circular(17),
            border: Border.all(
              color: const Color(0xFF7DC4FF).withValues(alpha: 0.24),
            ),
          ),
          child: Column(
            mainAxisAlignment: MainAxisAlignment.center,
        ...
        Row(
          children: [
            _item(
              ...
              '🎙️',
              'Panjika Voice',
              const PanjikaVoiceAssistantScreen(),
            ),
            const SizedBox(width: 9),
            _item(context, '🌅', 'Morning Brief', const MorningBriefScreen()),
            const SizedBox(width: 9),
            _item(context, '🗓️', 'Smart Agenda', const SmartAgendaScreen()),
          ],
        ),
      ],
    );
...
class PanjikaVoiceAssistantScreen extends StatefulWidget {
  const PanjikaVoiceAssistantScreen({super.key});

  @override
  State<PanjikaVoiceAssistantScreen> createState() =>
      _PanjikaVoiceAssistantScreenState();
}

class _PanjikaVoiceAssistantScreenState
    extends State<PanjikaVoiceAssistantScreen> {
  final stt.SpeechToText _speech = stt.SpeechToText();
  final TextEditingController _text = TextEditingController();
  bool _ready = false;
  bool _listening = false;
  bool _initializing = true;
  String _answer = 'মাইক চাপুন বা প্রশ্ন লিখুন।';
  ...
  DateTime? _nextEventDate(String category, {int maxDays = 150}) {
    final now = DateTime.now();
    final today = DateTime(now.year, now.month, now.day);
    for (var i = 0; i <= maxDays; i++) {
      final d = today.add(Duration(days: i));
      if (BengaliCalendarData.eventsFor(d).any((e) => e.category == category)) {
        return d;
      }
    }
    return null;
  ...
  String _dateLabel(DateTime date) {
    final info = BengaliDateUtil.monthInfoFor(date);
    final day = date.difference(info.start).inDays + 1;
    return '${bnNum(day)} ${info.name} • ${bnNum(date.day)} ${gregMonthBn(date.month)} ${bnNum(date.year)}';
  }

  void _answerQuestion(String raw) {
    ...
    if (q.contains('রাহু')) {
      final r = PanchangCalculator.rahuKalam(
        today,
        lat: AppLocation.lat,
        lon: AppLocation.lon,
      );
      answer =
          'আজ রাহুকাল ${bnTime12(r['start']!)} থেকে ${bnTime12(r['end']!)}।';
    } else if (q.contains('সূর্যোদ')) {
      final s = PanchangCalculator.sunTimes(
        today,
        lat: AppLocation.lat,
        lon: AppLocation.lon,
      );
      answer = 'আজ সূর্যোদয় ${bnTime12(s.sunrise)}।';
    } else if (q.contains('সূর্যাস্ত')) {
      final s = PanchangCalculator.sunTimes(
        today,
        lat: AppLocation.lat,
        lon: AppLocation.lon,
      );
      answer = 'আজ সূর্যাস্ত ${bnTime12(s.sunset)}।';
    } else if (q.contains('একাদশ')) {
      final d = _nextEventDate('ekadashi');
      answer = d == null
          ? 'আগামী একাদশীর তারিখ পাওয়া যায়নি।'
          : 'আগামী একাদশী ${_dateLabel(d)}।';
    } else if (q.contains('পূর্ণিম')) {
      final d = _nextEventDate('purnima');
      answer = d == null
          ? 'আগামী পূর্ণিমার তারিখ পাওয়া যায়নি।'
          : 'আগামী পূর্ণিমা ${_dateLabel(d)}।';
    } else if (q.contains('অমাবস্য')) {
      final d = _nextEventDate('amabasya');
      answer = d == null
          ? 'আগামী অমাবস্যার তারিখ পাওয়া যায়নি।'
          : 'আগামী অমাবস্যা ${_dateLabel(d)}।';
    } else if (q.contains('উৎসব') || q.contains('পূজা')) {
      final f = BengaliCalendarData.upcomingFestivals(count: 1);
      answer = f.isEmpty
          ? 'আগামী উৎসবের তথ্য পাওয়া যায়নি।'
          : 'আগামী বিশেষ দিন ${f.first['title'] ?? ''}, ${f.first['date'] ?? ''}।';
    } else if (q.contains('কাল')) {
      final d = today.add(const Duration(days: 1));
      final t = PanchangCalculator.tithiFor(d);
      final info = BengaliDateUtil.monthInfoFor(d);
      final bday = d.difference(info.start).inDays + 1;
      answer =
          'কাল ${bnNum(bday)} ${info.name}, ${t.paksha} পক্ষের ${t.name} তিথি।';
    } else if (q.contains('আজ') ||
        q.contains('পঞ্জিকা') ||
        q.contains('পঞ্চাঙ্গ')) {
      final t = PanchangCalculator.tithiFor(now);
      final nak = PanchangCalculator
          .nakshatraNames[PanchangCalculator.nakshatraIndexFor(now)];
      final rashi =
          PanchangCalculator.rashiNames[PanchangCalculator.rashiIndexFor(now)];
      answer =
          'আজ ${_dateLabel(today)}। ${t.paksha} পক্ষের ${t.name} তিথি। নক্ষত্র $nak, চন্দ্র রাশি $rashi।';
    } else {
      answer =
          'আমি এখন আজকের পঞ্জিকা, কালকের তিথি, রাহুকাল, সূর্যোদয়, সূর্যাস্ত, একাদশী, পূর্ণিমা, অমাবস্যা ও আগামী উৎসবের প্রশ্ন বুঝতে পারি।';
    }
    ...
    TtsService.instance.speak(answer);
  }
  ...
  Widget build(BuildContext context) {
    return CosmicBackground(
      child: Column(
        children: [
          const _ScreenHeader(title: '🎙️ বাংলা Panjika Voice'),
          Expanded(
            child: ListView(
              padding: const EdgeInsets.all(16),
              children: [
                Container(
                  padding: const EdgeInsets.all(18),
                  decoration: BoxDecoration(
                    color: const Color(0xFF07172F).withValues(alpha: 0.68),
                    borderRadius: BorderRadius.circular(22),
                    border: Border.all(
                      color: const Color(0xFFFFD36E).withValues(alpha: 0.28),
                    ),
                  ...
                  child: Column(
                    children: [
                      ...
                      const SizedBox(height: 8),
                      const Text(
                        '“আজ কী তিথি?”, “আগামী একাদশী কবে?”, “আজ রাহুকাল কখন?”',
                        textAlign: TextAlign.center,
                        style: TextStyle(color: Colors.white70, fontSize: 12.5),
                      ),
                      const SizedBox(height: 18),
                      GestureDetector(
                        onTap: _initializing || !_ready ? null : _listen,
                        child: AnimatedContainer(
                          duration: const Duration(milliseconds: 200),
                          width: 82,
                          height: 82,
                      ...
                      TextField(
                        ...
                        decoration: InputDecoration(
                          ...
                          suffixIcon: IconButton(
                            ...
                            onPressed: () => _answerQuestion(_text.text),
                          ),
                          border: OutlineInputBorder(
                            borderRadius: BorderRadius.circular(14),
                            borderSide: BorderSide.none,
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      Align(
                        ...
                        child: IconButton(
                          tooltip: 'আবার শুনুন',
                          onPressed: () => TtsService.instance.speak(_answer),
                          icon: const Icon(
                            Icons.volume_up_rounded,
                            color: Color(0xFFFFD36E),
                          ),
...
class _MorningBriefScreenState extends State<MorningBriefScreen> {
  List<FamilyMemberItem> _family = const [];
  ...
  void initState() {
    ...
    FamilyMemberStore.instance.load().then((_) {
      if (!mounted) return;
      setState(() {
        _family = FamilyMemberStore.instance.items
            .where((e) => e.daysLeft <= 7)
            .take(3)
            .toList();
      });
    });
  ...
  String _briefText() {
    ...
    final eventText = events.isEmpty
        ? 'আজ কোনো বিশেষ উৎসব নেই।'
        : 'আজ ${events.map((e) => e.label).join(', ')}।';
    return 'সুপ্রভাত। আজ ${bnNum(bday)} ${info.name} ${bnNum(info.year)}। '
        '${t.paksha} পক্ষের ${t.name} তিথি। সূর্যোদয় ${bnTime12(sun.sunrise)}, '
        'সূর্যাস্ত ${bnTime12(sun.sunset)}। রাহুকাল ${bnTime12(rahu['start']!)} থেকে '
        '${bnTime12(rahu['end']!)}। $eventText';
  }
  ...
  Widget build(BuildContext context) {
    ...
    final events = BengaliCalendarData.eventsFor(now);
    final festivals = BengaliCalendarData.upcomingFestivals(count: 1);

    return CosmicBackground(
      child: Column(
        children: [
          const _ScreenHeader(title: '🌅 Personalized Morning Brief'),
          Expanded(
            child: ListView(
              padding: const EdgeInsets.all(16),
              children: [
                Container(
                  ...
                  decoration: BoxDecoration(
                    gradient: const LinearGradient(
                      colors: [Color(0xFF5E2B08), Color(0xFF24153F)],
                    ),
                    borderRadius: BorderRadius.circular(22),
                    border: Border.all(
                      color: const Color(0xFFFFD36E).withValues(alpha: 0.32),
                    ),
                  ),
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      Text(
                        AppSettings.instance.profileName.trim().isEmpty
                            ? 'সুপ্রভাত 🌞'
                            : 'সুপ্রভাত, ${AppSettings.instance.profileName} 🌞',
                        style: const TextStyle(
                          color: Colors.white,
                          fontSize: 19,
                          fontWeight: FontWeight.w800,
                      ...
                      Text(
                        '${PanchangCalculator.weekdayName(now)} • ${t.paksha} পক্ষ • ${t.name}',
                        style: const TextStyle(
                          color: Colors.white70,
                          fontSize: 13,
                        ),
                      ...
                      Row(
                        children: [
                          ElevatedButton.icon(
                            onPressed: () =>
                                TtsService.instance.speak(_briefText()),
                ...
                _infoRow(
                  events.isEmpty ? '✨' : events.first.icon,
                  'আজকের বিশেষ দিন',
                  events.isEmpty
                      ? 'কোনো বিশেষ উৎসব নেই'
                ...
                _infoRow(
                  ...
                  festivals.isEmpty
                      ? 'তথ্য নেই'
                      : '${festivals.first['title'] ?? ''} • ${festivals.first['date'] ?? ''}',
                ),
                if (_family.isNotEmpty) ...[
                  const SizedBox(height: 6),
                  const Text(
                    '👨‍👩‍👧 পরিবারের আগামী ৭ দিন',
                    style: TextStyle(
                      color: Color(0xFFFFD36E),
                      fontWeight: FontWeight.w800,
                    ),
                  ...
                  for (final f in _family)
                    _infoRow(
                      '🎂',
                      f.name,
                      '${f.occasion} • ${bnNum(f.daysLeft)} দিন বাকি',
                    ),
                ],
              ],
            ),
...
class FamilyUpcomingBoardScreen extends StatefulWidget {
  const FamilyUpcomingBoardScreen({super.key});

  @override
  State<FamilyUpcomingBoardScreen> createState() =>
      _FamilyUpcomingBoardScreenState();
}

class _FamilyUpcomingBoardScreenState extends State<FamilyUpcomingBoardScreen> {
  bool _loading = true;
  ...
  @override
  Widget build(BuildContext context) {
    final items = FamilyMemberStore.instance.items;
    return CosmicBackground(
      child: Column(
        children: [
          const _ScreenHeader(title: '👨‍👩‍👧 Family Upcoming Board'),
          Expanded(
            child: _loading
                ? const Center(
                    child: CircularProgressIndicator(color: Color(0xFFFFD36E)),
                ...
                ? Center(
                    child: Padding(
                      ...
                      child: Column(
                        ...
                        children: [
                          const Text(
                            'এখনো পরিবারের কোনো দিন সেভ করা নেই।',
                            style: TextStyle(color: Colors.white70),
                          ),
                          const SizedBox(height: 12),
                          ElevatedButton(
                            ...
                            child: const Text('Family Calendar খুলুন'),
                          ),
                        ],
                      ),
                    ),
                            ...
                            Expanded(
                              child: Column(
                                ...
                                children: [
                                  ...
                                  const SizedBox(height: 3),
                                  Text(
                                    '${f.occasion} • ${bnNum(d.day)} ${gregMonthBn(d.month)}',
                                    style: const TextStyle(
                                      color: Colors.white70,
                                      fontSize: 12,
                                    ),
...
class _AgendaEntry {
  ...
  final String title;
  final String type;
  const _AgendaEntry(this.when, this.icon, this.title, this.type);
}

class SmartAgendaScreen extends StatefulWidget {
  const SmartAgendaScreen({super.key});

  @override
  State<SmartAgendaScreen> createState() => _SmartAgendaScreenState();
}

class _SmartAgendaScreenState extends State<SmartAgendaScreen> {
  bool _loading = true;
  List<_AgendaEntry> _items = const [];
  ...
  Future<void> _load() async {
    await FamilyMemberStore.instance.load();
    await ReminderStore.instance.load();
    final now = DateTime.now();
    final today = DateTime(now.year, now.month, now.day);
    final end = today.add(const Duration(days: 30));
    final all = <_AgendaEntry>[];

    for (var i = 0; i <= 30; i++) {
      final d = today.add(Duration(days: i));
      for (final e in BengaliCalendarData.eventsFor(d)) {
        all.add(_AgendaEntry(d, e.icon, e.label, 'পঞ্জিকা'));
      }
    }

    for (final f in FamilyMemberStore.instance.items) {
      final d = f.nextOccurrence;
      if (!d.isAfter(end)) {
        all.add(
          _AgendaEntry(
    ...
    for (final r in ReminderStore.instance.items) {
      if (r.when.isAfter(now) && !r.when.isAfter(end)) {
        all.add(_AgendaEntry(r.when, '⏰', r.text, 'রিমাইন্ডার'));
      }
    }
  ...
  Widget build(BuildContext context) {
    return CosmicBackground(
      child: Column(
        children: [
          const _ScreenHeader(title: '🗓️ Smart 30-Day Agenda'),
          Expanded(
            child: _loading
                ? const Center(
                    child: CircularProgressIndicator(color: Color(0xFFFFD36E)),
                ...
                ? const Center(
                    child: Text(
                      'আগামী ৩০ দিনে কোনো saved item নেই',
                      style: TextStyle(color: Colors.white70),
                    ),
                  )
                : ListView.builder(
                            ...
                            Expanded(
                              child: Column(
                                ...
                                children: [
                                  ...
                                  const SizedBox(height: 3),
                                  Text(
                                    '${a.type} • ${bnNum(a.when.day)} ${gregMonthBn(a.when.month)}',
                                    style: const TextStyle(
                                      color: Colors.white60,
                                      fontSize: 11.5,
                                    ),
...
class PanjikaDiaryScreen extends StatefulWidget {
  const PanjikaDiaryScreen({super.key});

  @override
  State<PanjikaDiaryScreen> createState() => _PanjikaDiaryScreenState();
}

class _PanjikaDiaryScreenState extends State<PanjikaDiaryScreen> {
  DateTime _date = DateTime.now();
  final TextEditingController _controller = TextEditingController();
  bool _loading = true;

  String get _key =>
      'panjika_diary_${_date.year}-${_date.month.toString().padLeft(2, '0')}-${_date.day.toString().padLeft(2, '0')}';
  ...
  Future<void> _load() async {
    ...
    if (!mounted) return;
    setState(() {
      _controller.text = p.getString(_key) ?? '';
      _loading = false;
    });
  }
  ...
  Future<void> _save() async {
    ...
    ScaffoldMessenger.of(
      context,
    ).showSnackBar(const SnackBar(content: Text('দিনলিপি ফোনে সেভ হয়েছে')));
  }
  ...
  Widget build(BuildContext context) {
    ...
    final t = PanchangCalculator.tithiFor(_date);
    return CosmicBackground(
      child: Column(
        children: [
          const _ScreenHeader(title: '📓 আমার পঞ্জিকা দিনলিপি'),
          Expanded(
            child: ListView(
              padding: const EdgeInsets.all(16),
              children: [
                        ...
                        Expanded(
                          child: Column(
                            ...
                            children: [
                              ...
                              const SizedBox(height: 3),
                              Text(
                                '${t.paksha} পক্ষ • ${t.name} • ${bnNum(_date.day)} ${gregMonthBn(_date.month)}',
                                style: const TextStyle(
                                  color: Colors.white70,
                                  fontSize: 12,
                                ),
                ...
                if (_loading)
                  ...
                  TextField(
                    ...
                    decoration: InputDecoration(
                      hintText:
                          'আজকের স্মৃতি, পূজার প্রস্তুতি, কাজ বা ব্যক্তিগত নোট লিখুন…',
                      hintStyle: const TextStyle(color: Colors.white38),
                      filled: true,
                ...
                ElevatedButton.icon(
                  ...
                  label: const Text('দিনলিপি সেভ করুন'),
                  style: ElevatedButton.styleFrom(
                    backgroundColor: const Color(0xFFFFD36E),
                    foregroundColor: const Color(0xFF071428),
                    padding: const EdgeInsets.symmetric(vertical: 14),
...
// =====================================================================
// SUPER APP PHASE-4 — Automation & habit intelligence
// Smart Digest • Shubho Alarm • Sadhana Streak • Family Occasion Assistant
// =====================================================================

class _Phase4PowerStrip extends StatelessWidget {
  const _Phase4PowerStrip();
      ...
      child: InkWell(
        ...
        child: Container(
          ...
          decoration: BoxDecoration(
            ...
            borderRadius: BorderRadius.circular(17),
            border: Border.all(
              color: const Color(0xFF7EE2B8).withValues(alpha: 0.24),
            ),
          ),
          child: Column(
            mainAxisAlignment: MainAxisAlignment.center,
        ...
        Row(
          children: [
            _item(
              ...
              '🔔',
              'Smart Digest',
              const SmartNotificationDigestScreen(),
            ),
            const SizedBox(width: 9),
            _item(context, '⏰', 'Shubho Alarm', const ShubhoAlarmScreen()),
            const SizedBox(width: 9),
            _item(context, '🔥', 'Sadhana Streak', const SadhanaStreakScreen()),
          ],
        ),
      ],
    );
...
class SmartNotificationDigestScreen extends StatefulWidget {
  const SmartNotificationDigestScreen({super.key});

  @override
  State<SmartNotificationDigestScreen> createState() =>
      _SmartNotificationDigestScreenState();
}

class _SmartNotificationDigestScreenState
    extends State<SmartNotificationDigestScreen> {
  static const _morningKey = 'phase4_digest_morning';
  static const _eveningKey = 'phase4_digest_evening';
  static const _morningBase = 960000;
  static const _eveningBase = 961000;
  ...
  Future<void> _load() async {
    ...
    if (!mounted) return;
    setState(() {
      _morning = p.getBool(_morningKey) ?? false;
      _evening = p.getBool(_eveningKey) ?? false;
      _busy = false;
    });
  }

  String _summaryFor(DateTime day, {required bool morning}) {
    final info = BengaliDateUtil.monthInfoFor(day);
    final bday = day.difference(info.start).inDays + 1;
    final tithi = PanchangCalculator.tithiFor(day);
    ...
    final eventText = events.isEmpty
        ? 'বিশেষ উৎসব নেই'
        : events.take(2).map((e) => '${e.icon} ${e.label}').join(' • ');
    if (morning) {
      return '${bnNum(bday)} ${info.name} • ${tithi.name} • $eventText • রাহুকাল ${bnTime12(rahu['start']!)}–${bnTime12(rahu['end']!)}';
    }
    final tomorrow = day.add(const Duration(days: 1));
    final tomorrowInfo = BengaliDateUtil.monthInfoFor(tomorrow);
    final tomorrowDay = tomorrow.difference(tomorrowInfo.start).inDays + 1;
    final tomorrowTithi = PanchangCalculator.tithiFor(tomorrow);
    final tomorrowEvents = BengaliCalendarData.eventsFor(tomorrow);
    final tomorrowEventText = tomorrowEvents.isEmpty
        ? 'বিশেষ উৎসব নেই'
        : tomorrowEvents.take(2).map((e) => '${e.icon} ${e.label}').join(' • ');
    return 'আগামীকাল ${bnNum(tomorrowDay)} ${tomorrowInfo.name} • ${tomorrowTithi.name} • $tomorrowEventText';
  }

  Future<void> _cancelScheduled() async {
    for (int i = 0; i < 14; i++) {
      await NotificationService.instance.cancel(_morningBase + i);
      await NotificationService.instance.cancel(_eveningBase + i);
    }
  }
  ...
  Future<void> _saveAndSchedule() async {
    setState(() => _busy = true);
    final p = await SharedPreferences.getInstance();
    await p.setBool(_morningKey, _morning);
    await p.setBool(_eveningKey, _evening);
    ...
    for (int i = 0; i < 14; i++) {
      final day = today.add(Duration(days: i));
      if (_morning) {
        final when = DateTime(day.year, day.month, day.day, 7, 0);
        if (when.isAfter(now)) {
          await NotificationService.instance.scheduleGeneric(
            _morningBase + i,
            '🌅 আজকের পঞ্জিকা ব্রিফ',
            _summaryFor(day, morning: true),
            when,
          );
        }
      }
      if (_evening) {
        final when = DateTime(day.year, day.month, day.day, 19, 0);
        if (when.isAfter(now)) {
          await NotificationService.instance.scheduleGeneric(
            _eveningBase + i,
            '🌙 আগামী দিনের পঞ্জিকা',
            _summaryFor(day, morning: false),
            when,
          );
        }
      }
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text(
          (!_morning && !_evening)
              ? 'Smart Digest বন্ধ করা হয়েছে'
              : 'পরবর্তী ১৪ দিনের Smart Digest schedule হয়েছে',
        ),
      ),
    );
  }
  ...
  @override
  Widget build(BuildContext context) {
    final preview = _summaryFor(DateTime.now(), morning: true);
    return CosmicBackground(
      child: Column(
        children: [
          const _ScreenHeader(title: '🔔 Smart Notification Digest'),
          Expanded(
            child: ListView(
              padding: const EdgeInsets.all(16),
              children: [
                _glass(
                  const Text(
                    'প্রতিদিন প্রয়োজনীয় পঞ্জিকা তথ্য একসাথে নোটিফিকেশনে পাবেন। আলাদা আলাদা ১০টা alert নয় — একটি পরিষ্কার digest।',
                    style: TextStyle(
                      color: Colors.white70,
                      fontSize: 13,
                      height: 1.5,
                  ...
                  Column(
                    children: [
                      ...
                      SwitchListTile.adaptive(
                        ...
                        subtitle: const Text(
                          'আগামী দিনের তিথি ও গুরুত্বপূর্ণ তথ্য',
                          style: TextStyle(
                            color: Colors.white60,
                            fontSize: 11.5,
                          ),
                ...
                const SizedBox(height: 14),
                ElevatedButton.icon(
                  onPressed: _busy ? null : _saveAndSchedule,
                  icon: _busy
                      ? const SizedBox(
                          width: 18,
                          height: 18,
                  ...
                  label: const Text('Digest Save & Schedule'),
                  style: ElevatedButton.styleFrom(
                    backgroundColor: const Color(0xFFFFD36E),
                    foregroundColor: const Color(0xFF071428),
                    padding: const EdgeInsets.symmetric(vertical: 14),
...
class ShubhoAlarmScreen extends StatefulWidget {
  const ShubhoAlarmScreen({super.key});

  @override
  State<ShubhoAlarmScreen> createState() => _ShubhoAlarmScreenState();
}

class _ShubhoAlarmScreenState extends State<ShubhoAlarmScreen> {
  DateTime _date = DateTime.now();
  String _type = 'অভিজিৎ মুহূর্ত';
  bool _busy = false;
  ...
  DateTime? _timeForType(String type) {
    if (type == 'সূর্যোদয়') {
      return PanchangCalculator.sunTimes(
        _date,
        lat: AppLocation.lat,
        lon: AppLocation.lon,
      ).sunrise;
    ...
    if (type == 'রাহুকাল শেষ') {
      return PanchangCalculator.rahuKalam(
        _date,
        lat: AppLocation.lat,
        lon: AppLocation.lon,
      )['end'];
    }
    final a = PanchangCalculator.abhijitMuhurta(
      _date,
      lat: AppLocation.lat,
      lon: AppLocation.lon,
    );
    if (a['applicable'] != true) return null;
    return a['start'] as DateTime?;
  }

  Future<void> _schedule() async {
    final target = _timeForType(_type);
    if (target == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('এই দিনে অভিজিৎ মুহূর্ত প্রযোজ্য নয়')),
      );
      return;
    }
    if (!target.isAfter(DateTime.now())) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('এই সময় ইতিমধ্যে পার হয়ে গেছে — অন্য দিন বেছে নিন'),
        ),
      );
      return;
    }
    setState(() => _busy = true);
    final id = target.millisecondsSinceEpoch.remainder(0x7FFFFFFF);
    await NotificationService.instance.scheduleGeneric(
      id,
      '⏰ শুভ সময় শুরু',
      '$_type এখন শুরু হচ্ছে • ${bnTime12(target)}',
      target,
    );
    if (!mounted) return;
    setState(() => _busy = false);
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text('$_type-এর alarm ${bnTime12(target)}-এ সেট হয়েছে'),
      ),
    );
  }
  ...
  Widget build(BuildContext context) {
    ...
    return CosmicBackground(
      child: Column(
        children: [
          const _ScreenHeader(title: '⏰ Shubho Alarm'),
          Expanded(
            child: ListView(
              padding: const EdgeInsets.all(16),
              children: [
                        ...
                        Expanded(
                          child: Column(
                            ...
                            children: [
                              ...
                              Text(
                                '${bnNum(_date.day)} ${gregMonthBn(_date.month)} ${bnNum(_date.year)}',
                                style: const TextStyle(
                                  color: Colors.white60,
                                  fontSize: 11.5,
                                ),
                  ...
                  Column(
                    ...
                    children: [
                      ...
                      Wrap(
                        spacing: 8,
                        runSpacing: 8,
                        children: ['অভিজিৎ মুহূর্ত', 'সূর্যোদয়', 'রাহুকাল শেষ']
                            .map(
                              (e) => ChoiceChip(
                                selected: _type == e,
                                onSelected: (_) => setState(() => _type = e),
                                label: Text(e),
                              ),
                            )
                            .toList(),
                ...
                ElevatedButton.icon(
                  ...
                  label: const Text('এই শুভ সময়ে Alarm সেট করুন'),
                  style: ElevatedButton.styleFrom(
                    backgroundColor: const Color(0xFFFFD36E),
                    foregroundColor: const Color(0xFF071428),
                    padding: const EdgeInsets.symmetric(vertical: 14),
                ...
                const SizedBox(height: 10),
                const Text(
                  'নোট: সময়গুলো পঞ্জিকা/সূর্য-গণনার ঐতিহ্যভিত্তিক নির্দেশিকা; গুরুত্বপূর্ণ সিদ্ধান্তে প্রয়োজন অনুযায়ী বিশেষজ্ঞের পরামর্শ নিন।',
                  style: TextStyle(
...
class SadhanaStreakScreen extends StatefulWidget {
  const SadhanaStreakScreen({super.key});

  @override
  State<SadhanaStreakScreen> createState() => _SadhanaStreakScreenState();
}

class _SadhanaStreakScreenState extends State<SadhanaStreakScreen> {
  static const _key = 'phase4_sadhana_log';
  Set<String> _done = <String>{};
  bool _loading = true;

  static const _actions = <String, String>{
    'panjika': '📖 আজকের পঞ্জিকা দেখেছি',
    'japa': '📿 জপ/ধ্যান করেছি',
    'puja': '🪔 পূজা/প্রার্থনা করেছি',
  };

  String _dayKey(DateTime d) =>
      '${d.year.toString().padLeft(4, '0')}-${d.month.toString().padLeft(2, '0')}-${d.day.toString().padLeft(2, '0')}';

  String _entry(DateTime d, String action) => '${_dayKey(d)}|$action';
  ...
  Future<void> _load() async {
    ...
    if (!mounted) return;
    setState(() {
      _done = (p.getStringList(_key) ?? const []).toSet();
      _loading = false;
    });
  }
  ...
  bool _hasAny(DateTime d) =>
      _actions.keys.any((a) => _done.contains(_entry(d, a)));
  ...
  int get _todayCount => _actions.keys
      .where((a) => _done.contains(_entry(DateTime.now(), a)))
      .length;
  ...
  Widget _glass(Widget child) => Container(
    ...
    decoration: BoxDecoration(
      ...
      borderRadius: BorderRadius.circular(18),
      border: Border.all(
        color: const Color(0xFF7EE2B8).withValues(alpha: 0.20),
      ),
    ),
    child: child,
  );
  ...
  Widget build(BuildContext context) {
    ...
    return CosmicBackground(
      child: Column(
        children: [
          const _ScreenHeader(title: '🔥 Sadhana Streak'),
          Expanded(
            child: _loading
                ? const Center(
                    child: CircularProgressIndicator(color: Color(0xFFFFD36E)),
                            ...
                            Expanded(
                              child: Column(
                                ...
                                children: [
                                  ...
                                  Text(
                                    'আজ $_todayCount/${_actions.length} কাজ সম্পূর্ণ',
                                    style: const TextStyle(
                                      color: Colors.white60,
                                      fontSize: 11.5,
                                    ),
                      ...
                      _glass(
                        Column(
                          children: _actions.entries
                              .map(
                                (e) => CheckboxListTile(
                                  value: _done.contains(_entry(today, e.key)),
                                  onChanged: (v) => _toggle(e.key, v ?? false),
                                  title: Text(
                                    e.value,
                                    style: const TextStyle(
                                      color: Colors.white,
...
class FamilyOccasionAssistantScreen extends StatefulWidget {
  const FamilyOccasionAssistantScreen({super.key});

  @override
  State<FamilyOccasionAssistantScreen> createState() =>
      _FamilyOccasionAssistantScreenState();
}

class _FamilyOccasionAssistantScreenState
    extends State<FamilyOccasionAssistantScreen> {
  List<FamilyMemberItem> _items = const [];
  FamilyMemberItem? _selected;
  Set<String> _done = <String>{};
  bool _loading = true;
  ...
  String _prefKey(FamilyMemberItem item) =>
      'phase4_family_assistant_${item.id}_${item.nextOccurrence.year}';

  List<String> _checklistFor(FamilyMemberItem item) {
    if (item.occasion == 'বিবাহবার্ষিকী') {
      return const [
        'উপহার ঠিক করুন',
        'ফুল/মিষ্টি প্রস্তুত',
        'শুভেচ্ছা বার্তা লিখুন',
        'ছবি/পোস্টার তৈরি',
        'ফোন/ভিডিও কলের সময় রাখুন',
      ];
    }
    if (item.occasion == 'জন্মদিন') {
      return const [
        'উপহার ঠিক করুন',
        'কেক/মিষ্টি প্রস্তুত',
        'শুভেচ্ছা বার্তা লিখুন',
    ...
    return const [
      'প্রয়োজনীয় প্রস্তুতি লিখুন',
      'উপহার/সামগ্রী ঠিক করুন',
      'শুভেচ্ছা পাঠান',
      'পরিবারকে জানান',
      'রিমাইন্ডার সেট করুন',
    ];
  ...
  Future<void> _loadDone() async {
    ...
    if (!mounted) return;
    setState(() {
      _done = (p.getStringList(_prefKey(item)) ?? const []).toSet();
      _loading = false;
    });
  }

  Future<void> _select(FamilyMemberItem item) async {
    setState(() {
      _selected = item;
      _loading = true;
    });
  ...
  Future<void> _toggle(String item, bool value) async {
    ...
    final p = await SharedPreferences.getInstance();
    await p.setStringList(_prefKey(selected), _done.toList());
  }

  Future<void> _remind() async {
    final item = _selected;
    if (item == null) return;
    final whenDay = item.nextOccurrence.subtract(const Duration(days: 1));
    final when = DateTime(whenDay.year, whenDay.month, whenDay.day, 9, 0);
    if (!when.isAfter(DateTime.now())) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('এই অনুষ্ঠানের আগের দিনের সময় পার হয়ে গেছে'),
        ),
      );
      return;
    }
    await NotificationService.instance.scheduleGeneric(
      970000 + item.id.remainder(20000),
      '🎁 পরিবারের অনুষ্ঠান আগামীকাল',
      '${item.name} • ${item.occasion} — প্রস্তুতি চেক করুন',
      when,
    );
    if (!mounted) return;
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(content: Text('আগের দিন সকাল ৯টার reminder সেট হয়েছে')),
    );
  }
  ...
  Widget _glass(Widget child) => Container(
    ...
    decoration: BoxDecoration(
      ...
      borderRadius: BorderRadius.circular(18),
      border: Border.all(
        color: const Color(0xFF7DC4FF).withValues(alpha: 0.20),
      ),
    ),
    child: child,
  );
  ...
  Widget build(BuildContext context) {
    ...
    return CosmicBackground(
      child: Column(
        children: [
          const _ScreenHeader(title: '🎁 Family Occasion Assistant'),
          Expanded(
            child: _loading
                ? const Center(
                    child: CircularProgressIndicator(color: Color(0xFFFFD36E)),
                    ...
                    child: Padding(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          const SizedBox(height: 12),
                          const Text(
                            'Family Calendar-এ আগে একটি জন্মদিন/বার্ষিকী যোগ করুন।',
                            textAlign: TextAlign.center,
                            style: TextStyle(
                              color: Colors.white70,
                              fontSize: 14,
                    ...
                    children: [
                      ...
                      if (item != null) ...[
                              ...
                              Expanded(
                                child: Column(
                                  ...
                                  children: [
                                    ...
                                    const SizedBox(height: 3),
                                    Text(
                                      '${item.occasion} • ${bnNum(item.daysLeft)} দিন বাকি',
                                      style: const TextStyle(
                                        color: Color(0xFFFFD36E),
                                        fontSize: 12.5,
                                        fontWeight: FontWeight.w700,
                                    ...
                                    Text(
                                      '${bnNum(item.nextOccurrence.day)} ${gregMonthBn(item.nextOccurrence.month)} ${bnNum(item.nextOccurrence.year)}',
                                      style: const TextStyle(
                                        color: Colors.white60,
                                        fontSize: 11,
                                      ),
                        ...
                        _glass(
                          Column(
                            ...
                            children: [
                              ...
                              const SizedBox(height: 6),
                              ..._checklistFor(item).map(
                                (c) => CheckboxListTile(
                                  value: _done.contains(c),
                        ...
                        ElevatedButton.icon(
                          ...
                          label: const Text('আগের দিন Reminder সেট করুন'),
                          style: ElevatedButton.styleFrom(
                            backgroundColor: const Color(0xFFFFD36E),
                            foregroundColor: const Color(0xFF071428),
                            padding: const EdgeInsets.symmetric(vertical: 14),
...
// =====================================================================
// SUPER APP PHASE-5 — Clean Home + Smart Life Layer
// Duplicate home entry boxes are consolidated into one Command Center.
// New: Festival Live Mode • Smart Reminder Center • Daily Routine • Year Map
// =====================================================================

class _Phase5HomeCommandCenter extends StatelessWidget {
  const _Phase5HomeCommandCenter();
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      decoration: BoxDecoration(
        gradient: LinearGradient(
          colors: [
            const Color(0xFF08172F).withValues(alpha: 0.78),
            const Color(0xFF301A4D).withValues(alpha: 0.62),
          ],
        ),
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 4),
          const Text(
            'বারবার একই box নয় — দরকারি কাজ এক জায়গা থেকে খুলুন',
            style: TextStyle(color: Colors.white54, fontSize: 10.5),
          ),
          ...
          GridView.count(
            ...
            children: [
              _action(
                ...
                icon: '🌞',
                title: 'আজকের পঞ্জিকা',
                subtitle: 'তিথি • সময় • timeline',
                page: const TodaySmartScreen(),
              ),
              _action(
                context,
                icon: '🎯',
                title: 'সেরা দিন খুঁজুন',
                subtitle: 'আগামী ভালো দিন',
                page: const BestDayFinderScreen(),
              ),
              _action(
                context,
                icon: '🎙️',
                ...
                page: const PanjikaVoiceAssistantScreen(),
              ),
              _action(
                context,
                icon: '🔔',
                title: 'Reminder Center',
                subtitle: 'সব alert এক জায়গায়',
                page: const SmartReminderCenterScreen(),
              ),
            ],
          ),
        ],
...
class _Phase5FestivalPulseCard extends StatelessWidget {
  const _Phase5FestivalPulseCard();

  @override
  Widget build(BuildContext context) {
    final items = BengaliCalendarData.upcomingFestivals(count: 3);
    if (items.isEmpty) {
      return const SizedBox.shrink();
    }

    final first = items.first;
    final firstDays = int.tryParse(first['daysLeft'] ?? '') ?? 0;

    return InkWell(
      borderRadius: BorderRadius.circular(20),
      onTap: () => Navigator.push(
        context,
        MaterialPageRoute(builder: (_) => const FestivalLiveModeScreen()),
      ),
      child: Container(
        padding: const EdgeInsets.all(14),
        decoration: BoxDecoration(
          gradient: const LinearGradient(
            colors: [Color(0xFF4B1E16), Color(0xFF25183E), Color(0xFF07172F)],
          ),
          borderRadius: BorderRadius.circular(20),
          border: Border.all(
            color: const Color(0xFFFFC45B).withValues(alpha: 0.35),
          ),
        ),
        child: Column(
          children: [
            Row(
              children: [
                ...
                Container(
                  ...
                  child: Text(
                    firstDays <= 0 ? 'আজ' : '${bnNum(firstDays)} দিন',
                    style: const TextStyle(
                      color: Color(0xFFFFD36E),
                      fontSize: 10.5,
                      fontWeight: FontWeight.w800,
...
class FestivalLiveModeScreen extends StatelessWidget {
  const FestivalLiveModeScreen({super.key});

  Widget _quick(BuildContext context, String icon, String label, Widget page) {
    return Expanded(
      child: InkWell(
        borderRadius: BorderRadius.circular(15),
        onTap: () =>
  ...
  @override
  Widget build(BuildContext context) {
    final festivals = BengaliCalendarData.upcomingFestivals(count: 12);

    return CosmicBackground(
      child: Column(
        children: [
          const _ScreenHeader(title: '🪔 Festival Live Mode'),
          Expanded(
            child: ListView(
                ...
                if (festivals.isNotEmpty)
                  Container(
                    ...
                    decoration: BoxDecoration(
                      ...
                      border: Border.all(
                        color: const Color(0xFFFFC45B).withValues(alpha: 0.38),
                      ),
                    ),
                    child: Column(
                      children: [
                        Text(
                          festivals.first['icon'] ?? '🎉',
                          style: const TextStyle(fontSize: 46),
                        ),
                        const SizedBox(height: 7),
                        Text(
                          festivals.first['title'] ?? '',
                          textAlign: TextAlign.center,
                          style: const TextStyle(
                            color: Colors.white,
                            fontSize: 21,
                        ...
                        const SizedBox(height: 8),
                        Text(
                          (int.tryParse(festivals.first['daysLeft'] ?? '') ??
                                      0) <=
                                  0
                              ? 'আজকের বিশেষ দিন'
                              : 'আর ${bnNum(int.tryParse(festivals.first['daysLeft'] ?? '') ?? 0)} দিন বাকি',
                          style: const TextStyle(
                            color: Colors.white70,
                            fontSize: 12,
                          ),
                  ...
                  Container(
                    ...
                    child: Row(
                      children: [
                        ...
                        Text(
                          (int.tryParse(f['daysLeft'] ?? '') ?? 0) <= 0
                              ? 'আজ'
                              : '${bnNum(int.tryParse(f['daysLeft'] ?? '') ?? 0)} দিন',
                          style: const TextStyle(
                            color: Color(0xFFFFD36E),
                            fontSize: 10.5,
                            fontWeight: FontWeight.w700,
...
class _SmartReminderCenterScreenState extends State<SmartReminderCenterScreen> {
  ...
  Widget _launch(BuildContext context, String icon, String title, Widget page) {
    return Expanded(
      child: InkWell(
        borderRadius: BorderRadius.circular(15),
        onTap: () =>
  ...
  Widget build(BuildContext context) {
    ...
    final upcomingReminders =
        ReminderStore.instance.items.where((e) => e.when.isAfter(now)).toList()
          ..sort((a, b) => a.when.compareTo(b.when));
    final family = FamilyMemberStore.instance.items.toList()
      ..sort((a, b) => a.daysLeft.compareTo(b.daysLeft));

    return CosmicBackground(
      child: Column(
        children: [
          const _ScreenHeader(title: '🔔 Smart Reminder Center'),
          Expanded(
            child: _loading
                ...
                : ListView(
                    ...
                    children: [
                      ...
                      if (upcomingReminders.isEmpty)
                        _empty('কোনো future reminder নেই')
                      else
                        for (final r in upcomingReminders.take(4))
                          _line(
                            '⏰',
                            r.text,
                            '${bnNum(r.when.day)} ${gregMonthBn(r.when.month)} • ${bnTime12(r.when)}',
                          ),
                      const SizedBox(height: 18),
                      const Text(
                        'Family Upcoming',
                      ...
                      const SizedBox(height: 8),
                      if (family.isEmpty)
                        _empty('Family Calendar-এ এখনো কিছু যোগ করা হয়নি')
                      else
                        for (final f in family.take(4))
                          _line(
                            '🎁',
                            '${f.name} • ${f.occasion}',
                            f.daysLeft == 0
                                ? 'আজ'
                                : '${bnNum(f.daysLeft)} দিন পরে',
                          ),
                    ],
                  ),
          ),
  ...
  Widget _centerStat(String icon, String value, String label) {
    return Column(
      children: [
        Text(icon, style: const TextStyle(fontSize: 20)),
        const SizedBox(height: 3),
  ...
  Widget _line(String icon, String title, String sub) {
    return Container(
      margin: const EdgeInsets.only(bottom: 8),
      padding: const EdgeInsets.all(11),
      decoration: BoxDecoration(
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                Text(
                  sub,
                  style: const TextStyle(color: Colors.white54, fontSize: 9.8),
                ),
              ],
            ),
          ),
...
class _DailyRoutineScreenState extends State<DailyRoutineScreen> {
  ...
  String get _key {
    ...
    return 'phase5_routine_${d.year}_${d.month}_${d.day}';
  }
  ...
  Future<void> _toggle(int index) async {
    ...
    final p = await SharedPreferences.getInstance();
    await p.setStringList(_key, _done.map((e) => '$e').toList());
  }

  void _openTask(BuildContext context, int index) {
    final Widget page = switch (index) {
      0 => const TodaySmartScreen(),
      1 => const JapaCounterScreen(),
      2 => const SettingsScreen(),
      _ => const PanjikaDiaryScreen(),
    };
    Navigator.push(context, MaterialPageRoute(builder: (_) => page));
  }
  ...
  @override
  Widget build(BuildContext context) {
    final progress = _done.length / _tasks.length;

    return CosmicBackground(
      child: Column(
        children: [
          const _ScreenHeader(title: '🌿 My Daily Routine'),
          Expanded(
            child: _loading
                ? const Center(child: CircularProgressIndicator())
                : ListView(
                    ...
                    children: [
                      Container(
                        ...
                        decoration: BoxDecoration(
                          gradient: const LinearGradient(
                            colors: [Color(0xFF173A2E), Color(0xFF13213B)],
                          ),
                          borderRadius: BorderRadius.circular(22),
                          border: Border.all(
                            color: const Color(
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            const SizedBox(height: 8),
                            Text(
                              '${bnNum(_done.length)} / ${bnNum(_tasks.length)} সম্পন্ন',
                              style: const TextStyle(
...
class MyYearMapScreen extends StatefulWidget {
  const MyYearMapScreen({super.key});

  @override
  State<MyYearMapScreen> createState() => _MyYearMapScreenState();
}

class _MyYearMapScreenState extends State<MyYearMapScreen> {
  bool _loading = true;
  late List<Map<String, int>> _months;
  ...
  Future<void> _buildMap() async {
    ...
    for (int month = 1; month <= 12; month++) {
      ...
      for (int day = 1; day <= days; day++) {
        calendarEvents += BengaliCalendarData.eventsFor(
          DateTime(year, month, day),
        ).length;
      }

      final familyCount = FamilyMemberStore.instance.items
          .where((e) => e.date.month == month)
          .length;

      final reminderCount = ReminderStore.instance.items
  ...
  Widget build(BuildContext context) {
    return CosmicBackground(
      child: Column(
        children: [
          const _ScreenHeader(title: '🗺️ My Year Map'),
          Expanded(
            child: _loading
                ? const Center(child: CircularProgressIndicator())
                : ListView(
                    ...
                    children: [
                      const Text(
                        'এক বছরে পঞ্জিকা ইভেন্ট, Family occasion ও আপনার Reminder — এক নজরে।',
                        style: TextStyle(
                          color: Colors.white70,
                          fontSize: 11.5,
                          height: 1.45,
                      ...
                      const SizedBox(height: 14),
                      for (final m in _months) _monthCard(m),
                    ],
                  ),
          ),
        ],
  ...
  Widget _monthCard(Map<String, int> m) {
    final total =
        (m['events'] ?? 0) + (m['family'] ?? 0) + (m['reminders'] ?? 0);
    final strength = (total / 20).clamp(0.08, 1.0);
        ...
        children: [
              ...
              Expanded(
                child: ClipRRect(
                  ...
                  child: LinearProgressIndicator(
                    ...
                    valueColor: const AlwaysStoppedAnimation(Color(0xFFFFD36E)),
                  ),
                ),
              ),
            ],
          ...
          Row(
            children: [
              Expanded(
                child: Text(
                  '📅 ${bnNum(m['events']!)} পঞ্জিকা',
                  style: const TextStyle(color: Colors.white60, fontSize: 10),
                ),
              ),
              Expanded(
                child: Text(
                  '👨‍👩‍👧 ${bnNum(m['family']!)} পরিবার',
                  style: const TextStyle(color: Colors.white60, fontSize: 10),
                ),
              ),
              Expanded(
                child: Text(
                  '🔔 ${bnNum(m['reminders']!)} Reminder',
                  textAlign: TextAlign.right,
                  style: const TextStyle(color: Colors.white60, fontSize: 10),
                ),
              ),
...
// =====================================================================
// SUPER APP PHASE-6 — Next-Gen Daily Intelligence
// New: Panchang Time Machine • Next 24H Timeline • Bengali Life Planner
//      • Travel Panchang Planner • Daily Intelligence Home Card
// All calculations reuse the app's existing Panchang engine and local stores.
// =====================================================================

class PanchangTimeMachineScreen extends StatefulWidget {
  const PanchangTimeMachineScreen({super.key});

  @override
  State<PanchangTimeMachineScreen> createState() =>
      _PanchangTimeMachineScreenState();
}

class _PanchangTimeMachineScreenState extends State<PanchangTimeMachineScreen> {
  DateTime _date = DateTime.now();
  ...
  Widget _info(String icon, String label, String value) {
    return Padding(
  ...
  Widget build(BuildContext context) {
    ...
    final rashi = PanchangCalculator
        .rashiNames[PanchangCalculator.rashiIndexFor(sun.sunrise)];
    final rahu = PanchangCalculator.rahuKalam(_date);
    final abhijit = PanchangCalculator.abhijitMuhurta(_date);
    final info = BengaliDateUtil.monthInfoFor(_date);
    final bnDay = _date.difference(info.start).inDays + 1;
    final events = BengaliCalendarData.eventsFor(_date);

    return CosmicBackground(
      child: Column(
        children: [
          const _ScreenHeader(title: '🕰️ পঞ্জিকা Time Machine'),
          Expanded(
            child: ListView(
              padding: const EdgeInsets.fromLTRB(15, 8, 15, 28),
              children: [
                InkWell(
                  ...
                  child: Container(
                    ...
                    decoration: BoxDecoration(
                      gradient: const LinearGradient(
                        colors: [Color(0xFF4A1B69), Color(0xFF172744)],
                      ),
                      borderRadius: BorderRadius.circular(20),
                      border: Border.all(
                        color: const Color(0xFFFFD36E).withValues(alpha: 0.35),
                      ...
                      children: [
                        ...
                        Expanded(
                          child: Column(
                            ...
                            children: [
                              Text(
                                '${bnNum(bnDay)} ${info.name} ${bnNum(info.year)}',
                                style: const TextStyle(
                                  color: Color(0xFFFFD36E),
                                  fontSize: 20,
                                  fontWeight: FontWeight.w900,
                              ...
                              const SizedBox(height: 3),
                              Text(
                                '${bnNum(_date.day)} ${gregMonthBn(_date.month)} ${bnNum(_date.year)} • ${PanchangCalculator.weekdayName(_date)}',
                                style: const TextStyle(
                                  color: Colors.white70,
                                  fontSize: 11.5,
                                ),
                ...
                _glass(
                  Column(
                    ...
                    children: [
                      ...
                      _info('⭐', 'নক্ষত্র', nak),
                      _info('🪐', 'চন্দ্র রাশি', rashi),
                      _info('🌅', 'সূর্যোদয়', bnTime12(sun.sunrise)),
                      _info('🌇', 'সূর্যাস্ত', bnTime12(sun.sunset)),
                      _info(
                        '⏳',
                        'রাহুকাল',
                        '${bnTime12(rahu['start']!)} – ${bnTime12(rahu['end']!)}',
...
class Next24HoursPanchangScreen extends StatelessWidget {
  const Next24HoursPanchangScreen({super.key});
  ...
  List<Map<String, dynamic>> _timeline() {
    ...
    void add(DateTime time, String icon, String title) {
      if (!time.isBefore(now) && !time.isAfter(until)) {
        rows.add({'time': time, 'icon': icon, 'title': title});
      }
    }
    ...
    for (int offset = 0; offset <= 1; offset++) {
      ...
      final rahu = PanchangCalculator.rahuKalam(day);
      final abhijit = PanchangCalculator.abhijitMuhurta(day);
      add(sun.sunrise, '🌅', 'সূর্যোদয়');
      add(rahu['start']!, '⏳', 'রাহুকাল শুরু');
      add(rahu['end']!, '✅', 'রাহুকাল শেষ');
      if (abhijit['applicable'] == true) {
        add(abhijit['start'] as DateTime, '☀️', 'অভিজিৎ মুহূর্ত শুরু');
        add(abhijit['end'] as DateTime, '✨', 'অভিজিৎ মুহূর্ত শেষ');
      }
      add(sun.sunset, '🌇', 'সূর্যাস্ত');
    }
  ...
  Widget build(BuildContext context) {
    ...
    final todayEvents = BengaliCalendarData.eventsFor(now);
    final tomorrowEvents = BengaliCalendarData.eventsFor(
      now.add(const Duration(days: 1)),
    );
    ...
    return CosmicBackground(
      child: Column(
        children: [
          const _ScreenHeader(title: '⏱️ আগামী ২৪ ঘণ্টা'),
          Expanded(
            child: ListView(
              padding: const EdgeInsets.fromLTRB(15, 8, 15, 28),
              children: [
                Container(
                  ...
                  decoration: BoxDecoration(
                    gradient: const LinearGradient(
                      colors: [Color(0xFF173A4A), Color(0xFF171D3B)],
                    ),
                    borderRadius: BorderRadius.circular(20),
                    border: Border.all(
                      color: const Color(0xFF77D7F5).withValues(alpha: 0.3),
                    ),
                  ),
                  child: const Text(
                    'সূর্যোদয়, রাহুকাল, অভিজিৎ মুহূর্ত ও সূর্যাস্ত—আগামী ২৪ ঘণ্টার সময়ক্রমে।',
                    style: TextStyle(
                      color: Colors.white,
                      fontSize: 12,
                      height: 1.45,
                  ...
                  Container(
                    ...
                    child: Row(
                      children: [
                        ...
                        Text(
                          bnTime12(rows[i]['time'] as DateTime),
                          style: const TextStyle(
                            color: Color(0xFFFFD36E),
                            fontSize: 11.5,
                            fontWeight: FontWeight.w800,
...
class _Phase6EventSummary extends StatelessWidget {
  final List<CalendarEvent> todayEvents;
  final List<CalendarEvent> tomorrowEvents;

  const _Phase6EventSummary({
    required this.todayEvents,
    required this.tomorrowEvents,
  });

  Widget _line(String title, List<CalendarEvent> events) {
    return Padding(
      padding: const EdgeInsets.only(bottom: 8),
      child: Row(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          ...
          Expanded(
            child: Text(
              events.isEmpty
                  ? 'বিশেষ event নেই'
                  : events.map((e) => '${e.icon} ${e.label}').join(' • '),
              style: const TextStyle(color: Colors.white70, fontSize: 11),
            ),
          ),
        ],
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 9),
          _line('আজ', todayEvents),
          _line('কাল', tomorrowEvents),
        ],
      ),
    );
  }
...
class _LifePlanItem {
  ...
  _LifePlanItem copyWith({bool? done}) => _LifePlanItem(
    id: id,
    title: title,
    category: category,
    when: when,
  ...
  static _LifePlanItem fromJson(Map<String, dynamic> m) => _LifePlanItem(
    id: m['id'].toString(),
    title: m['title'].toString(),
    category: m['category']?.toString() ?? 'অন্যান্য',
    when: DateTime.fromMillisecondsSinceEpoch(m['when'] as int),
    done: m['done'] == true,
  );
}

class _LifePlannerStore {
  _LifePlannerStore._();
  static final _LifePlannerStore instance = _LifePlannerStore._();
  static const _key = 'phase6_bengali_life_planner';

  final List<_LifePlanItem> items = [];
  ...
  Future<void> load() async {
    ...
    items
      ...
      ..addAll(
        raw.map(
          (e) => _LifePlanItem.fromJson(jsonDecode(e) as Map<String, dynamic>),
        ),
      );
    _sort();
  }
  ...
  Future<void> add(_LifePlanItem item) async {
    items.add(item);
    _sort();
    await _save();
  }
...
class BengaliLifePlannerScreen extends StatefulWidget {
  const BengaliLifePlannerScreen({super.key});

  @override
  State<BengaliLifePlannerScreen> createState() =>
      _BengaliLifePlannerScreenState();
}

class _BengaliLifePlannerScreenState extends State<BengaliLifePlannerScreen> {
  bool _loading = true;
  ...
  Future<void> _load() async {
    await _LifePlannerStore.instance.load();
    if (!mounted) return;
    setState(() => _loading = false);
  }
  ...
  Future<void> _addTask() async {
    ...
    final ok = await showModalBottomSheet<bool>(
      ...
      builder: (sheetContext) => StatefulBuilder(
        builder: (context, setLocal) {
          final info = BengaliDateUtil.monthInfoFor(selected);
          final bnDay = selected.difference(info.start).inDays + 1;
          return Padding(
            ...
            child: Container(
              ...
              child: Column(
                ...
                children: [
                  ...
                  DropdownButtonFormField<String>(
                    value: category,
                    dropdownColor: const Color(0xFF0C1D38),
                    style: const TextStyle(color: Colors.white),
                    decoration: InputDecoration(
                      filled: true,
                      fillColor: Colors.white.withValues(alpha: 0.06),
                    ...
                    children: [
                      Expanded(
                        child: OutlinedButton.icon(
                          ...
                          label: Text('${bnNum(bnDay)} ${info.name}'),
                        ),
                      ),
                      const SizedBox(width: 8),
                      Expanded(
    ...
    if (ok == true && controller.text.trim().isNotEmpty) {
      await _LifePlannerStore.instance.add(
        _LifePlanItem(
  ...
  @override
  Widget build(BuildContext context) {
    final items = _LifePlannerStore.instance.items;
    return CosmicBackground(
      child: Column(
        children: [
          const _ScreenHeader(title: '🗓️ Bengali Life Planner'),
          Padding(
            ...
            child: _loading
                    ...
                    child: Padding(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          const SizedBox(height: 10),
                          const Text(
                            'বাংলা তারিখের সঙ্গে নিজের কাজ, পূজা ও পারিবারিক পরিকল্পনা রাখুন।',
                            textAlign: TextAlign.center,
                            style: TextStyle(
                              color: Colors.white70,
                              height: 1.5,
                          ...
                          ElevatedButton.icon(
                            onPressed: _addTask,
                            icon: const Icon(Icons.add),
                            label: const Text('প্রথম Plan যোগ করুন'),
                          ),
                        ],
                      ),
                    ),
                ...
                : ListView.builder(
                    ...
                    itemBuilder: (context, index) {
                      ...
                      final info = BengaliDateUtil.monthInfoFor(item.when);
                      final bnDay = item.when.difference(info.start).inDays + 1;
                      return Dismissible(
                        key: ValueKey(item.id),
                        direction: DismissDirection.endToStart,
                        onDismissed: (_) async {
                          await _LifePlannerStore.instance.remove(item.id);
                          if (mounted) setState(() {});
                        },
                        background: Container(
                          margin: const EdgeInsets.only(bottom: 9),
                        ...
                        child: Container(
                          ...
                          child: Row(
                            children: [
                              InkWell(
                                onTap: () async {
                                  await _LifePlannerStore.instance.toggle(
                                    item.id,
                                  );
                                  if (mounted) setState(() {});
                                },
                              ...
                              Expanded(
                                child: Column(
                                  ...
                                  children: [
                                    ...
                                    const SizedBox(height: 3),
                                    Text(
                                      '${bnNum(bnDay)} ${info.name} • ${bnTime12(item.when)} • ${item.category}',
                                      style: const TextStyle(
                                        color: Colors.white54,
                                        fontSize: 10,
                                      ),
...
class TravelPanchangPlannerScreen extends StatefulWidget {
  const TravelPanchangPlannerScreen({super.key});

  @override
  State<TravelPanchangPlannerScreen> createState() =>
      _TravelPanchangPlannerScreenState();
}

class _TravelPanchangPlannerScreenState
    extends State<TravelPanchangPlannerScreen> {
  DateTime _date = DateTime.now();
  ...
  List<Map<String, DateTime>> _windows(
    DateTime sunrise,
    DateTime sunset,
    DateTime rahuStart,
    DateTime rahuEnd,
  ) {
    final rows = <Map<String, DateTime>>[];
    if (rahuStart.difference(sunrise).inMinutes >= 45) {
      rows.add({'start': sunrise, 'end': rahuStart});
    }
    if (sunset.difference(rahuEnd).inMinutes >= 45) {
      rows.add({'start': rahuEnd, 'end': sunset});
    }
    return rows;
  }
  ...
  Widget build(BuildContext context) {
    ...
    return CosmicBackground(
      child: Column(
        children: [
          const _ScreenHeader(title: '🧭 Travel Panchang Planner'),
          Expanded(
            child: ListView(
              padding: const EdgeInsets.fromLTRB(15, 8, 15, 28),
              children: [
                InkWell(
                  ...
                  child: Container(
                    ...
                    decoration: BoxDecoration(
                      gradient: const LinearGradient(
                        colors: [Color(0xFF1C4A45), Color(0xFF16213D)],
                      ),
                      borderRadius: BorderRadius.circular(20),
                      border: Border.all(
                        color: const Color(0xFF7EE2B8).withValues(alpha: 0.28),
                      ),
                    ),
                    child: Row(
                      children: [
                        ...
                        Expanded(
                          child: Text(
                            '${bnNum(bnDay)} ${info.name} • ${PanchangCalculator.weekdayName(_date)}',
                            style: const TextStyle(
                              color: Colors.white,
                              fontSize: 14,
                              fontWeight: FontWeight.w900,
                ...
                Container(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      Text(
                        abhijit['applicable'] == true
                            ? '☀️ অভিজিৎ: ${bnTime12(abhijit['start'] as DateTime)} – ${bnTime12(abhijit['end'] as DateTime)}'
                            : '☀️ অভিজিৎ মুহূর্ত আজ প্রযোজ্য নয়',
                        style: const TextStyle(
                          color: Colors.white,
                          fontSize: 12,
                        ),
                ...
                const SizedBox(height: 13),
                const Text(
                  '✅ রাহুকালের বাইরে দিনের সম্ভাব্য যাত্রা-উইন্ডো',
                  style: TextStyle(
                    color: Colors.white,
                    fontSize: 13,
                    fontWeight: FontWeight.w900,
                ...
                if (windows.isEmpty)
                  const Text(
                    'দীর্ঘ daylight window পাওয়া যায়নি।',
                  ...
                  ...List.generate(windows.length, (i) {
                    ...
                    return Container(
                      ...
                      padding: const EdgeInsets.all(13),
                      decoration: BoxDecoration(
                        color: const Color(0xFF15382F).withValues(alpha: 0.55),
                        borderRadius: BorderRadius.circular(16),
                        border: Border.all(
                          color: const Color(
                            0xFF7EE2B8,
                      ...
                      child: Row(
                        children: [
                          ...
                          Text(
                            '${bnTime12(w['start']!)} – ${bnTime12(w['end']!)}',
                            style: const TextStyle(
                              color: Color(0xFF7EE2B8),
                              fontSize: 11.5,
                              fontWeight: FontWeight.w800,
                ...
                Container(
                  ...
                  child: const Text(
                    'ℹ️ এটি পঞ্জিকা-ভিত্তিক ঐতিহ্যগত পরিকল্পনা সহায়ক। বাস্তব যাত্রায় আবহাওয়া, রাস্তা, স্বাস্থ্য ও নিরাপত্তার তথ্যকে অগ্রাধিকার দিন।',
                    style: TextStyle(
                      color: Colors.white54,
                      fontSize: 10.5,
                      height: 1.45,
...
class _Phase6DailyIntelligenceCard extends StatefulWidget {
  const _Phase6DailyIntelligenceCard();

  @override
  State<_Phase6DailyIntelligenceCard> createState() =>
      _Phase6DailyIntelligenceCardState();
}

class _Phase6DailyIntelligenceCardState
    extends State<_Phase6DailyIntelligenceCard> {
  int _openPlans = 0;
  ...
  Future<void> _load() async {
    ...
    setState(() {
      _openPlans = _LifePlannerStore.instance.items.where((e) {
        return !e.done && DateUtils.isSameDay(e.when, today);
      }).length;
    });
  }
  ...
  Widget build(BuildContext context) {
    final now = DateTime.now();
    final events = BengaliCalendarData.eventsFor(now);
    final festival = BengaliCalendarData.upcomingFestivals(count: 1);
    final next = festival.isEmpty ? null : festival.first;
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        gradient: LinearGradient(
          colors: [
            const Color(0xFF10243C).withValues(alpha: 0.82),
            const Color(0xFF203C39).withValues(alpha: 0.62),
          ],
        ),
        borderRadius: BorderRadius.circular(20),
        border: Border.all(
          color: const Color(0xFF7EE2B8).withValues(alpha: 0.22),
        ),
      ),
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          ...
          const SizedBox(height: 8),
          Text(
            'আজ ${bnNum(events.length)}টি পঞ্জিকা event • ${bnNum(_openPlans)}টি personal plan',
            style: const TextStyle(
              color: Colors.white,
              fontSize: 11.5,
              fontWeight: FontWeight.w700,
          ...
          if (next != null) ...[
            const SizedBox(height: 4),
            Text(
              'পরবর্তী: ${next['icon'] ?? '🎉'} ${next['title'] ?? ''} • ${bnNum(int.tryParse(next['daysLeft'] ?? '') ?? 0)} দিন বাকি',
              style: const TextStyle(color: Colors.white60, fontSize: 10.5),
            ),
          ],
          const SizedBox(height: 10),
              ...
              Expanded(
                child: OutlinedButton.icon(
                  onPressed: () => Navigator.push(
                    context,
                    MaterialPageRoute(
                      builder: (_) => const BengaliLifePlannerScreen(),
                    ),
                  ).then((_) => _load()),
                  icon: const Icon(Icons.checklist_rounded, size: 17),
                  label: const Text('Life Planner'),
                ),
              ),
              const SizedBox(width: 8),
              Expanded(
                child: OutlinedButton.icon(
                  onPressed: () => Navigator.push(
                    context,
                    MaterialPageRoute(
                      builder: (_) => const PanchangTimeMachineScreen(),
                    ),
                  ),
                  icon: const Icon(Icons.travel_explore_rounded, size: 17),
                  label: const Text('Time Machine'),
                ),
              ),
            ],
          ),
...
// =====================================================================
// SUPER APP PHASE-7 — SUPER 30 FEATURE HUB
// 30 new/expanded ideas are grouped in one clean hub so Home stays simple.
// Existing working screens are reused wherever possible; new local tools below
// reuse the app's Panchang engine, SharedPreferences, notifications and stores.
// =====================================================================

class _Super30Feature {
  final int id;
  final String icon;
  final String title;
  final String subtitle;
  const _Super30Feature(this.id, this.icon, this.title, this.subtitle);
}

const List<_Super30Feature> _super30Features = [
  _Super30Feature(
    1,
    '🧠',
    'Explainable Panchang',
    'কেন দিনটি ভালো/সতর্ক — কারণসহ',
  ),
  _Super30Feature(
    2,
    '🎯',
    'Smart Day Recommendation',
    'আগামী দিনের মধ্যে সেরা দিন খুঁজুন',
  ),
  _Super30Feature(3, '⚖️', 'Compare 2–3 Dates', 'একাধিক দিনের পঞ্চাঙ্গ তুলনা'),
  _Super30Feature(
    4,
    '🎊',
    'Festival Live Theme',
    'উৎসব অনুযায়ী live visual mode',
  ),
  _Super30Feature(
    5,
    '📍',
    'Local Bengali Hub',
    'জেলা, স্থানীয় পূজা ও saved temple event',
  ),
  _Super30Feature(
    6,
    '👨‍👩‍👧',
    'Family QR Sync',
    'পরিবারের event share/import sync code',
  ),
  _Super30Feature(7, '🧓', 'Elder Mode', 'বড় লেখা ও সহজ ৪টি প্রধান কাজ'),
  _Super30Feature(
    8,
    '🎙️',
    'Full Bengali Voice Command',
    'বাংলায় প্রশ্ন ও voice assistant',
  ),
  _Super30Feature(9, '🔔', 'Smart Reminder Center', 'সব reminder এক জায়গায়'),
  _Super30Feature(
    10,
    '🌅',
    'Morning Lock-Screen Brief',
    'সকালের পঞ্জিকা notification',
  ),
  _Super30Feature(
    11,
    '📱',
    'Smart Home Widget Pack',
    'তারিখ, তিথি ও festival widget preview',
  ),
  _Super30Feature(
    12,
    '🗓️',
    'Bengali Life Planner',
    'বাংলা তারিখে জীবন/কাজ পরিকল্পনা',
  ),
  _Super30Feature(
    13,
    '📊',
    'Personal Year Dashboard',
    'বছরের event, family ও reminder overview',
  ),
  _Super30Feature(
    14,
    '📿',
    'Devotional Hub',
    'জপ, পূজা checklist, temple, streak',
  ),
  _Super30Feature(
    15,
    '🌕',
    'Advanced Moon Center',
    'Moon phase, illumination ও next moon days',
  ),
  _Super30Feature(
    16,
    '🧭',
    'Journey Planner',
    'রাহুকাল এড়িয়ে traditional travel helper',
  ),
  _Super30Feature(
    17,
    '📷',
    'Daily Bengali Card Studio',
    'আজকের পঞ্জিকা share card',
  ),
  _Super30Feature(
    18,
    '🎨',
    'Festival Poster Studio Pro',
    'উৎসবের poster তৈরি ও share',
  ),
  _Super30Feature(
    19,
    '📰',
    'আজকের বাংলা Feed',
    'ইতিহাস, ঋতু, উৎসব ও দিনের তথ্য',
  ),
  _Super30Feature(
    20,
    '🌦️',
    'Weather + Panchang',
    'আবহাওয়া ও পঞ্জিকা এক card-এ',
  ),
  _Super30Feature(
    21,
    '🌾',
    'Season & Agriculture Calendar',
    'বাংলা ঋতু ও মৌসুমি তথ্য',
  ),
  _Super30Feature(
    22,
    '🛕',
    'Temple & Mela Map',
    'জেলা অনুযায়ী Maps search shortcut',
  ),
  _Super30Feature(
    23,
    '📥',
    'Offline Year Pack',
    'offline-ready year data status',
  ),
  _Super30Feature(
    24,
    '🔐',
    'Private Family Vault',
    'লোকাল PIN-এ family data সুরক্ষা',
  ),
  _Super30Feature(25, '☁️', 'Multi-device Backup', 'cloud backup/restore'),
  _Super30Feature(
    26,
    '✅',
    'Accuracy Center',
    'location ও calculation source transparency',
  ),
  _Super30Feature(27, '🔎', 'Universal Search', 'উৎসব, তিথি ও event search'),
  _Super30Feature(28, '✨', 'Dynamic Home', 'সময় অনুযায়ী Home intelligence'),
  _Super30Feature(
    29,
    '🏆',
    'Daily Streak & Badges',
    'পঞ্জিকা/সাধনা activity streak',
  ),
  _Super30Feature(
    30,
    '🧩',
    'Mini Apps',
    'Clock, converter, age, countdown tools',
  ),
];

class _Super30HomeCard extends StatelessWidget {
  const _Super30HomeCard();

  String _message() {
    final h = DateTime.now().hour;
    if (h < 6) return 'রাতের শান্ত সময় • আগামী দিনের পঞ্জিকা প্রস্তুত';
    if (h < 11) return 'সকালের Brief • আজকের পঞ্জিকা দিয়ে দিন শুরু করুন';
    if (h < 16) return 'দুপুরের Smart Tools • শুভ সময় ও কাজ মিলিয়ে নিন';
    if (h < 20) return 'সন্ধ্যার পূজা • প্রদীপ • পরিবার • আগামীকাল';
    return 'রাতের Review • Diary • Reminder • আগামী দিনের প্রস্তুতি';
  }
  ...
  Widget build(BuildContext context) {
    return InkWell(
      onTap: () => Navigator.push(
        context,
        MaterialPageRoute(builder: (_) => const Super30FeaturesScreen()),
      ),
      borderRadius: BorderRadius.circular(20),
      child: Container(
        width: double.infinity,
        ...
        decoration: BoxDecoration(
          gradient: LinearGradient(
            colors: [
              const Color(0xFF5B1A78).withValues(alpha: 0.82),
              const Color(0xFF07213D).withValues(alpha: 0.88),
            ],
          ),
          borderRadius: BorderRadius.circular(20),
          border: Border.all(
...
class Super30FeaturesScreen extends StatelessWidget {
  const Super30FeaturesScreen({super.key});

  void _push(BuildContext context, Widget page) {
    Navigator.push(context, MaterialPageRoute(builder: (_) => page));
  }

  void _open(BuildContext context, _Super30Feature f) {
    switch (f.id) {
      case 2:
        _push(context, const BestDayFinderScreen());
        return;
      case 3:
        _push(context, const PanchangCompareScreen());
        return;
      case 4:
        _push(context, const FestivalLiveModeScreen());
        return;
      case 8:
        _push(context, const PanjikaVoiceAssistantScreen());
        return;
      case 9:
        _push(context, const SmartReminderCenterScreen());
        return;
      case 11:
        _push(context, const HomeWidgetInfoScreen());
        return;
      case 12:
        _push(context, const BengaliLifePlannerScreen());
        return;
      case 13:
        _push(context, const MyYearMapScreen());
        return;
      case 16:
        _push(context, const TravelPanchangPlannerScreen());
        return;
      case 17:
        _push(context, const PanchangShareCardScreen());
        return;
      case 18:
        _push(context, const FestivalPosterScreen());
        return;
      case 25:
        _push(context, const CloudBackupScreen());
        return;
      case 27:
        _push(context, SearchScreen());
        return;
      case 29:
        _push(context, const SadhanaStreakScreen());
        return;
      default:
        _push(context, Super30DetailScreen(feature: f));
    }
  }
        ...
        children: [
          ...
          Expanded(
            child: GridView.builder(
              ...
              itemCount: _super30Features.length,
                  ...
                  child: Container(
                    ...
                    child: Column(
                      ...
                      children: [
                        Row(
                          children: [
                            Text(f.icon, style: const TextStyle(fontSize: 25)),
                            const Spacer(),
                            Text(
                              '${f.id}/30',
                              style: const TextStyle(
...
class Super30DetailScreen extends StatefulWidget {
  final _Super30Feature feature;
  const Super30DetailScreen({super.key, required this.feature});

  @override
  State<Super30DetailScreen> createState() => _Super30DetailScreenState();
}

class _Super30DetailScreenState extends State<Super30DetailScreen> {
  ...
  Widget _glass(Widget child, {EdgeInsets padding = const EdgeInsets.all(14)}) {
    return Container(
      width: double.infinity,
      padding: padding,
      decoration: BoxDecoration(
        color: const Color(0xFF06162F).withValues(alpha: 0.68),
        borderRadius: BorderRadius.circular(18),
        border: Border.all(color: Colors.white.withValues(alpha: 0.13)),
      ),
      child: child,
  ...
  Widget _line(String icon, String text, {Color color = Colors.white70}) =>
      Padding(
        padding: const EdgeInsets.symmetric(vertical: 5),
        child: Row(
          crossAxisAlignment: CrossAxisAlignment.start,
  ...
  Future<void> _scheduleMorningBrief() async {
    ...
    final t = PanchangCalculator.tithiFor(when);
    final info = BengaliDateUtil.monthInfoFor(when);
    final bnDay = when.difference(info.start).inDays + 1;
    await NotificationService.instance.scheduleGeneric(
      930701,
      '🌅 সকালের বাংলা পঞ্জিকা',
      '${bnNum(bnDay)} ${info.name} • ${t.paksha} পক্ষ • ${t.name}',
      when,
    );
    if (!mounted) return;
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text('আগামী সকাল ${bnTime12(when)}-এর Brief সেট হয়েছে'),
      ),
    );
  }

  String _familyExportCode() {
    final rows = FamilyMemberStore.instance.items
        .map(
          (f) => {
            'name': f.name,
            'occasion': f.occasion,
        ...
        .toList();
    return jsonEncode({'v': 1, 'family': rows});
  }

  Future<void> _importFamily() async {
    try {
      final data =
          jsonDecode(_syncController.text.trim()) as Map<String, dynamic>;
      final rows = (data['family'] as List?) ?? const [];
      for (final raw in rows) {
        final m = raw as Map<String, dynamic>;
        final item = FamilyMemberItem(
          m['name'].toString(),
          m['occasion'].toString(),
          DateTime.fromMillisecondsSinceEpoch((m['date'] as num).toInt()),
        );
        final exists = FamilyMemberStore.instance.items.any(
          (x) =>
              x.name == item.name &&
              x.occasion == item.occasion &&
              x.date.month == item.date.month &&
              x.date.day == item.date.day,
        );
        if (!exists) FamilyMemberStore.instance.items.add(item);
      }
      FamilyMemberStore.instance._sort();
      await FamilyMemberStore.instance._persist();
      if (!mounted) return;
      setState(() {});
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Family sync import সম্পূর্ণ')),
      );
    } catch (_) {
      if (!mounted) return;
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(const SnackBar(content: Text('Sync code সঠিক নয়')));
    }
  }
  ...
  Future<void> _openMaps(String query) async {
    final uri = Uri.https('www.google.com', '/maps/search/', {
      'api': '1',
      'query': '$query ${AppLocation.district}',
    });
    await launchUrl(uri, mode: LaunchMode.externalApplication);
  }

  Future<void> _vaultAction() async {
    final p = await SharedPreferences.getInstance();
    final saved = p.getString('super30_family_vault_pin');
    ...
    final entered = await showDialog<String>(
      ...
      builder: (context) => AlertDialog(
        title: Text(
          saved == null ? 'নতুন Family Vault PIN' : 'Family Vault PIN',
        ),
        content: TextField(
          controller: ctrl,
          ...
          obscureText: true,
          maxLength: 6,
          decoration: const InputDecoration(hintText: '৪–৬ সংখ্যার PIN'),
        ),
        actions: [
          TextButton(
            onPressed: () => Navigator.pop(context),
    ...
    if (saved == null) {
      await p.setString('super30_family_vault_pin', entered);
      ...
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(const SnackBar(content: Text('PIN মিলেনি')));
    }
  }

  Widget _explainablePanchang() {
    final now = DateTime.now();
    final sun = PanchangCalculator.sunTimes(
      now,
      lat: AppLocation.lat,
    ...
    final t = PanchangCalculator.tithiFor(sun.sunrise);
    final nak = PanchangCalculator
        .nakshatraNames[PanchangCalculator.nakshatraIndexFor(sun.sunrise)];
    final rashi = PanchangCalculator
        .rashiNames[PanchangCalculator.rashiIndexFor(sun.sunrise)];
    final rahu = PanchangCalculator.rahuKalam(now);
    final abh = PanchangCalculator.abhijitMuhurta(now);
    final insideRahu =
        now.isAfter(rahu['start']!) && now.isBefore(rahu['end']!);
    int score = 70;
    if (abh['applicable'] == true) score += 12;
    if (insideRahu) score -= 18;
    if (BengaliCalendarData.eventsFor(now).isNotEmpty) score += 4;
      ...
      children: [
        _glass(
          Column(
            ...
            children: [
              _heading(
                'আজকের ঐতিহ্যভিত্তিক Panchang Score • ${bnNum(score)}/১০০',
              ),
              const SizedBox(height: 8),
              _line('🕉', '${t.paksha} পক্ষ • ${t.name}'),
              _line('⭐', 'নক্ষত্র: $nak'),
              _line('🪐', 'চন্দ্র রাশি: $rashi'),
              _line(
                '⏳',
                insideRahu
                    ? 'এখন রাহুকালের মধ্যে — নতুন কাজের সময় বাছাইয়ে সতর্কতা'
                    : 'এখন রাহুকালের বাইরে',
              ),
              _line(
                '☀️',
                abh['applicable'] == true
                    ? 'আজ অভিজিৎ মুহূর্ত পাওয়া যাচ্ছে'
                    : 'আজ অভিজিৎ মুহূর্ত প্রযোজ্য নয়',
              ),
              const SizedBox(height: 8),
              const Text(
                'এটি ঐতিহ্যভিত্তিক planning helper; নিশ্চিত ভবিষ্যদ্বাণী বা পেশাদার পরামর্শ নয়।',
  ...
  Widget _localHub() {
    final festivals = BengaliCalendarData.upcomingFestivals(count: 4);
    return Column(
      children: [
        _glass(
          Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              _heading('📍 ${AppLocation.district} Local Hub'),
              const SizedBox(height: 8),
              _line(
                '🎉',
                'আসন্ন উৎসব: ${festivals.isEmpty ? 'তথ্য নেই' : festivals.map((e) => e['title']).join(' • ')}',
              ),
              _line(
                '🛕',
                'আপনার saved temple/puja event: ${bnNum(TempleStore.instance.items.length)} টি',
              ),
              _line(
                '👨‍👩‍👧',
                'Family occasion: ${bnNum(FamilyMemberStore.instance.items.length)} টি saved',
              ),
            ],
          ),
        ),
        const SizedBox(height: 10),
        _button('স্থানীয় মন্দির Maps-এ খুঁজুন', () => _openMaps('temples in')),
      ],
    );
  }

  Widget _familySync() {
    final code = _familyExportCode();
    return Column(
      children: [
        _glass(
          Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              _heading('Family QR / Sync Code'),
              const SizedBox(height: 8),
              const Text(
                'এই build-এ camera QR scanner যোগ না করে dependency-free share/import sync code রাখা হয়েছে। একই code পরে QR-এ encode করা যাবে।',
                style: TextStyle(
                  color: Colors.white60,
                  fontSize: 10.5,
                  height: 1.4,
              ...
              const SizedBox(height: 10),
              _button(
                'Family Sync Code Share',
                () => Share.share(code, subject: 'Bangla Panjika Family Sync'),
              ),
            ],
          ),
        ),
          ...
          Column(
            children: [
              TextField(
                ...
                style: const TextStyle(color: Colors.white, fontSize: 11),
                decoration: const InputDecoration(
                  hintText: 'অন্য ফোনের sync code paste করুন',
                  hintStyle: TextStyle(color: Colors.white38),
                ),
              ),
              const SizedBox(height: 8),
              _button('Import Family Data', _importFamily),
            ],
          ),
        ),
      ],
  ...
  Widget _elderMode() {
    Widget big(String emoji, String label, Widget page) => Padding(
    ...
    return Column(
      children: [
        big('🌞', 'আজকের পঞ্জিকা', const TodaySmartScreen()),
        big('📅', 'ক্যালেন্ডার', const BengaliCalendarScreen()),
        big('🎙️', 'কথা বলে জিজ্ঞাসা', const PanjikaVoiceAssistantScreen()),
        big('🔔', 'রিমাইন্ডার', const ReminderScreen()),
      ],
    );
  }

  Widget _morningLockBrief() {
    final now = DateTime.now();
    final t = PanchangCalculator.tithiFor(now);
    final info = BengaliDateUtil.monthInfoFor(now);
    final bnDay = now.difference(info.start).inDays + 1;
    return Column(
      children: [
        _glass(
          Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              _heading('Lock-screen Morning Preview'),
              const SizedBox(height: 8),
              _line('📅', '${bnNum(bnDay)} ${info.name} ${bnNum(info.year)}'),
              _line('🕉', '${t.paksha} পক্ষ • ${t.name}'),
              _line(
                '🎉',
                BengaliCalendarData.eventsFor(now).isEmpty
                    ? 'আজ বড় কোনো saved festival নেই'
        ...
        const SizedBox(height: 10),
        _button(
          'আগামীকাল সকাল ৭টার Brief Set করুন',
      ...
      Column(
        children: [
          _button(
            ...
            () => Navigator.push(
              context,
              MaterialPageRoute(builder: (_) => const JapaCounterScreen()),
            ),
          ),
          const SizedBox(height: 8),
          _button(
          ...
          _button(
            ...
            () => Navigator.push(
              context,
              MaterialPageRoute(builder: (_) => const SadhanaStreakScreen()),
            ),
          ),
        ],
      ),
  ...
  Widget _advancedMoon() {
    final now = DateTime.now();
    const synodic = 29.530588853;
    final age = PanchangCalculator.moonAgeDays(now) % synodic;
    final angle = age / synodic * 2 * math.pi;
    final illumination = ((1 - math.cos(angle)) / 2 * 100).round();
    final waxing = age < synodic / 2;
    DateTime? nextPurnima;
    DateTime? nextAmavasya;
    for (int i = 0; i < 45; i++) {
      final d = DateTime(now.year, now.month, now.day).add(Duration(days: i));
      final ev = BengaliCalendarData.eventsFor(d);
      if (nextPurnima == null && ev.any((e) => e.category == 'purnima'))
        nextPurnima = d;
      if (nextAmavasya == null && ev.any((e) => e.category == 'amabasya'))
        nextAmavasya = d;
    }
    return Column(
      children: [
        _glass(
          Column(
            ...
            children: [
              ...
              const SizedBox(height: 10),
              _line('💡', 'আলোকিত অংশ প্রায় ${bnNum(illumination)}%'),
              _line(
                '🌗',
                waxing ? 'শুক্লপক্ষ • চাঁদ বাড়ছে' : 'কৃষ্ণপক্ষ • চাঁদ কমছে',
              ),
              _line(
                '🌕',
                nextPurnima == null
                    ? 'পরবর্তী পূর্ণিমা তালিকায় পাওয়া যায়নি'
                    : 'পরবর্তী পূর্ণিমা: ${bnNum(nextPurnima.day)} ${gregMonthBn(nextPurnima.month)}',
              ),
              _line(
                '🌑',
                nextAmavasya == null
                    ? 'পরবর্তী অমাবস্যা তালিকায় পাওয়া যায়নি'
                    : 'পরবর্তী অমাবস্যা: ${bnNum(nextAmavasya.day)} ${gregMonthBn(nextAmavasya.month)}',
              ),
            ],
          ),
        ),
  ...
  Widget _dailyFeed() {
    ...
    final info = BengaliDateUtil.monthInfoFor(now);
    final bnDay = now.difference(info.start).inDays + 1;
    final eventNames = BengaliCalendarData.eventsFor(
      now,
    ).map((e) => '${e.icon} ${e.label}').toList();
    final history = _historicalFigures
        .where((f) => f.month == now.month && f.day == now.day)
        .take(4)
        .toList();
    final season = _seasonName(info.name);
    return Column(
      children: [
        _glass(
          Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              _heading('📰 আজকের বাংলা'),
              const SizedBox(height: 8),
              _line('📅', '${bnNum(bnDay)} ${info.name} • $season'),
              _line(
                '🎉',
                eventNames.isEmpty
                    ? 'আজ কোনো বড় saved festival নেই'
                    : eventNames.join(' • '),
              ),
              if (history.isEmpty)
                _line('📚', 'আজকের verified history list-এ কোনো entry নেই')
              else
                ...history.map(
                  (h) => _line(
                    h.emoji,
                    '${h.name} • ${h.eventType}দিন • ${h.role}',
                  ),
                ),
            ],
          ),
  ...
  String _seasonName(String month) {
    if (['বৈশাখ', 'জ্যৈষ্ঠ'].contains(month)) return 'গ্রীষ্ম';
    if (['আষাঢ়', 'শ্রাবণ'].contains(month)) return 'বর্ষা';
    if (['ভাদ্র', 'আশ্বিন'].contains(month)) return 'শরৎ';
    if (['কার্তিক', 'অগ্রহায়ণ'].contains(month)) return 'হেমন্ত';
    if (['পৌষ', 'মাঘ'].contains(month)) return 'শীত';
    return 'বসন্ত';
  }

  Widget _weatherPanchang() {
    final now = DateTime.now();
    final temp = WeatherService.instance.tempC;
    final rain = WeatherService.instance.isRaining;
    final sun = PanchangCalculator.sunTimes(
      now,
      lat: AppLocation.lat,
      lon: AppLocation.lon,
    ...
    return _glass(
      Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          _heading('🌦️ Weather + Panchang'),
          const SizedBox(height: 8),
          _line(
            rain ? '🌧️' : '🌤️',
            temp == null
                ? 'লাইভ temperature এখনও পাওয়া যায়নি'
                : 'তাপমাত্রা ${temp.toStringAsFixed(1)}°C • ${rain ? 'বৃষ্টি হচ্ছে' : 'বৃষ্টি নেই'}',
          ),
          _line('🕉', '${t.paksha} পক্ষ • ${t.name}'),
          _line(
            '🌅',
            'সূর্যোদয় ${bnTime12(sun.sunrise)} • সূর্যাস্ত ${bnTime12(sun.sunset)}',
          ),
          _line(
            '⏳',
            'রাহুকাল ${bnTime12(rahu['start']!)}–${bnTime12(rahu['end']!)}',
  ...
  Widget _agriSeason() {
    final info = BengaliDateUtil.monthInfoFor(DateTime.now());
    final season = _seasonName(info.name);
    final notes = <String, List<String>>{
      'গ্রীষ্ম': [
        'গরম ও জলসংরক্ষণে নজর দিন',
        'স্থানীয় কৃষি দপ্তরের পরামর্শ অনুযায়ী সেচ পরিকল্পনা করুন',
      ],
      'বর্ষা': [
        'জলনিকাশ ও অতিবৃষ্টির ঝুঁকি নজরে রাখুন',
        'রোপণ/চাষের সিদ্ধান্তে স্থানীয় আবহাওয়া ও কৃষি দপ্তরের তথ্য দেখুন',
      ],
      'শরৎ': [
        'বর্ষা-পরবর্তী জমির অবস্থা পরীক্ষা করুন',
        'উৎসব মৌসুমে ফসল/বাজার পরিকল্পনা আলাদা করে রাখুন',
      ],
      'হেমন্ত': [
        'শুষ্কতা বাড়তে পারে—মাটির আর্দ্রতা দেখুন',
        'রবি মৌসুমের স্থানীয় কৃষি নির্দেশিকা অনুসরণ করুন',
      ],
      'শীত': [
        'কুয়াশা ও ঠান্ডার প্রভাব নজরে রাখুন',
        'সেচ ও রোগবালাই বিষয়ে স্থানীয় বিশেষজ্ঞের তথ্য নিন',
      ],
      'বসন্ত': [
        'তাপমাত্রা পরিবর্তন দ্রুত হতে পারে',
        'ফসল কাটাই/পরবর্তী মৌসুমের প্রস্তুতি স্থানীয় তথ্য ধরে করুন',
      ],
    };
    return _glass(
      Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          _heading('🌾 ${info.name} • $season'),
          const SizedBox(height: 8),
          for (final n in notes[season] ?? const <String>[]) _line('•', n),
          const SizedBox(height: 8),
          const Text(
            'এটি সাধারণ মৌসুমি তথ্য; কৃষি সিদ্ধান্তের জন্য স্থানীয় কৃষি বিশেষজ্ঞ/সরকারি পরামর্শ অনুসরণ করুন।',
            style: TextStyle(color: Colors.white38, fontSize: 9.5, height: 1.4),
          ),
        ],
      ),
    );
      ...
      children: [
        _glass(
          Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              _heading('🛕 ${AppLocation.district} Temple & Mela Map'),
              const SizedBox(height: 8),
              _line(
                '📍',
                'Google Maps app/web-এ আপনার নির্বাচিত জেলার search খুলবে।',
              ),
              _line(
                '🗓️',
                'Saved temple events: ${bnNum(TempleStore.instance.items.length)}',
              ),
            ],
          ),
        ),
        const SizedBox(height: 10),
        _button('মন্দির খুঁজুন', () => _openMaps('temples in')),
        const SizedBox(height: 8),
        _button(
          'মেলা / Puja Pandal খুঁজুন',
          () => _openMaps('mela puja pandal in'),
        ),
      ],
    );
  }
  ...
  Widget _offlinePack() {
    ...
    int eventCount = 0;
    for (int i = 0; i < 366; i++) {
      eventCount += BengaliCalendarData.eventsFor(
    ...
    return _glass(
      Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          _heading('📥 Offline Year Pack • ${bnNum(now.year)}'),
          const SizedBox(height: 8),
          _line(
            '✅',
            'বাংলা date conversion ও Panchang calculation app-এর ভেতরেই আছে',
          ),
          _line('✅', 'এই বছরের local event entries: ${bnNum(eventCount)}'),
          _line('✅', 'Reminder/Diary/Family data ফোনে local save থাকে'),
          _line(
            '🌐',
            'Weather, cloud backup, payment এবং online map-এর জন্য internet লাগবে',
          ),
        ],
      ),
    );
  ...
  Widget _vault() {
        ...
        children: [
          _glass(
            Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                _heading('🔐 Private Family Vault'),
                const SizedBox(height: 8),
                const Text(
                  'লোকাল PIN দিয়ে family list-এর এই private view খুলুন। এটি high-security encrypted vault নয়; biometric/local_auth dependency ছাড়াই lightweight privacy layer।',
                  style: TextStyle(
                    color: Colors.white60,
                    fontSize: 10.5,
                    height: 1.4,
    ...
    return _glass(
      Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          _heading('🔓 Family Vault Open'),
          const SizedBox(height: 8),
          if (FamilyMemberStore.instance.items.isEmpty)
            const Text(
              'Family Calendar-এ এখনো data নেই।',
              style: TextStyle(color: Colors.white60),
            )
          else
            ...FamilyMemberStore.instance.items.map(
              (f) => _line(
                '👤',
                '${f.name} • ${f.occasion} • ${bnNum(f.date.day)} ${gregMonthBn(f.date.month)}',
              ),
            ),
        ],
      ),
  ...
  Widget _accuracy() {
    ...
    return _glass(
      Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          _heading('✅ Panchang Accuracy Center'),
          const SizedBox(height: 8),
          _line(
            '📍',
            'Location: ${AppLocation.district} • lat ${AppLocation.lat.toStringAsFixed(4)} • lon ${AppLocation.lon.toStringAsFixed(4)}',
          ),
          _line('🌅', 'এই location দিয়ে আজ সূর্যোদয় ${bnTime12(sun.sunrise)}'),
          _line(
            '🧮',
            'তিথি, নক্ষত্র, রাশি ও সময় app-এর PanchangCalculator engine থেকে হিসাব করা হয়',
          ),
          _line(
            '📅',
            'উৎসব/বিশেষ event BengaliCalendarData-এর local curated rules/data থেকে আসে',
          ),
          _line(
            '⚠️',
            'শুভদিন/score ঐতিহ্যভিত্তিক helper; গুরুত্বপূর্ণ সিদ্ধান্তে বিশেষজ্ঞ যাচাই উপযোগী',
          ),
        ],
      ),
    );
  ...
  Widget _dynamicHome() {
    ...
    if (h < 10) {
      ...
      items = [
        'আজকের তিথি দেখুন',
        'সূর্যোদয় ও রাহুকাল',
        'আজকের কাজ ও reminder',
      ];
    } else if (h < 17) {
      title = '☀️ Day Home';
      items = ['Best Day / Muhurta', 'Life Planner', 'Weather + Panchang'];
    } else if (h < 21) {
      title = '🪔 Evening Home';
      items = ['পূজা Checklist', 'সন্ধ্যা প্রদীপ', 'Family Occasion'];
    } else {
      title = '🌙 Night Home';
      items = ['Panjika Diary', 'আগামীকাল preview', 'Reminder review'];
    }
    return _glass(
      Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          _heading(title),
          const SizedBox(height: 8),
          for (final x in items) _line('✨', x),
          const SizedBox(height: 8),
          const Text(
            'Home-এর SUPER 30 card-টিও সময় অনুযায়ী message বদলায়।',
            style: TextStyle(color: Colors.white38, fontSize: 9.5),
          ),
        ],
      ),
  ...
  Widget _miniApps() {
    final now = DateTime.now();
    String ageText = 'জন্মতারিখ নির্বাচন করুন';
    if (_miniDob != null) {
      int y = now.year - _miniDob!.year;
      if (DateTime(now.year, _miniDob!.month, _miniDob!.day).isAfter(now)) y--;
      ageText = 'বয়স প্রায় ${bnNum(y)} বছর';
    }
    String cdText = 'Countdown date নির্বাচন করুন';
    if (_countdownDate != null) {
      ...
      cdText = days < 0 ? 'তারিখটি চলে গেছে' : '${bnNum(days)} দিন বাকি';
    }
    return Column(
      children: [
        _glass(
            ...
            children: [
              ...
              _button(
                ...
                () => Navigator.push(
                  context,
                  MaterialPageRoute(
                    builder: (_) => const FestivalCountdownScreen(),
                  ),
                ),
              ),
            ],
        ...
        _glass(
          Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              _heading('🎂 Age Calculator'),
              const SizedBox(height: 6),
              Text(ageText, style: const TextStyle(color: Colors.white70)),
              TextButton(
                onPressed: () async {
                  final d = await showDatePicker(
                    context: context,
                    initialDate: _miniDob ?? DateTime(1995),
                    firstDate: DateTime(1900),
                    lastDate: now,
                  );
                  if (d != null && mounted) setState(() => _miniDob = d);
                },
                child: const Text('জন্মতারিখ বেছে নিন'),
              ),
            ],
          ),
        ),
        ...
        _glass(
          Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              _heading('⏳ Countdown Maker'),
              const SizedBox(height: 6),
              Text(cdText, style: const TextStyle(color: Colors.white70)),
              TextButton(
                onPressed: () async {
                  final d = await showDatePicker(
                    context: context,
                    initialDate:
                        _countdownDate ?? now.add(const Duration(days: 30)),
                    firstDate: now,
                    lastDate: DateTime(now.year + 10),
                  );
                  if (d != null && mounted) setState(() => _countdownDate = d);
                },
                child: const Text('Future date বেছে নিন'),
              ),
            ],
          ),
        ),
  ...
  Widget _content() {
    switch (widget.feature.id) {
      case 1:
        return _explainablePanchang();
      case 5:
        return _localHub();
      case 6:
        return _familySync();
      case 7:
        return _elderMode();
      case 10:
        return _morningLockBrief();
      case 14:
        return _devotionalHub();
      case 15:
        return _advancedMoon();
      case 19:
        return _dailyFeed();
      case 20:
        return _weatherPanchang();
      case 21:
        return _agriSeason();
      case 22:
        return _templeMap();
  ...
  Widget build(BuildContext context) {
    return CosmicBackground(
      child: Column(
        children: [
          _ScreenHeader(
            title: '${widget.feature.icon} ${widget.feature.title}',
          ),
          Expanded(
            child: ListView(
              padding: const EdgeInsets.fromLTRB(14, 8, 14, 30),
...
// =====================================================================
// SUPER APP PHASE-9 — গৃহ ও বাস্তু (Traditional Vastu Companion)
// নোট: এই অংশটি ঐতিহ্যগত বাস্তু-ধারণার তথ্য/প্ল্যানিং সহায়ক। এটি
// structural engineering, electrical/plumbing/fire-safety বা স্থানীয়
// building-code-এর বিকল্প নয়। বাস্তব নির্মাণে licensed architect/engineer
// ও স্থানীয় নিয়মই চূড়ান্ত।
// =====================================================================

class _VastuGlassCard extends StatelessWidget {
  final Widget child;
  final EdgeInsetsGeometry padding;
  const _VastuGlassCard({
    required this.child,
...
class VastuHomeHubScreen extends StatelessWidget {
  const VastuHomeHubScreen({super.key});
  ...
  @override
  Widget build(BuildContext context) {
    final tools = <(String, String, String, Widget)>[
      (
        '🧭',
        '৮ দিক ও ব্রহ্মস্থান',
        'কোন কোণে কী রাখা হয় — traditional guide',
        const VastuDirectionGuideScreen(),
      ),
      (
        '📐',
        'Room Planner Check',
        'ঘর + দিক বেছে ঐতিহ্যগত suitability দেখুন',
        const VastuRoomPlannerScreen(),
      ),
      (
        '🚪',
        'Main Door & Plot',
        'Plot facing ও প্রবেশদ্বারের direction notes',
        const VastuEntranceScreen(),
      ),
      (
        '🔥',
        'Kitchen & Puja',
        'অগ্নিকোণ, ঈশানকোণ ও layout tips',
        const VastuKitchenPujaScreen(),
      ),
      (
        '🛏️',
        'Bedroom & Study',
        'শোওয়া, পড়াশোনা ও guest room guide',
        const VastuBedroomStudyScreen(),
      ),
      (
        '🚿',
        'Utility Zones',
        'Toilet, staircase, water/storage traditional notes',
        const VastuUtilityScreen(),
      ),
      (
        '✅',
        'Home Vastu Checklist',
        'নিজের বাড়ির planning checklist save করুন',
        const VastuHomeChecklistScreen(),
      ),
      (
        '⭐',
        'গৃহ/ভূমি শুভদিন',
        'গৃহপ্রবেশ ও শুভ মুহূর্ত finder',
        const ShubhoMuhurtaScreen(),
      ),
    ];
    ...
    return CosmicBackground(
      child: Column(
        children: [
          const _ScreenHeader(title: '🏠 গৃহ ও বাস্তু'),
          Expanded(
            child: ListView(
              padding: const EdgeInsets.fromLTRB(14, 8, 14, 30),
              children: [
                _VastuGlassCard(
                  child: Column(
                    ...
                    children: [
                      ...
                      const SizedBox(height: 6),
                      const Text(
                        'দিক, ঘরের অবস্থান, প্রবেশদ্বার ও গৃহ-পরিকল্পনার প্রচলিত বাস্তু ধারণা এক জায়গায়।',
                        style: TextStyle(
                          color: Colors.white,
                          fontSize: 13,
                          height: 1.45,
                      ...
                      Container(
                        ...
                        child: const Text(
                          '⚠️ বাস্তু ঐতিহ্যভিত্তিক নির্দেশিকা। Foundation, column, electrical, plumbing, fire safety, setback ও structural design অবশ্যই qualified architect/engineer ও স্থানীয় building rules অনুযায়ী করুন।',
                          style: TextStyle(
                            color: Colors.white70,
                            fontSize: 11.2,
                            height: 1.4,
                ...
                GridView.builder(
                  ...
                  itemBuilder: (context, i) {
                    final t = tools[i];
                    return InkWell(
                      onTap: () => _push(context, t.$4),
                      borderRadius: BorderRadius.circular(18),
                      child: _VastuGlassCard(
                        padding: const EdgeInsets.all(12),
                        child: Column(
                          mainAxisAlignment: MainAxisAlignment.center,
                          children: [
                            Text(t.$1, style: const TextStyle(fontSize: 30)),
                            const SizedBox(height: 7),
                            Text(
                              t.$2,
                              textAlign: TextAlign.center,
...
class _VastuDirectionInfo {
  final String icon;
  final String name;
  final String compass;
  final String traditionalUse;
  final String note;
  const _VastuDirectionInfo(
    this.icon,
    this.name,
    this.compass,
    this.traditionalUse,
...
const List<_VastuDirectionInfo> _vastuDirections = [
  _VastuDirectionInfo(
    '🕉️',
    'ঈশান কোণ',
    'উত্তর-পূর্ব • NE',
    'পূজা/ধ্যান, হালকা ও পরিষ্কার স্থান',
    'প্রচলিত মতে এই কোণ খোলা, আলো-বাতাসপূর্ণ ও কম ভারী রাখা পছন্দ করা হয়।',
  ),
  _VastuDirectionInfo(
    '🌅',
    'পূর্ব',
    'East • E',
    'আলো, জানালা, study/entrance-এর সম্ভাব্য zone',
    'সকালের আলো পাওয়ার দিক হিসেবে পূর্বকে গুরুত্ব দেওয়া হয়।',
  ),
  _VastuDirectionInfo(
    '🔥',
    'অগ্নি কোণ',
    'দক্ষিণ-পূর্ব • SE',
    'রান্নাঘর/আগুন-সম্পর্কিত zone',
    'প্রচলিত বাস্তুতে kitchen-এর সবচেয়ে পরিচিত zone দক্ষিণ-পূর্ব।',
  ),
  _VastuDirectionInfo(
    '🧱',
    'দক্ষিণ',
    'South • S',
    'ভারী storage/দেয়াল বা secondary zone',
    'এখানে planning করার সময় ventilation ও বাস্তব heat-gain-ও বিবেচনা করুন।',
  ),
  _VastuDirectionInfo(
    '🛏️',
    'নৈঋত কোণ',
    'দক্ষিণ-পশ্চিম • SW',
    'Master bedroom/ভারী furniture',
    'প্রচলিত মতে স্থিতিশীল/ভারী অংশের জন্য এই zone ব্যবহৃত হয়।',
  ),
  _VastuDirectionInfo(
    '🌇',
    'পশ্চিম',
    'West • W',
    'Dining/store/secondary room',
    'ঘরের প্রকৃত ব্যবহার, আলো ও ventilation অনুযায়ী layout ঠিক করুন।',
  ),
  _VastuDirectionInfo(
    '🌬️',
    'বায়ব্য কোণ',
    'উত্তর-পশ্চিম • NW',
    'Guest/utility/চলাচল-ভিত্তিক room',
    'প্রচলিত মতে guest বা movement-oriented space-এর সাথে এই দিক যুক্ত।',
  ),
  _VastuDirectionInfo(
    '💼',
    'উত্তর',
    'North • N',
    'Study/work/opening/storage-এর কিছু ব্যবহার',
    'উত্তরকে হালকা/খোলা রাখা নিয়ে বহু প্রচলিত বাস্তু নির্দেশনা আছে।',
  ),
  _VastuDirectionInfo(
    '✨',
    'ব্রহ্মস্থান',
    'বাড়ির কেন্দ্র',
    'খোলা, পরিষ্কার ও কম clutter',
    'প্রচলিত মতে কেন্দ্রস্থলে অতিরিক্ত ভারী obstruction কম রাখা ভালো ধরা হয়।',
  ),
];

class VastuDirectionGuideScreen extends StatelessWidget {
  const VastuDirectionGuideScreen({super.key});
  ...
  Widget build(BuildContext context) {
    return CosmicBackground(
      child: Column(
        children: [
          const _ScreenHeader(title: '🧭 ৮ দিক ও ব্রহ্মস্থান'),
          Expanded(
            child: ListView.separated(
              padding: const EdgeInsets.fromLTRB(14, 8, 14, 30),
              itemCount: _vastuDirections.length,
              separatorBuilder: (_, _) => const SizedBox(height: 10),
              itemBuilder: (context, i) {
                final d = _vastuDirections[i];
                return _VastuGlassCard(
                  child: Row(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      Container(
...
class _RoomVastuRule {
  final List<String> preferred;
  final List<String> acceptable;
  final String reason;
  const _RoomVastuRule(this.preferred, this.acceptable, this.reason);
}

const Map<String, _RoomVastuRule> _roomVastuRules = {
  'পূজার ঘর': _RoomVastuRule(
    ['NE', 'E'],
    ['N'],
    'প্রচলিত বাস্তুতে ঈশান/পূর্ব দিককে পূজা ও ধ্যানের জন্য বেশি পছন্দ করা হয়।',
  ),
  'রান্নাঘর': _RoomVastuRule(
    ['SE'],
    ['NW'],
    'অগ্নি-সম্পর্কিত কাজের জন্য দক্ষিণ-পূর্বকে সবচেয়ে প্রচলিত kitchen zone ধরা হয়।',
  ),
  'Master Bedroom': _RoomVastuRule(
    ['SW'],
    ['S', 'W'],
    'নৈঋত কোণকে স্থিতিশীল/ভারী zone হিসেবে master bedroom-এর সাথে যুক্ত করা হয়।',
  ),
  'Guest Room': _RoomVastuRule(
    ['NW'],
    ['N', 'W'],
    'বায়ব্যকে movement/guest-oriented zone হিসেবে দেখা হয়।',
  ),
  'Study Room': _RoomVastuRule(
    ['E', 'N', 'NE'],
    ['W'],
    'পূর্ব/উত্তর/ঈশানকে আলো, মনোযোগ ও হালকা ব্যবহারের সাথে যুক্ত করা হয়।',
  ),
  'Dining': _RoomVastuRule(
    ['W', 'E'],
    ['N'],
    'Dining-এর ক্ষেত্রে practical circulation, kitchen proximity ও ventilation-ও সমান গুরুত্বপূর্ণ।',
  ),
};

class VastuRoomPlannerScreen extends StatefulWidget {
  const VastuRoomPlannerScreen({super.key});
  @override
  State<VastuRoomPlannerScreen> createState() => _VastuRoomPlannerScreenState();
}

class _VastuRoomPlannerScreenState extends State<VastuRoomPlannerScreen> {
  String _room = _roomVastuRules.keys.first;
  String _direction = 'NE';

  String get _status {
    final r = _roomVastuRules[_room]!;
    if (r.preferred.contains(_direction)) return 'খুব প্রচলিত পছন্দ';
    if (r.acceptable.contains(_direction)) return 'গ্রহণযোগ্য বিকল্প';
    return 'অন্য layout-ও বিবেচনা করুন';
  }

  Color get _statusColor {
    final r = _roomVastuRules[_room]!;
    if (r.preferred.contains(_direction)) return const Color(0xFF65D69A);
    if (r.acceptable.contains(_direction)) return const Color(0xFFFFD36E);
    return const Color(0xFFFF9B7A);
  }

  @override
  Widget build(BuildContext context) {
    const dirs = ['N', 'NE', 'E', 'SE', 'S', 'SW', 'W', 'NW'];
    final rule = _roomVastuRules[_room]!;
    return CosmicBackground(
      child: Column(
        children: [
          const _ScreenHeader(title: '📐 Room Planner Check'),
          Expanded(
            child: ListView(
              padding: const EdgeInsets.fromLTRB(14, 8, 14, 30),
              children: [
                ...
                _VastuGlassCard(
                  child: Column(
                    ...
                    children: [
                      ...
                      const SizedBox(height: 8),
                      Text(
                        'Preferred: ${rule.preferred.join(', ')}  •  Alternative: ${rule.acceptable.join(', ')}',
                        style: const TextStyle(
                          color: Colors.white60,
                          fontSize: 10.5,
                        ),
                      ...
                      const SizedBox(height: 10),
                      const Text(
                        'এই ফল traditional Vastu reference; বাড়ির বাস্তব shape, ventilation, sunlight, plumbing ও structure দেখে architect-এর plan-ই চূড়ান্ত।',
                        style: TextStyle(
                          color: Colors.white54,
                          fontSize: 10.2,
                          height: 1.35,
...
class VastuEntranceScreen extends StatefulWidget {
  const VastuEntranceScreen({super.key});
  @override
  State<VastuEntranceScreen> createState() => _VastuEntranceScreenState();
}

class _VastuEntranceScreenState extends State<VastuEntranceScreen> {
  String _facing = 'East';
  static const notes = {
    'East':
        'পূর্বমুখী plot-এ সকালের আলো ও entrance circulation-কে গুরুত্ব দিন। Traditional Vastu-তে পূর্ব/উত্তর-পূর্ব অংশ তুলনামূলক খোলা রাখার কথা বলা হয়।',
    'North':
        'উত্তরমুখী plot-এ north/north-east opening নিয়ে প্রচলিত ইতিবাচক ধারণা আছে। বাস্তবে road level, drainage ও privacy আগে দেখুন।',
    'West':
        'পশ্চিমমুখী plot-ও ব্যবহারযোগ্য। Afternoon heat, shading, ventilation এবং entrance placement বাস্তব design-এ গুরুত্বপূর্ণ।',
    'South':
        'দক্ষিণমুখী plot স্বয়ংক্রিয়ভাবে “খারাপ” নয়। Traditional rules schoolভেদে বদলায়; heat control, shade, drainage ও functional plan-কে অগ্রাধিকার দিন।',
  };
  ...
  Widget build(BuildContext context) {
    return CosmicBackground(
      child: Column(
        children: [
          const _ScreenHeader(title: '🚪 Main Door & Plot'),
          Expanded(
            child: ListView(
              padding: const EdgeInsets.fromLTRB(14, 8, 14, 30),
              children: [
                        ...
                        children: notes.keys
                            .map(
                              (d) => ChoiceChip(
                                label: Text(d),
                                selected: _facing == d,
                                onSelected: (_) => setState(() => _facing = d),
                                selectedColor: const Color(0xFFFFD36E),
                                backgroundColor: Colors.white.withValues(
                                  alpha: 0.08,
                                ),
                ...
                const _VastuGlassCard(
                  child: Column(
                    ...
                    children: [
                      ...
                      SizedBox(height: 8),
                      Text(
                        '• দরজার সামনে নিরাপদ ও বাধাহীন circulation\n• পর্যাপ্ত আলো ও ventilation\n• বৃষ্টির পানি/ড্রেনেজ যেন entrance-এ না জমে\n• Door swing যেন passage block না করে\n• স্থানীয় setback ও fire-safety rule মানুন',
                        style: TextStyle(
                          color: Colors.white70,
                          fontSize: 12,
                          height: 1.55,
...
class VastuKitchenPujaScreen extends StatelessWidget {
  const VastuKitchenPujaScreen({super.key});
  @override
  Widget build(BuildContext context) {
    return CosmicBackground(
      child: Column(
        children: [
          const _ScreenHeader(title: '🔥 Kitchen & Puja'),
          Expanded(
            child: ListView(
              padding: const EdgeInsets.fromLTRB(14, 8, 14, 30),
              children: const [
                _VastuGlassCard(
                  child: Column(
                    ...
                    children: [
                      ...
                      SizedBox(height: 7),
                      Text(
                        'Traditional: দক্ষিণ-পূর্ব (SE) সবচেয়ে প্রচলিত; উত্তর-পশ্চিম (NW) অনেক নির্দেশিকায় alternative।\n\nPractical: chimney/exhaust, LPG/electrical safety, sink ও cooking clearance, fire extinguisher এবং cross-ventilation বেশি গুরুত্বপূর্ণ।',
                        style: TextStyle(
                          color: Colors.white,
                          fontSize: 12.2,
                          height: 1.5,
                ...
                _VastuGlassCard(
                  child: Column(
                    ...
                    children: [
                      ...
                      SizedBox(height: 7),
                      Text(
                        'Traditional: ঈশান (NE), পূর্ব বা উত্তর অংশকে বেশি পছন্দ করা হয়। জায়গা পরিষ্কার, শান্ত, আলো-বাতাসপূর্ণ রাখা হয়।\n\nSafety: প্রদীপ/ধূপের পাশে পর্দা, কাগজ বা দাহ্য জিনিস রাখবেন না; ventilation রাখুন।',
                        style: TextStyle(
                          color: Colors.white,
                          fontSize: 12.2,
                          height: 1.5,
...
class VastuBedroomStudyScreen extends StatelessWidget {
  const VastuBedroomStudyScreen({super.key});
  @override
  Widget build(BuildContext context) {
    return CosmicBackground(
      child: Column(
        children: [
          const _ScreenHeader(title: '🛏️ Bedroom & Study'),
          Expanded(
            child: ListView(
              padding: const EdgeInsets.fromLTRB(14, 8, 14, 30),
              children: const [
                _VastuGlassCard(
                  child: Column(
                    ...
                    children: [
                      ...
                      SizedBox(height: 7),
                      Text(
                        'Traditional Vastu-তে দক্ষিণ-পশ্চিম (SW) সবচেয়ে প্রচলিত। South/West অনেক layout-এ alternative হিসেবে ব্যবহৃত হয়।\n\nPractical: noise, privacy, natural ventilation, window shading ও emergency egress দেখুন।',
                        style: TextStyle(
                          color: Colors.white,
                          fontSize: 12.2,
                          height: 1.5,
                ...
                _VastuGlassCard(
                  child: Column(
                    ...
                    children: [
                      ...
                      SizedBox(height: 7),
                      Text(
                        'Traditional: পূর্ব, উত্তর বা উত্তর-পূর্বকে study/work-এর জন্য পছন্দ করা হয়।\n\nPractical: glare-free daylight, ergonomic table/chair, quiet zone, charging/electrical points ও ventilation নিশ্চিত করুন।',
                        style: TextStyle(
                          color: Colors.white,
                          fontSize: 12.2,
                          height: 1.5,
                ...
                _VastuGlassCard(
                  child: Column(
                    ...
                    children: [
                      ...
                      SizedBox(height: 7),
                      Text(
                        'Traditional: উত্তর-পশ্চিম (NW) guest room-এর সাথে প্রায়ই যুক্ত। বাস্তবে bathroom access, privacy ও circulation দেখে final position নিন।',
                        style: TextStyle(
                          color: Colors.white,
                          fontSize: 12.2,
                          height: 1.5,
...
class VastuUtilityScreen extends StatelessWidget {
  const VastuUtilityScreen({super.key});
  @override
  Widget build(BuildContext context) {
    const rows = [
      (
        '🚿',
        'Toilet / Bathroom',
        'বাস্তু schoolভেদে দিকের নিয়ম আলাদা। Plumbing slope, waterproofing, exhaust/ventilation, hygiene ও leakage prevention-কে priority দিন।',
      ),
      (
        '🪜',
        'Staircase',
        'প্রচলিতভাবে south/west/SW zone নিয়ে নির্দেশনা দেখা যায়। Structural calculation, headroom, handrail, riser/tread ও fire escape code অবশ্যই engineer অনুযায়ী।',
      ),
      (
        '💧',
        'Water',
        'Underground/overhead water-এর traditional zone rule একরকম নয়। বাস্তবে soil, pump, plumbing pressure, waterproofing, maintenance access ও load calculation জরুরি।',
      ),
      (
        '📦',
        'Storage',
        'ভারী storage দক্ষিণ/পশ্চিম/SW-এ রাখার প্রচলিত ধারণা আছে। কিন্তু slab load capacity ও safe access engineer অনুযায়ী নিশ্চিত করুন।',
      ),
    ];
    return CosmicBackground(
      child: Column(
        children: [
          const _ScreenHeader(title: '🚿 Utility Zones'),
          Expanded(
            child: ListView.separated(
              padding: const EdgeInsets.fromLTRB(14, 8, 14, 30),
              itemCount: rows.length,
              separatorBuilder: (_, _) => const SizedBox(height: 10),
              itemBuilder: (context, i) => _VastuGlassCard(
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    Text(
...
class VastuHomeChecklistScreen extends StatefulWidget {
  const VastuHomeChecklistScreen({super.key});
  @override
  State<VastuHomeChecklistScreen> createState() =>
      _VastuHomeChecklistScreenState();
}

class _VastuHomeChecklistScreenState extends State<VastuHomeChecklistScreen> {
  static const _storageKey = 'vastu_home_checklist_v1';
  static const _items = [
    'পূজা/ধ্যানের জায়গা শান্ত ও পরিষ্কার',
    'Kitchen-এ exhaust/ventilation আছে',
    'Main door circulation বাধাহীন',
    'Bedroom-এ privacy ও ventilation ঠিক',
    'Study area-তে পর্যাপ্ত আলো',
    'Toilet waterproofing/exhaust plan হয়েছে',
    'Staircase safety/handrail/code check হয়েছে',
    'Water tank/plumbing load engineer check করেছেন',
    'Foundation/column structural engineer approve করেছেন',
    'Local setback/building permission check হয়েছে',
  ];
  final Set<int> _done = {};
  bool _loaded = false;
  ...
  Future<void> _load() async {
    final p = await SharedPreferences.getInstance();
    final raw = p.getStringList(_storageKey) ?? const [];
    if (!mounted) return;
    setState(() {
      _done
        ..clear()
  ...
  Future<void> _toggle(int i, bool value) async {
    ...
    final p = await SharedPreferences.getInstance();
    await p.setStringList(_storageKey, _done.map((e) => '$e').toList());
  }
  ...
  @override
  Widget build(BuildContext context) {
    final pct = _items.isEmpty
        ? 0
        : ((_done.length / _items.length) * 100).round();
    return CosmicBackground(
      child: Column(
        children: [
          const _ScreenHeader(title: '✅ Home Vastu Checklist'),
          Expanded(
            child: !_loaded
                ? const Center(child: CircularProgressIndicator())
                : ListView(
                      ...
                      _VastuGlassCard(
                        child: Row(
                          children: [
                            ...
                            Expanded(
                              child: Text(
                                '${_done.length}/${_items.length} planning check complete',
                                style: const TextStyle(
                                  color: Colors.white,
                                  fontSize: 14,
                                  fontWeight: FontWeight.w800,
...
class SuperServicesScreen extends StatelessWidget {
  ...
  void _open(BuildContext context, String title) {
    ...
    void openSheet(String sheetTitle) {
      showModalBottomSheet(
        context: context,
        backgroundColor: Colors.transparent,
        isScrollControlled: true,
        builder: (_) => _FeatureSheet(title: sheetTitle),
      );
    }

    switch (title) {
      case 'বাংলা ক্যালেন্ডার':
        pushPage(const BengaliCalendarScreen());
        return;

      case 'আজকের পঞ্জিকা':
        pushPage(const TodaySmartScreen());
        return;
      ...
      case 'পূর্ণিমা':
        showModalBottomSheet(
          ...
          backgroundColor: Colors.transparent,
          isScrollControlled: true,
          builder: (_) => const _MoonPhaseSheet(initialIsPurnima: true),
        );
        return;
      ...
      case 'অমাবস্যা':
        showModalBottomSheet(
          ...
          backgroundColor: Colors.transparent,
          isScrollControlled: true,
          builder: (_) => const _MoonPhaseSheet(initialIsPurnima: false),
        );
        return;

      case 'উৎসব':
        openSheet('উৎসব ও বিশেষ দিন');
        return;
      ...
      case 'জমি কেনা':
      case 'ব্যবসা শুরু':
        pushPage(const ShubhoMuhurtaScreen());
        return;

      case 'Family Calendar':
      case 'বাংলা জন্মদিন':
        pushPage(const FamilyCalendarScreen());
        return;

      case 'Reminder':
      case 'বাংলা Voice':
      case 'Festival Alert':
        pushPage(const ReminderScreen());
        return;

      case 'Favourite Day':
        pushPage(const FavouriteDayScreen());
        return;

      case 'Date Converter':
        pushPage(const DateConverterScreen());
        return;

      case 'আজ কী ভালো':
      case 'Panchang Timeline':
        pushPage(const TodaySmartScreen());
        return;

      case 'Month Summary':
        pushPage(const MonthSummaryScreen());
        return;

      case 'Puja Checklist':
        pushPage(const PujaChecklistScreen());
        return;

      case 'জপ কাউন্টার':
        pushPage(const JapaCounterScreen());
        return;

      case 'উপবাস তথ্য':
        pushPage(const UpobasInfoScreen());
        return;

      case 'তিথির অর্থ':
        pushPage(const TithiGuideScreen());
        return;

      case 'আজকের ইতিহাস':
        pushPage(const TodayHistoryScreen());
        return;

      case 'Home Widget':
        pushPage(const HomeWidgetInfoScreen());
        return;

      case 'Panjika Voice':
        pushPage(const PanjikaVoiceAssistantScreen());
        return;

      case 'Morning Brief':
        pushPage(const MorningBriefScreen());
        return;

      case 'Smart Agenda':
        pushPage(const SmartAgendaScreen());
        return;

      case 'Family Board':
        pushPage(const FamilyUpcomingBoardScreen());
        return;

      case 'Panjika Diary':
        pushPage(const PanjikaDiaryScreen());
        return;

      case 'Live Muhurta':
        pushPage(const LiveMuhurtaRadarScreen());
        return;

      case 'Best Day Finder':
        pushPage(const BestDayFinderScreen());
        return;

      case 'Day Compare':
        pushPage(const PanchangCompareScreen());
        return;

      case 'Festival Hub':
        pushPage(const FestivalHubScreen());
        return;

      case 'Smart Digest':
        pushPage(const SmartNotificationDigestScreen());
        return;

      case 'Shubho Alarm':
        pushPage(const ShubhoAlarmScreen());
        return;

      case 'Sadhana Streak':
        pushPage(const SadhanaStreakScreen());
        return;

      case 'Family Assistant':
        pushPage(const FamilyOccasionAssistantScreen());
        return;

      case 'Festival Live':
        pushPage(const FestivalLiveModeScreen());
        return;

      case 'Reminder Center':
        pushPage(const SmartReminderCenterScreen());
        return;

      case 'Daily Routine':
        pushPage(const DailyRoutineScreen());
        return;

      case 'Year Map':
        pushPage(const MyYearMapScreen());
        return;

      case 'Time Machine':
        pushPage(const PanchangTimeMachineScreen());
        return;

      case '24H Timeline':
        pushPage(const Next24HoursPanchangScreen());
        return;

      case 'Life Planner':
        pushPage(const BengaliLifePlannerScreen());
        return;

      case 'Travel Planner':
        pushPage(const TravelPanchangPlannerScreen());
        return;

      case 'গৃহ ও বাস্তু':
      case 'Vastu Hub':
        pushPage(const VastuHomeHubScreen());
        return;

      case 'দিক ও ব্রহ্মস্থান':
        pushPage(const VastuDirectionGuideScreen());
        return;

      case 'Room Planner Check':
        pushPage(const VastuRoomPlannerScreen());
        return;

      case 'Main Door & Plot':
        pushPage(const VastuEntranceScreen());
        return;

      case 'Kitchen & Puja':
        pushPage(const VastuKitchenPujaScreen());
        return;

      case 'Bedroom & Study':
        pushPage(const VastuBedroomStudyScreen());
        return;

      case 'Utility Zones':
        pushPage(const VastuUtilityScreen());
        return;

      case 'Home Checklist':
        pushPage(const VastuHomeChecklistScreen());
        return;

      case 'Super 30 Features':
        pushPage(const Super30FeaturesScreen());
        return;

      case 'World Clock':
        pushPage(const WorldClockScreen());
        return;

      case 'আমার Notes':
        pushPage(const NotesScreen());
        return;
      ...
      case 'Puja Planner':
        pushPage(const TempleScreen());
        return;

      case 'Festival Countdown':
        pushPage(const FestivalCountdownScreen());
        return;

      case 'Daily Share Card':
        pushPage(const PanchangShareCardScreen());
        return;

      case 'Festival Poster':
        pushPage(const FestivalPosterScreen());
        return;

      case 'Backup':
        pushPage(const CloudBackupScreen());
        return;
      ...
      default:
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
              '$title — এই নতুন feature-এর full working screen এখন তৈরি করা হবে',
            ),
            duration: const Duration(seconds: 2),
          ),
        );
  ...
  Widget build(BuildContext context) {
    ...
    const smartDay = [
      _SuperServiceData('📡', 'Live Muhurta'),
      _SuperServiceData('🎯', 'Best Day Finder'),
      _SuperServiceData('⭐', 'শুভ মুহূর্ত'),
      _SuperServiceData('⚖️', 'Day Compare'),
      _SuperServiceData('🔎', 'Smart Search'),
      _SuperServiceData('🔄', 'Date Converter'),
    ];

    const festival = [
      _SuperServiceData('🪔', 'Festival Live'),
    ...
    const personal = [
      _SuperServiceData('🎙️', 'Panjika Voice'),
      _SuperServiceData('🌅', 'Morning Brief'),
      _SuperServiceData('🗓️', 'Smart Agenda'),
      _SuperServiceData('👨‍👩‍👧', 'Family Calendar'),
      _SuperServiceData('📓', 'Panjika Diary'),
      _SuperServiceData('❤️', 'Favourite Day'),
    ];

    const automation = [
      _SuperServiceData('🔔', 'Reminder Center'),
      _SuperServiceData('📨', 'Smart Digest'),
      _SuperServiceData('⏰', 'Shubho Alarm'),
      _SuperServiceData('🌿', 'Daily Routine'),
      _SuperServiceData('🔥', 'Sadhana Streak'),
      _SuperServiceData('🎁', 'Family Assistant'),
    ];

    const insight = [
      _SuperServiceData('🗺️', 'Year Map'),
      _SuperServiceData('👨‍👩‍👧', 'Family Board'),
      _SuperServiceData('📚', 'আজকের ইতিহাস'),
      _SuperServiceData('🕒', 'World Clock'),
      _SuperServiceData('📱', 'Home Widget'),
      _SuperServiceData('📝', 'আমার Notes'),
    ];

    const nextGen = [
      _SuperServiceData('💎', 'Super 30 Features'),
      _SuperServiceData('🕰️', 'Time Machine'),
      _SuperServiceData('⏱️', '24H Timeline'),
      _SuperServiceData('🗓️', 'Life Planner'),
      _SuperServiceData('🧭', 'Travel Planner'),
    ];

    const vastu = [
      _SuperServiceData('🏠', 'গৃহ ও বাস্তু'),
      _SuperServiceData('🧭', 'দিক ও ব্রহ্মস্থান'),
      _SuperServiceData('📐', 'Room Planner Check'),
      _SuperServiceData('🚪', 'Main Door & Plot'),
      _SuperServiceData('🔥', 'Kitchen & Puja'),
      _SuperServiceData('✅', 'Home Checklist'),
    ];
      ...
      child: Scaffold(
        ...
        body: ListView(
          ...
          children: [
            Container(
              ...
              child: const Row(
                children: [
                  ...
                  Expanded(
                    child: Text(
                      'পঞ্জিকা • Smart Intelligence • Personal Planner • Automation — সব এক জায়গায়',
                      style: TextStyle(
                        color: Colors.white,
                        fontSize: 14,
                        height: 1.3,
            ...
            _section(context, '📅 পঞ্জিকা', panjika),
            _section(context, '🎯 Smart Day Tools', smartDay),
            _section(context, '🪔 পূজা ও Festival', festival),
            _section(context, '🧠 Personal Intelligence', personal),
            _section(context, '⚡ Reminder & Routine', automation),
            _section(context, '🗺️ Life Overview', insight),
            _section(context, '🚀 Next-Gen Tools', nextGen),
            _section(context, '🏠 গৃহ ও বাস্তু', vastu),
            _section(context, '🎨 Create & Settings', create),
          ],
        ),
      ),
    );
...
class _BengaliCalendarScreenState extends State<BengaliCalendarScreen> {
  ...
  Widget _sidebarPanel(BuildContext context, BengaliMonthInfo info) {
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Text(
                      value,
                      style: TextStyle(
                        color: warn ? const Color(0xFFFF8A80) : Colors.white,
                        fontSize: 11.5,
                        fontWeight: FontWeight.w700,
                        height: 1.15,
                      ),
    ...
    return Container(
      padding: const EdgeInsets.all(12),
      decoration: BoxDecoration(
        color: const Color(0xFF07182E).withValues(alpha: 0.58),
        borderRadius: BorderRadius.circular(18),
        border: Border.all(color: Colors.white.withValues(alpha: 0.14)),
      ),
      child: Column(
        ...
        children: [
          ...
          Divider(height: 16, color: Colors.white.withValues(alpha: 0.10)),
          // ---- সময়গুলো ২ কলামে ----
          Row(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
  ...
  Widget build(BuildContext context) {
    ...
    // Calendar V2: live Cosmic background + readable glass calendar.
    // Date cells remain high-contrast, but the page itself stays transparent.
    return CosmicBackground(
      child: Material(
        ...
        child: Column(
          children: [
            ...
            Container(
              ...
              decoration: BoxDecoration(
                color: const Color(0xFF06152B).withValues(alpha: 0.72),
                border: Border(
                  bottom: BorderSide(
                    color: const Color(0xFFFFD36E).withValues(alpha: 0.35),
                  ),
...
class _MonthlyPanjikaSliderState extends State<_MonthlyPanjikaSlider> {
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        borderRadius: BorderRadius.circular(16),
        color: const Color(0xFF2B0E45).withValues(alpha: 0.68),
        border: Border.all(
...
class _CalendarDayCellState extends State<_CalendarDayCell> {
  ...
  Widget build(BuildContext context) {
    ...
    final numberColor =
        categoryColor ??
        (widget.isSunday ? const Color(0xFFFF8A80) : Colors.white);
        ...
        child: AnimatedContainer(
          ...
          decoration: BoxDecoration(
            color: widget.highlightTint
                ? const Color(0xFF6A2D72).withValues(alpha: 0.42)
                : widget.isToday
                ? const Color(0xFFFFD36E).withValues(alpha: 0.18)
                : const Color(0xFF07182E).withValues(alpha: 0.62),
