  ...
  Widget _quickIconRow(BuildContext context) {
    ...
    return SizedBox(
      ...
      child: ListView(
        scrollDirection: Axis.horizontal,
        children: [
          item('📅', 'আজ + কাল', () => Navigator.pop(context)),
          item(
            '🔯',
            'শুভ মুহূর্ত',
            () => Navigator.push(
          ...
          item(
            ...
            () => showModalBottomSheet(
              ...
              isScrollControlled: true,
              builder: (_) =>
                  const _FeatureSheet(title: 'উৎসব ও বিশেষ দিন'),
            ),
          ),
          item(
            '♈',
          ...
          item('⛅', 'ওয়েজার', () => _showWeatherQuick(context)),
          item(
            '⚙️',
            'সেটিংস',
            () => Navigator.push(
...
class _CalendarDayCellState extends State<_CalendarDayCell> {
  ...
  void didUpdateWidget(covariant _CalendarDayCell oldWidget) {
    ...
    // মাস পাল্টালে গ্রিডের একই ঘরে (স্লট) সম্পূর্ণ ভিন্ন তারিখ চলে আসে —
    // তখন সাইকেল নতুন করে শুরু করা দরকার
  ...
  void _maybeStartCycle() {
    ...
    if (widget.events.length > 1) {
      // ভিন্ন ভিন্ন ঘর ভিন্ন ভিন্ন সময়ে বদলাবে — একসাথে সব ঘর ঝলক দিলে
      // অস্বাভাবিক দেখাবে, তাই bengaliDay দিয়ে ছোট এলোমেলো বিলম্ব
      final startDelay = Duration(
        milliseconds: (widget.bengaliDay * 733) % 1400,
      );
      Future.delayed(startDelay, () {
        ...
        _timer = Timer.periodic(const Duration(seconds: 3), (_) {
          if (!mounted) return;
          setState(() {
            _cycleIndex = (_cycleIndex + 1) % widget.events.length;
          });
        });
      });
    }
  ...
  Widget build(BuildContext context) {
    ...
    final currentEvent = events.isNotEmpty
        ? events[_cycleIndex % events.length]
        : null;
    ...
    final isSpecialTithi =
        currentEvent != null &&
        (currentEvent.category == 'purnima' ||
            currentEvent.category == 'amabasya' ||
            currentEvent.category == 'ekadashi');
    final iconToShow = currentEvent?.icon ?? (widget.isSankranti ? '🔔' : null);
    // বিশেষ দিন/উৎসবের জন্য অ্যাপের নিজস্ব আঁকা illustration (PanjikaArt) —
    // থাকলে ইমোজির বদলে সেটাই দেখানো হয়
    final motif = currentEvent != null
        ? PanjikaArt.motifFor(
            currentEvent.label,
            currentEvent.category,
            sankranti: widget.isSankranti,
          )
        : (widget.isSankranti ? 'sun' : null);
    // রবিবার লাল, বাকি বার নেভি — রেফারেন্স ছাপা ক্যালেন্ডারের মতো
    final numberColor = widget.isSunday
        ? const Color(0xFFC62828)
        : const Color(0xFF16215C);
      ...
      child: Opacity(
        ...
        child: Container(
          ...
          child: Column(
            ...
            mainAxisSize: MainAxisSize.min,
            children: [
              // সংক্রান্তির দিনে বাংলা মাসের নাম — ছোট ক্যাপশন, উপরে
              if (widget.secondaryDateText.isNotEmpty)
                Text(
                  widget.secondaryDateText,
                  maxLines: 1,
                  overflow: TextOverflow.ellipsis,
                  style: const TextStyle(fontSize: 7, color: Colors.black45),
                ),
              // বাংলা তারিখ — প্রধান, বড় (পুরো বাংলা ক্যালেন্ডার, তাই বাংলা
              // সংখ্যাই বড়); আজকের দিন হলে সোনালি বৃত্তে হাইলাইট; বিশেষ
              // তিথি (পূর্ণিমা/অমাবস্যা/একাদশী) হলে রঙিন বৃত্তরেখা
              AnimatedSwitcher(
                ...
                child: Container(
                  ...
                  decoration: widget.isToday
                      ? const BoxDecoration(
                          shape: BoxShape.circle,
                          color: Color(0xFFF7BD53),
                        )
                      : (isSpecialTithi
                            ? BoxDecoration(
                                shape: BoxShape.circle,
                                border: Border.all(
                                  color: catColor ?? numberColor,
                                  width: 1.3,
                                ),
                              )
                            : null),
                  child: Text(
                    ...
                    style: TextStyle(
                      ...
                      color: widget.isToday
                          ...
                          : (catColor ?? numberColor),
                    ),
                  ),
                ),
              ),
              // ইংরেজি তারিখ — ছোট, secondary (নিচে)
              Text(
                widget.greg.day.toString(),
                style: const TextStyle(
                  fontSize: 8.5,
              ...
              if (isSpecialTithi)
                // বিশেষ তিথির দিনে — অ্যাপের নিজস্ব ছোট illustration
                // (চাঁদ/তুলসী ইত্যাদি), রেফারেন্স ছাপা পঞ্জিকার মতো
                AnimatedSwitcher(
                  duration: const Duration(milliseconds: 550),
                  switchInCurve: Curves.easeOut,
                  switchOutCurve: Curves.easeIn,
                ...
                // উৎসবের দিনে — অ্যাপের নিজস্ব আঁকা illustration
                AnimatedSwitcher(
                  duration: const Duration(milliseconds: 550),
                  switchInCurve: Curves.easeOut,
                  switchOutCurve: Curves.easeIn,
                ...
                // সাধারণ দিনে — সেই দিনের তিথির নাম (আসল বাংলা পঞ্জিকার মতো)
                Text(
                  widget.tithiName,
                  maxLines: 1,
                  overflow: TextOverflow.ellipsis,
