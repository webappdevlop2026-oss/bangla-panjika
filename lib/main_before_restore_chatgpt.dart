...
class AppSettings extends ChangeNotifier {
  ...
  Future<void> load() async {
    ...
    profileDob = dobMs != null ? DateTime.fromMillisecondsSinceEpoch(dobMs) : null;
...
class PaymentService {
  ...
  Future<Map<String, dynamic>> createOrder(String plan) async {
    ...
    final data = jsonDecode(res.body) as Map<String, dynamic>;
    if (res.statusCode != 200 || data['success'] != true) {
      throw Exception(data['message']?.toString() ?? 'অর্ডার তৈরি ব্যর্থ হয়েছে');
    }
    return data;
  }
...
class _CosmicBackgroundState extends State<CosmicBackground>
  ...
  static const Map<String, double> _planetRingDiameter = {
    'mercury': 66, 'venus': 88, 'mars': 112, 'jupiter': 136,
    'saturn': 160, 'uranus': 184, 'neptune': 208,
  };
  ...
  void _refreshPhase() {
    if (!mounted) return;
    final now = DateTime.now();
    final phase = SkyPhase.forTime(now, lat: AppLocation.lat, lon: AppLocation.lon);
  ...
  Widget _buildPlanetsLayer() {
    return AnimatedBuilder(
      ...
      builder: (context, _) {
        final realElapsedMs =
            DateTime.now().difference(_planetSimAnchor).inMilliseconds;
        final virtualTime = _planetSimAnchor.add(
          Duration(
            milliseconds: (realElapsedMs * _planetTimeAcceleration).round(),
          ),
        );
        final sunAlt = PanchangCalculator.celestialAltAz(
          'sun', virtualTime, lat: AppLocation.lat, lon: AppLocation.lon,
        )['altitude'] ?? 0.0;
  ...
  Widget _buildPlanetDot(_PlanetVisual pv, DateTime virtualTime, double duskFactor) {
    final altAz = PanchangCalculator.celestialAltAz(
      pv.key, virtualTime, lat: AppLocation.lat, lon: AppLocation.lon,
    );
...
class _SkyEventsLayerState extends State<_SkyEventsLayer>
  ...
  void _fireMeteor() {
    ...
    final angle = (_rnd.nextDouble() * 0.5 + 0.2) * math.pi; // তির্যক নিচের দিকে
    const len = 0.28;
    setState(() {
    ...
    if (meteorT > 0.0 && meteorT < 1.0) {
      ...
      final streak = Paint()
        ..shader = ui.Gradient.linear(
          p2,
          p1,
          [meteorColor.withValues(alpha: 0.0), meteorColor.withValues(alpha: fadeAlpha)],
        )
        ..strokeWidth = 2.0
        ..strokeCap = StrokeCap.round;
      canvas.drawLine(p2, p1, streak);
      canvas.drawCircle(p1, 1.6, Paint()..color = meteorColor.withValues(alpha: fadeAlpha));
    }
...
class _ConstellationsPainter extends CustomPainter {
  ...
  static const List<List<Offset>> _patterns = [
    ...
    [
      Offset(0.12, 0.14), Offset(0.17, 0.12), Offset(0.22, 0.135),
      Offset(0.27, 0.155), Offset(0.27, 0.19), Offset(0.22, 0.20),
      Offset(0.19, 0.175),
    ],
    // কালপুরুষ (Orion) — সরলীকৃত বেল্ট ও কাঁধ/পা
    [
      Offset(0.68, 0.10), Offset(0.80, 0.09), Offset(0.72, 0.155),
      Offset(0.76, 0.155), Offset(0.80, 0.155), Offset(0.70, 0.22),
      Offset(0.82, 0.22),
    ],
  ];
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (final pattern in _patterns) {
      for (int i = 0; i < pattern.length; i++) {
        final p = Offset(pattern[i].dx * size.width, pattern[i].dy * size.height);
...
class _HeroDateCardState extends State<_HeroDateCard> {
  ...
  void initState() {
    super.initState();
    // আজকের তাপমাত্রা — বিদ্যমান WeatherService থেকেই, নতুন কোনো API নয়
    WeatherService.instance.addListener(_onWeather);
    WeatherService.instance.refresh();
  }

  @override
  void dispose() {
    WeatherService.instance.removeListener(_onWeather);
    super.dispose();
  }
  ...
  /// হেডারে আজকের তাপমাত্রা — ডেটা আসার আগে জায়গা দখল করে না
  Widget _tempChip() {
    ...
    return Padding(
      ...
      child: Container(
        ...
        child: Row(
          mainAxisSize: MainAxisSize.min,
          children: [
            // সিস্টেমের ইমোজির বদলে অ্যাপের নিজের আঁকা সূর্য (PanjikaArt) —
            // গেরুয়া রঙে, ছাপা পঞ্জিকার ধাঁচে। প্রতিটা ফোনে একই রকম
            // দেখাবে, ইমোজির মতো ফোনভেদে বদলাবে না।
            raining
                ? const Icon(
  ...
  void _shareToday() {
    Share.share(buildTodayPanchangSummary(), subject: 'আজকের পঞ্চাঙ্গ — বাংলা পঞ্জিকা');
  }
...
class _UpcomingFestivalsCard extends StatelessWidget {
  ...
  void _shareFestival(String icon, String title, String date, String countdown) {
    Share.share(
  ...
  Widget build(BuildContext context) {
    ...
    final boxDecoration = BoxDecoration(
      ...
      borderRadius: BorderRadius.circular(18),
      border: Border.all(
        color: const Color(0xFFF3B35D).withValues(alpha: 0.4),
      ),
    );
...
const List<_WorldCity> _worldCities = [
  ...
  _WorldCity('যুক্তরাষ্ট্র (লস অ্যাঞ্জেলেস)', '🇺🇸', 'America/Los_Angeles', -8.0),
                  ...
                  child: Row(
                    children: [
                      SizedBox(
                        ...
                        height: 68,
                        child: CustomPaint(
                          painter: _AnalogClockPainter(local),
                        ),
                      ),
                      const SizedBox(width: 12),
                      Expanded(
            ...
            children: [
              ...
              for (int i = 0; i < 12; i++)
                Transform.translate(
                  offset: Offset(
                    labelRadius *
                        math.cos(baseAngle + i * 30 * math.pi / 180),
                    labelRadius *
                        math.sin(baseAngle + i * 30 * math.pi / 180),
                  ),
                  child: Column(
                    mainAxisSize: MainAxisSize.min,
                    children: [
...
class KundliMilanCalculator {
  ...
  static KundliMilanResult compute({
    ...
    koots.add(GunaKoot('বশ্য', vashyaScore, 2, 'পারস্পরিক আকর্ষণ ও নিয়ন্ত্রণ'));
                  ...
                  (a) => InkWell(
                    ...
                    child: Container(
                      ...
                      decoration: BoxDecoration(
                        ...
                        borderRadius: BorderRadius.circular(14),
                        border: Border.all(
                          color: const Color(0xFFFFD36E).withValues(
                            alpha: _selected == a ? 0.6 : 0.2,
                          ),
                        ),
                      ),
                      child: Row(
...
class ContentData {
  ...
  static final List<_EclipseEvent> _eclipseEvents = [
    _EclipseEvent(
      ...
      indiaVisibility: 'ভারত থেকে দেখা যাবে না (দক্ষিণ গোলার্ধ/আন্টার্কটিকা থেকে দেখা যাবে)',
    ),
    _EclipseEvent(
      DateTime(2026, 3, 3),
      '🌕',
  ...
  static List<List<String>> _liveUpcomingFestivals() {
    ...
    if (festivals.isEmpty) {
      return [['🎉 উৎসব', 'আগামী কিছুদিনে তথ্য পাওয়া যাচ্ছে না']];
    }
    return festivals
  ...
  static List<List<String>> _livePanjika() {
    ...
    return [
      ...
      ['করণ', karana],
      ['রাহুকাল (এড়িয়ে চলুন)', '${bnTime12(rahu['start']!)}–${bnTime12(rahu['end']!)}'],
      [
        'অভিজিৎ মুহূর্ত (শুভ)',
...
class _TrialSignupFormState extends State<_TrialSignupForm> {
  ...
  Future<void> _submit() async {
    ...
    if (name.isEmpty || address.isEmpty || _dob == null) {
      setState(() => _error = 'নাম, জন্ম তারিখ-সময় ও ঠিকানা — সবগুলো দিতে হবে');
      return;
    }
...
class VoiceReminderParser {
  ...
  static VoiceReminderParseResult parse(String rawSpoken) {
    ...
    if (dateFound || timeFound) {
      ...
      final h = hour ?? 9; // সময় না বললে ডিফল্ট সকাল ৯টা — প্রিভিউতে বদলানো যায়
...
class _VillageHorizonSceneState extends State<VillageHorizonScene>
  ...
  void _recomputeSun() {
    ...
    final sky = SkyPhase.forTime(now, lat: AppLocation.lat, lon: AppLocation.lon);
    if (mounted) {
      setState(() {
        _sunFrac = frac;
...
class _VillageHorizonPainter extends CustomPainter {
  ...
  final double darkAmount; // 0..1 — কতটা অন্ধকার (street light/window আলো নিয়ন্ত্রণ করে)
  ...
  void paint(Canvas canvas, Size size) {
    ...
    void hut(double x, double baseY, double s, {double glow = 0.0}) {
      ...
      if (glow > 0.02) {
        ...
        canvas.drawRect(
          windowRect,
          Paint()..color = const Color(0xFFFFCF6B).withValues(
            alpha: glow.clamp(0.0, 1.0) * 0.85,
          ),
        );
      }
    }
    ...
    void grazingAnimal(
      ...
      for (final dx in [-bodyW * 0.32, -bodyW * 0.1, bodyW * 0.1, bodyW * 0.32]) {
        canvas.drawLine(
          Offset(x + dx, baseY - 1),
      ...
      canvas.drawCircle(
        ...
        Paint()..color = treePaint.color.withValues(alpha: dayAmount.clamp(0.0, 1.0)),
      );
    }
...
class PanchangCalculator {
  ...
  static const Map<String, List<double>> _planetElements = {
    'mercury': [
      0.38709927, 0.00000037, 0.20563593, 0.00001906, 7.00497902,
      -0.00594749, 252.25032350, 149472.67411175, 77.45779628, 0.16047689,
    ...
    'uranus': [
      19.18916464, -0.00196176, 0.04725744, -0.00004397, 0.77263783,
      -0.00242939, 313.23810451, 428.48202785, 170.95427630, 0.40805281,
      74.01692503, 0.04240589,
    ],
    'neptune': [
      30.06992276, 0.00026291, 0.00859048, 0.00005105, 1.77004347,
      0.00035372, -55.12002969, 218.45945325, 44.96476227, -0.32241464,
  ...
  static Map<String, double> _eclipticToEquatorial(double lonDeg, double latDeg) {
  ...
  static Map<String, double> _equatorialToHorizontal(
    ...
    final sinAlt =
        math.sin(decR) * math.sin(latR) + math.cos(decR) * math.cos(latR) * math.cos(ha);
    final alt = math.asin(sinAlt.clamp(-1.0, 1.0));
    final cosAltLat = math.cos(alt) * math.cos(latR);
    final cosAz = cosAltLat.abs() < 1e-9
        ? 0.0
        : ((math.sin(decR) - math.sin(alt) * math.sin(latR)) / cosAltLat)
            .clamp(-1.0, 1.0);
    ...
    DateTime searchEdge({required bool forward}) {
      ...
      for (int i = 0; i < 20; i++) {
        final mid = DateTime.fromMillisecondsSinceEpoch(
          (known.millisecondsSinceEpoch + unknown.millisecondsSinceEpoch) ~/
              2,
        );
...
class PanjikaArt extends StatelessWidget {
  ...
  static String? motifFor(String label, String category, {bool sankranti = false}) {
    switch (category) {
      case 'purnima':
        return 'moonFull';
      case 'amabasya':
    ...
    if (has(['দুর্গা', 'ষষ্ঠী', 'সপ্তমী', 'অষ্টমী', 'নবমী', 'দশমী', 'মহালয়া'])) {
      return 'trishul';
    }
    ...
    switch (motif) {
      ...
      case 'temple':
        ...
        final door = Path()
          ...
          ..arcToPoint(Offset(s * 0.57, s * 0.68), radius: Radius.circular(s * 0.07))
      ...
      case 'peacock':
        ...
        canvas.drawOval(
          Rect.fromCenter(center: p(0.5, 0.28), width: s * 0.36, height: s * 0.5),
          fill,
        );
        fill.color = _peaGreen;
        canvas.drawOval(
          Rect.fromCenter(center: p(0.5, 0.30), width: s * 0.24, height: s * 0.34),
          fill,
        );
        fill.color = _gold;
      ...
      case 'flag':
        ...
        fill.color = _saffron;
        canvas.drawRect(Rect.fromLTWH(s * 0.24, s * 0.20, s * 0.54, s * 0.12), fill);
        fill.color = Colors.white;
        canvas.drawRect(Rect.fromLTWH(s * 0.24, s * 0.32, s * 0.54, s * 0.12), fill);
        fill.color = _green;
        canvas.drawRect(Rect.fromLTWH(s * 0.24, s * 0.44, s * 0.54, s * 0.12), fill);
        stroke
          ..color = _navy
          ..strokeWidth = s * 0.02;
      ...
      case 'tulsi':
        ...
        canvas.drawRect(Rect.fromLTWH(s * 0.30, s * 0.52, s * 0.40, s * 0.06), fill);
        // গাছ
        stroke
          ..color = _green
          ..strokeWidth = s * 0.035;
...
class _BengaliCalendarScreenState extends State<BengaliCalendarScreen> {
  ...
  // নতুন বেগুনি রেফারেন্সের মতো সপ্তাহের পুরো নাম (আগে সংক্ষিপ্ত ছিল)
  ...
  static const Color _amabasyaColor = Color(0xFFA285E8); // গাঢ় বেগুনি — অমাবস্যা
  static const Color _ekadashiColor = Color(0xFFFF9A3E); // জাফরান — একাদশী
  static const Color _sankrantiColor = Color(0xFF2FBFA3); // টিল — সংক্রান্তি (মাস শুরু)
  static const Color _festivalColor = Color(0xFFE0384A); // লাল — উৎসব/শুভ দিন/রবিবার

  // ---- পরিষ্কার থিম: গাঢ় নীল + সোনালি, বাকি সব নিরপেক্ষ ধূসর ----
  // মাত্র দুটো প্রধান রং, তাই স্ক্রিন ঝকঝকে থাকে; বিশেষ তিথির রংগুলো
  // শুধু ছোট বিন্দু ও নরম ছোপে ব্যবহার হয়, বড় জায়গা দখল করে না।
  static const Color _purpleAccent = Color(0xFF4A3494); // অ্যাকশন/লিংকের নীল
  static const Color _purpleDeep = Color(0xFF221D3D); // প্রধান গাঢ় টেক্সট
  static const Color _lavenderBg = Color(0xFFEDEBF3); // স্ক্রিনের ব্যাকগ্রাউন্ড
  static const Color _lavenderBorder = Color(0xFFE7E4F0); // কার্ড/সেলের বর্ডার
  static const Color _muted = Color(0xFF7C7695); // সেকেন্ডারি ধূসর টেক্সট
  static const Color _sundayRed = Color(0xFFC23B48); // রবিবার/উৎসবের লাল

  static const Color _surface2 = Color(0xFFF5F3FA); // হালকা ভরাট (বোতাম/ট্যাগ)
  static const Color _inkSoft = Color(0xFF4A4463); // মাঝারি টেক্সট
  static const Color _gold = Color(0xFFC08A1E); // সোনালি — শকাব্দ/পূর্ণিমা
  static const Color _goldSoft = Color(0xFFFDF6E1);
  static const Color _goldLine = Color(0xFFEFDFB4);
  static const Color _vermSoft = Color(0xFFFDF0F1);
  static const Color _vermLine = Color(0xFFF3D6D9);
  static const Color _leaf = Color(0xFF2F8A55); // সবুজ — একাদশী
  static const Color _leafSoft = Color(0xFFEFF9F2);
  static const Color _leafLine = Color(0xFFCDE9D8);
  static const Color _night = Color(0xFF332C4E); // গাঢ় — অমাবস্যা

  /// "১৬ Aug" ধাঁচের ছোট ইংরেজি তারিখ-লেবেল (রেফারেন্সের মতো)
  static String engDayLabel(DateTime d) {
    const m = [
      'Jan',
      'Feb',
      'Mar',
  ...
  /// চলতি মাসের বাইরের (আগের/পরের মাসের) দিনের ঘর — ধূসর রঙে, তবে
  /// রেফারেন্সের মতোই বাংলা তারিখ, ইংরেজি তারিখ ও তিথি তিনটেই দেখায়
  Widget _outsideDayCell(String bnDay, DateTime greg) {
    final tithi = PanchangCalculator.tithiFor(
      PanchangCalculator.sunTimes(
        greg,
        lat: AppLocation.lat,
        lon: AppLocation.lon,
      ).sunrise,
    ).name;
    const faded = Color(0xFFB9B6C6);
    return Container(
      alignment: Alignment.center,
      decoration: BoxDecoration(
        color: _surface2,
  ...
  /// মাস আগে-পরে করার গোলাকার হালকা-বেগুনি বোতাম
  Widget _roundNavButton(IconData icon, String tip, VoidCallback onTap) {
    return Tooltip(
      message: tip,
      child: InkWell(
        onTap: onTap,
        ...
        child: Container(
          ...
          child: Icon(icon, size: 24, color: _purpleAccent),
        ),
      ),
    );
  }
  ...
  /// নিচের নির্দেশিকা সারির একটা আইটেম — রঙিন বিন্দু + নাম
  Widget _legendDot(Color dot, String label) {
    return Row(
      mainAxisSize: MainAxisSize.min,
      children: [
        Container(
  ...
  /// ---- "এই মাসের উৎসব" তালিকা ----
  /// ফোনের ঘর মাত্র ~৪৭px চওড়া, তাই উৎসবের নাম ঘরে ঠেসে দিলে পড়াই যায় না।
  /// এখানে প্রতিটা উৎসব পুরো চওড়ায় — তারিখ-ট্যাগ, নাম, তিথি-নক্ষত্র ও
  /// অ্যাপের নিজস্ব আঁকা ছবি — স্পষ্ট দেখানো হয়। কোনো তথ্য বাদ যায় না।
  Widget _festivalSection(BuildContext context, BengaliMonthInfo info) {
    final total = info.end.difference(info.start).inDays + 1;
    final rows = <Widget>[];

    for (var i = 0; i < total; i++) {
      final greg = info.start.add(Duration(days: i));
      final events = BengaliCalendarData.eventsFor(greg);
      if (events.isEmpty) continue;
      final e = events.first;

      // তারিখ-ট্যাগের রং — গ্রিডের ঘরের বিন্দুর সাথে মিলিয়ে
      Color tagBg = _surface2;
      Color tagLine = _lavenderBorder;
      Color tagInk = _purpleDeep;
      if (e.category == 'purnima') {
        tagBg = _goldSoft;
        tagLine = _goldLine;
        tagInk = _gold;
      } else if (e.category == 'ekadashi') {
        tagBg = _leafSoft;
        tagLine = _leafLine;
        tagInk = _leaf;
      } else if (e.category != 'amabasya') {
        tagBg = _vermSoft;
        tagLine = _vermLine;
        tagInk = _sundayRed;
      }
      ...
      final nak = PanchangCalculator
          .nakshatraNames[PanchangCalculator.nakshatraIndexFor(sunrise)];
      final motif = PanjikaArt.motifFor(e.label, e.category);
      // একই দিনে একাধিক উৎসব থাকলে সবগুলোর নামই দেখানো হয়
      final title = events.map((x) => x.label).join(' • ');
      final isFirst = rows.isEmpty;

      rows.add(
        InkWell(
          onTap: () => setState(() => _selectedKey = _key(greg)),
          child: Container(
            padding: const EdgeInsets.symmetric(horizontal: 14, vertical: 13),
            decoration: BoxDecoration(
              border: isFirst
                  ? null
                  : const Border(top: BorderSide(color: _lavenderBorder)),
            ),
            child: Row(
              children: [
                Container(
                  ...
                  decoration: BoxDecoration(
                    ...
                    border: Border.all(color: tagLine),
                  ),
                  child: Column(
                    children: [
                      Text(
                ...
                const SizedBox(width: 10),
                if (motif != null)
                  PanjikaArt(motif, size: 24)
                else
                  Text(e.icon, style: const TextStyle(fontSize: 19)),
              ],
            ),
          ),
        ),
  ...
  Widget _sidebarPanel(BuildContext context, BengaliMonthInfo info) {
    ...
    // একটা তথ্য-জোড়া: আইকন, ছোট ধূসর লেবেল, নিচে গাঢ় বোল্ড মান
    Widget kv(String icon, String label, String value, {bool caution = false}) =>
        Row(
          children: [
            SizedBox(
              width: 20,
                ...
                children: [
                  ...
                  FittedBox(
                    ...
                    child: Text(
                      ...
                      maxLines: 1,
                      style: TextStyle(
                        color: caution ? _sundayRed : _purpleDeep,
                        fontSize: 12.5,
                        fontWeight: FontWeight.w800,
                        height: 1.35,
                      ),
    ...
    // দুই কলামের এক সারি — উপরে সরু বিভাজক রেখা
    Widget statRow(Widget left, Widget? right) => Container(
      decoration: const BoxDecoration(
        border: Border(top: BorderSide(color: _lavenderBorder)),
      ),
      padding: const EdgeInsets.symmetric(vertical: 12),
      child: Row(
    ...
    return Container(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          // ---- কার্ড হেডার: প্রদীপ আইকন + শিরোনাম + ডানে শকাব্দ/বঙ্গাব্দ ----
          Padding(
              ...
              children: [
                ...
                Column(
                  ...
                  children: [
                    ...
                    Text(
                      'বঙ্গাব্দ ${bnNum(info.year)} • $ayana',
          ...
          // ---- সময়ের তথ্য, দুই কলামে ----
          Padding(
            padding: const EdgeInsets.symmetric(horizontal: 16),
            child: Column(
              children: [
                statRow(
                  kv('🌅', 'সূর্যোদয়', bnTime12(sun.sunrise)),
  ...
  Widget build(BuildContext context) {
    ...
    // এই স্ক্রিনে নিজস্ব কোনো হেডার বার বা নিচের নেভিগেশন বার নেই —
    // শুধু ক্যালেন্ডারের কন্টেন্ট। (অ্যাপের নিজের হেডার/নেভিগেশন বাইরে
    // থেকেই আসে, তাই এখানে আবার দিলে দুবার দেখাত।)
    ...
    return Material(
      ...
      child: SafeArea(
        child: ListView(
          ...
          children: [
                  // ---- পঞ্চাঙ্গ তথ্য কার্ড — রেফারেন্সের মতো একদম উপরে ----
                  _sidebarPanel(context, info),
                  const SizedBox(height: 12),
                  // ---- ট্যাব বার (বিবাহ/অন্নপ্রাশন ইত্যাদি ফিল্টার) ----
                  // রেফারেন্স স্ক্রিনশটে এটা নেই, কিন্তু এটা অ্যাপের পুরনো
                  // ফিচার — তাই বাদ না দিয়ে কার্ডের নিচে সরিয়ে আনা হয়েছে,
                  // যাতে উপরের অংশ হুবহু রেফারেন্সের মতো দেখায়।
                  SizedBox(
                    height: 34,
                    child: ListView.separated(
                      scrollDirection: Axis.horizontal,
                      ...
                      itemBuilder: (context, i) {
                        ...
                        return GestureDetector(
                          ...
                          child: Container(
                            ...
                            alignment: Alignment.center,
                            decoration: BoxDecoration(
                              color: active ? _purpleAccent : Colors.white,
                              borderRadius: BorderRadius.circular(999),
                              border: Border.all(
                                color: active
                                    ? _purpleAccent
                  ...
                  Column(
                    children: [
                                  ...
                                  Expanded(
                                    child: Column(
                                      children: [
                                        ...
                                        // দুপাশে ছোট অলংকরণ (রেফারেন্সের মতো)
                                        FittedBox(
                                          fit: BoxFit.scaleDown,
                                          child: Row(
                                            mainAxisSize: MainAxisSize.min,
                            ...
                            GridView.builder(
                              ...
                              gridDelegate:
                                  // fixed উচ্চতার বদলে aspect ratio — ফোন,
                                  // ট্যাব, ওয়েব সব মাপেই ঘরের আকার একই
                                  // অনুপাতে থাকে। ছাপা পঞ্জিকার মতো ঘরে
                                  // তিথি + নক্ষত্র + উৎসবের ছবি ও নাম —
                                  // সব পুরো মাপে বসার মতো জায়গা দেয়।
                              ...
                              itemBuilder: (context, i) {
                                // --- আগের মাসের ধূসর করা দিনগুলো ---
                                // রেফারেন্সের মতো এগুলোও পুরো তথ্য দেখায়
                                // (বাংলা তারিখ + ইংরেজি তারিখ + তিথি), শুধু
                                // ধূসর রঙে — যাতে চলতি মাস আলাদা বোঝা যায়
                                if (i < leading) {
                                  final d = prevTotalDays - leading + 1 + i;
                                  return _outsideDayCell(
                                    _bn(d),
                                    info.start.subtract(
                                ...
                                // --- পরের মাসের ধূসর করা দিনগুলো ---
                                if (i >= leading + totalDays) {
                                  final d = i - leading - totalDays + 1;
                                  return _outsideDayCell(
                                    _bn(d),
                                    info.end.add(Duration(days: d)),
                                  );
                                }
                                // --- বর্তমান মাসের দিন ---
                                ...
                                final isSunday =
                                    greg.weekday == DateTime.sunday;
                                final matchesFilter = filterCat.isEmpty
                                    ? events.isNotEmpty
                                    : events.any(
                                        (e) => e.category == filterCat,
                                      );
                                final dim =
                                    _tab != 'সম্পূর্ণ মাস' && !matchesFilter;
                  ...
                  Padding(
                    ...
                    child: Wrap(
                      ...
                      runSpacing: 9,
                      children: [
                        _legendDot(_gold, 'পূর্ণিমা'),
                        _legendDot(_night, 'অমাবস্যা'),
                        _legendDot(_leaf, 'একাদশী'),
                        _legendDot(_sundayRed, 'উৎসব'),
                      ],
                    ),
                  ),
                  const SizedBox(height: 22),
                  // ---- এই মাসের উৎসব — নাম, তিথি-নক্ষত্র ও ছবি সহ ----
                  _festivalSection(context, info),
                  const SizedBox(height: 22),
                  Container(
                    width: double.infinity,
                    padding: const EdgeInsets.all(14),
                    ...
                    child: _selectedKey == null
                        ...
                        : Builder(
                            builder: (context) {
                              ...
                              final nakIdx = PanchangCalculator
                                  .nakshatraIndexFor(sun.sunrise);
                              final rashiIdx = PanchangCalculator
                                  .rashiIndexFor(sun.sunrise);
                              ...
                              final abhijit = PanchangCalculator
                                  .abhijitMuhurta(
                                    sel,
                                    lat: AppLocation.lat,
                                    lon: AppLocation.lon,
                                  );
                              final (tithiStart, tithiEnd) = PanchangCalculator
                                  .tithiTiming(sun.sunrise);
                              ...
                              headMotif ??= PanchangCalculator
                                      .tithiFor(sun.sunrise)
                                      .name
                                      .contains('পূর্ণিমা')
                                  ? 'moonFull'
                                  : null;
...
class _CalendarDayCell extends StatefulWidget {
  ...
  final Color? Function(List<CalendarEvent> events, bool isSankranti, bool isSunday)
...
class _CalendarDayCellState extends State<_CalendarDayCell> {
  ...
  Widget build(BuildContext context) {
    ...
    // ---- ঘরের নিচের ছোট রঙিন বিন্দু ----
    // পূর্ণিমা সোনালি, অমাবস্যা গাঢ়, একাদশী সবুজ, বাকি উৎসব লাল।
    // উৎসবের নাম/ছবি ঘরে না দিয়ে নিচের "এই মাসের উৎসব" তালিকায় দেখানো
    // হয় — ফোনের ছোট ঘরে ঠেসে দিলে কিছুই পড়া যায় না।
    Color? pip;
...
class _FamilyCalendarScreenState extends State<FamilyCalendarScreen> {
  ...
  void _addMember() {
    ...
    if (name.isEmpty || _pickedDate == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('নাম ও তারিখ দুটোই দিন')),
      );
      return;
    }
  ...
  void _setReminder(FamilyMemberItem m) {
    ...
    ReminderStore.instance
        .add(ReminderItem('${_emojiFor(m.occasion)} ${m.name} — ${m.occasion}', when))
        .then((_) {
          if (mounted) {
            ScaffoldMessenger.of(context).showSnackBar(
                      ...
                      child: Row(
                        children: [
                          ...
                          IconButton(
                            ...
                            onPressed: () =>
                                FamilyMemberStore.instance.remove(m).then((
                                  _,
                                ) {
                                  if (mounted) setState(() {});
                                }),
...
class _FestivalPosterScreenState extends State<FestivalPosterScreen> {
  ...
  Future<void> _share() async {
    ...
    if (festival.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('উৎসবের নাম দিন')),
      );
      return;
    }
    setState(() => _sharing = true);
...
class _PremiumScreenState extends State<PremiumScreen> {
  ...
  Future<void> _onPaymentSuccess(PaymentSuccessResponse response) async {
    try {
      ...
      setState(() => _processing = false);
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('যাচাই ব্যর্থ: ${e.toString().replaceFirst('Exception: ', '')}')),
      );
    }
  }
  ...
  void _onPaymentError(PaymentFailureResponse response) {
    ...
    setState(() => _processing = false);
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(content: Text('পেমেন্ট ব্যর্থ হয়েছে: ${response.message ?? ''}')),
    );
  }
